...
def pixel_to_numpy_dtype(pixel):
    _pixel_to_numpy = {1: 'b',
                       ...
                       320: 'int32',
                       64: 'int64',
                       24: 'uint8',  # RGB 和 RGBA 时以三波段或四波段表示
                       32: 'uint8',
                       6400: 'float64',
                       3200: 'float32', }
...
def decode_tile(bys):
    ...
    import numpy as np
    cells = np.array(proto_tile.values[:]).astype(pixel_to_numpy_dtype(pixel))
    if bands > 1:
...
def decode_geometry(bys):
    if isinstance(bys, ProtoGeometry):
        ...
        elif geo_type is GeometryType.GEOLINE:
            geo = GeoLine()
            for part_points in points:
                geo.add_part(part_points)
        elif geo_type is GeometryType.GEOREGION:
            geo = GeoRegion()
            for part_points in points:
                geo.add_part(part_points)
        else:
...
def encode_proto_geometry(geo, proto_geo):
    ...
    elif geo.type is GeometryType.GEOLINE or geo.type is GeometryType.GEOREGION:
...
def encode_field_default_value_to_pb(value, field_type):
    ...
    if field_type is FieldType.BOOLEAN:
        ...
        return str(value)
    elif field_type is FieldType.WTEXT or field_type == FieldType.TEXT:
        return str(value)
    elif field_type is FieldType.BYTE:
        return str(int(value))
    elif field_type is FieldType.INT16:
        return str(int(value))
    elif field_type is FieldType.INT32:
        return str(int(value))
    elif field_type is FieldType.INT64:
        return str(int(value))
    elif field_type is FieldType.SINGLE:
        return str(float(value))
    elif field_type is FieldType.DOUBLE:
        return str(float(value))
    elif field_type is FieldType.JSONB:
        return str(value)
