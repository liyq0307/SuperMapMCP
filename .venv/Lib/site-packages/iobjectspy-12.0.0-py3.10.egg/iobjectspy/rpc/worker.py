...
from ._utils import (
    ...
    decode_tile,
    decode_process_info,
    read_length_and_bytes,
    write_length_and_bytes,
    read_int,
    write_int)
...
def _create_entry_object(infile, outfile):
    ...
    code = process_info.py_code  # str(code_bytes, 'utf-8')
    entry_class_name = process_info.entry_class  # str(infile.read(read_int(infile)), 'utf-8')
    exec(compile(code, '', 'exec'))
    local_p = locals()
    for key in local_p:
        globals()[key] = local_p[key]
    return locals()[entry_class_name](), process_info.kvargs
...
def _main(infile, outfile):
    ...
    elif command == EXECUTE_PROCESS_TILES:
        return execute_process_tiles(infile, outfile)
    elif command == EXECUTE_PROCESS_PIPE:
