...
def get_output_datasource(value):
    ...
    if isinstance(value, Datasource):
        ...
        return None
    elif isinstance(value, DatasourceConnectionInfo):
    ...
    if result is not None:
        ...
        if not out_datasource._is_existed():
            if isinstance(result, (tuple, list)):
                ...
                if not isinstance(result, Recordset):
                    out_datasource.close()
                return result
        else:
            return result
...
def create_geometry_buffer(geo, distance, prj=None, unit=None):
    ...
    return Geometry._from_java_object(get_jvm().com.supermap.analyst.spatialanalyst.BufferAnalystGeometry. \
                                      createBuffer(geo._jobject, buffer_param, javaPrj))
...
def convert_value_to_python(value, field_type):
    ...
    if field_type == FieldType.DATETIME:
        ...
        try:
            ...
            return None
    elif field_type in (FieldType.SINGLE, FieldType.DOUBLE):
        try:
            return float(value)
        except:
            return None
    elif field_type in (FieldType.WTEXT, FieldType.TEXT, FieldType.CHAR, FieldType.JSONB):
        return str(value)
    elif field_type is FieldType.LONGBINARY:
        if isinstance(value, str):
...
def to_java_point2ds(values):
    if values is None:
        ...
        if isinstance(values, Point2D):
            ...
            return points
        elif isinstance(values, GeoPoint):
            points = jvm.com.supermap.data.Point2Ds()
...
def to_java_point3ds(values):
    if values is None:
        ...
        if isinstance(values, Point3D):
            ...
            points.add(values._jobject)
            return points
        elif isinstance(values, GeoPoint3D):
            points = jvm.com.supermap.data.Point3Ds()
            points.add(values.point._jobject)
            return points
        elif isinstance(values, (list, tuple)):
