...
__all__ = ['DatasourceConnectionInfo', 'Datasource']
...
class DatasourceConnectionInfo(JVMBase):
    ...
    创建 OraclePlus 数据库连接信息::
        ...
        'server'
    """
    def __init__(self, server=None, engine_type=None, alias=None,
                 is_readonly=None, database=None,
                 driver=None, user=None, password=None):
        """
        构造数据源连接对象
    ...
    def _parse_engine_type_from_server(server):
        ...
        elif _server.endswith('.udb') or _server.endswith('.udd'):
            return EngineType.UDB
        elif _server.endswith('.udbx'):
    ...
    def type(self):
        """EngineType: 数据源类型"""
        if self._type is None and self._server is not None:
    ...
    def server(self):
        ...
        if self._server is not None:
            if self._type == EngineType.UDB or self._type == EngineType.UDBX or \
                    self._type == EngineType.VECTORFILE or self._type == EngineType.IMAGEPLUGINS:
                self._server = str(self._server).replace('\\', '/')
            else:
                self._server = str(self._server)
        return self._server
    ...
    def set_server(self, value):
        ...
        """
        if value is not None:
            if self._type == EngineType.UDB or self._type == EngineType.UDBX \
                    or self._type == EngineType.VECTORFILE or self._type == EngineType.IMAGEPLUGINS:
                self._server = str(value).replace('\\', '/')
            else:
    ...
    def alias(self):
        ...
        if self._alias is None:
            if self.server.lower().endswith('.udb') or self.server.lower().endswith('.udbx'):
                ...
                if not Workspace().is_contains_datasource(_alias):
                    self._alias = _alias
                else:
    ...
    def is_same(self, other):
        ...
        if not isinstance(other, DatasourceConnectionInfo):
        ...
        if self.type in set([EngineType.UDB, EngineType.UDBX, EngineType.VECTORFILE, EngineType.MEMORY,
            ...
            if (self.database is not None and len(self.database) > 0) and (
                    other.database is not None and len(other.database) > 0):
                if self.database != other.database:
                    return False
            if (self.user is not None and len(self.user) > 0) and (
                    other.user is not None and len(other.user) > 0):
                if self.user != other.user:
                    return False
            if (self.password is not None and len(self.password) > 0) and (
                    other.password is not None and len(other.password) > 0):
                if self.password != other.password:
                    return False
            if self.is_readonly is not None and other.is_readonly is not None:
    ...
    def make(value):
        ...
        if isinstance(value, DatasourceConnectionInfo):
            ...
            return DatasourceConnectionInfo.make_from_dict(value)
        elif isinstance(value, str):
            ser = value.lower()
            try:
                if ser == ':memory:' or ser.endswith('.udb') or ser.endswith('.udd') or ser.endswith('.udbx'):
                    return DatasourceConnectionInfo(value)
                elif ser.endswith('.dcf'):
                    return DatasourceConnectionInfo.load_from_dcf(value)
                else:
                    conn = DatasourceConnectionInfo.load_from_xml(value)
                    if conn is None:
                        return DatasourceConnectionInfo.from_json(value)
                    else:
                        return conn
...
#
# def _opened_datasource_event_fun(event, workspace):
#     jds = event.getDatasource()
#     ds = Datasource._from_java_object(jds, workspace)
#     workspace._datasources.append(ds)
#
#
# class DatasourceDisconnectedListener(PythonListenerBase):
#     def __init__(self, datasource, fun):
#         self.name = 'DatasourceDisconnected'
#         self.func = fun
#         self.datasource = datasource
#         PythonListenerBase.__init__(self)
#
#     def datasourceDisconnected(self, event):
#         if self.func is not None:
#             self.func(event, self.datasource)
#
#     class Java:
#         implements = ["com.supermap.data.DatasourceDisconnectedListener"]
#
#
# class DatasourceRefreshedListener(PythonListenerBase):
#     def __init__(self, datasource, fun):
#         self.name = 'DatasourceRefreshed'
#         self.func = fun
#         self.datasource = datasource
#         PythonListenerBase.__init__(self)
#
#     def datasourceRefreshed(self, event):
#         if self.func is not None:
#             self.func(event, self.datasource)
#
#     class Java:
#         implements = ["com.supermap.data.DatasourceRefreshedListener"]
#
#
# class DatasetCreatedListener(PythonListenerBase):
#     def __init__(self, ds, fun):
#         self.name = 'DatasetCreated'
#         self.func = fun
#         self.datasource = ds
#         PythonListenerBase.__init__(self)
#
#     def datasetCreated(self, event):
#         if self.func is not None:
#             self.func(event, self.datasource)
#
#     class Java:
#         implements = ["com.supermap.data.DatasetCreatedListener"]
#
#
# class DatasetDeletedAllListener(PythonListenerBase):
#     def __init__(self, ds, fun):
#         self.name = 'DatasetDeletedAll'
#         self.func = fun
#         self.datasource = ds
#         PythonListenerBase.__init__(self)
#
#     def datasetDeletedAll(self, event):
#         if self.func is not None:
#             self.func(event, self.datasource)
#
#     class Java:
#         implements = ["com.supermap.data.DatasetDeletedAllListener"]
#
#
# class DatasetDeletedListener(PythonListenerBase):
#     def __init__(self, ds, fun):
#         self.name = 'DatasetDeletedAll'
#         self.func = fun
#         self.datasource = ds
#         PythonListenerBase.__init__(self)
#
#     def DatasetDeleted(self, event):
#         if self.func is not None:
#             self.func(event, self.datasource)
#
#     class Java:
#         implements = ["com.supermap.data.DatasetDeletedListener"]
#
#
# class DatasetRenamedListener(PythonListenerBase):
#     def __init__(self, ds, fun):
#         self.name = 'DatasetDeletedAll'
#         self.func = fun
#         self.datasource = ds
#         PythonListenerBase.__init__(self)
#
#     def datasetRenamed(self, event):
#         if self.func is not None:
#             self.func(event, self.datasource)
#
#     class Java:
#         implements = ["com.supermap.data.DatasetRenamedListener"]
#
#
# def _rename_dataset_event_fun(event, ds):
#     oldName = event.getOldName()
#     newName = event.getNewName()
#     for dt in ds._datasets:
#         if dt.name == oldName:
#             dt._name = newName
#             return
#
#
# def _created_dataset_event_fun(event, ds):
#     java_dataset = ds._java_object.getDatasets().get(event.getDatasetName())
#     ds._datasets.append(Dataset._from_java_object(java_dataset, ds))
#
#
# def _deleted_dataset_event_fun(event, ds):
#     name = event.getDatasetName()
#     ds._remove_dataset(name)
#
#
# def _deleted_all_dataset_event_fun(event, ds):
#     ds._datasets.clear()
#
#
# def _disconnected_datasource_event_fun(event, ds):
#     ds._datasets.clear()
#
#
# def _refreshed_datasource_event_fun(event, ds):
#     ds._refresh_datasets()
...
class Datasource(JVMBase):
    ...
    # _datasets = []
    # _dataset_created_listener = None
    # _dataset_deleted_listener = None
    # _dataset_deleted_all_listener = None
    # _dataset_rename_listener = None
    # _datasource_refreshed_listener = None
    # _datasource_disconnected_listener = None
    ...
    #     safe_start_callback_server()
    #
    #     if self._dataset_created_listener is None:
    #         self._dataset_created_listener = DatasetCreatedListener(self, _created_dataset_event_fun)
    #         self._java_object.getDatasets().addCreatedListener(self._dataset_created_listener)
    #
    #     if self._dataset_deleted_listener is None:
    #         self._dataset_deleted_listener = DatasetDeletedListener(self, _deleted_dataset_event_fun)
    #         self._java_object.getDatasets().addDeletedListener(self._dataset_deleted_listener)
    #
    #     if self._dataset_deleted_all_listener is None:
    #         self._dataset_deleted_all_listener = DatasetDeletedAllListener(self, _deleted_all_dataset_event_fun)
    #         self._java_object.getDatasets().addDeletedAllListener(self._dataset_deleted_all_listener)
    #
    #     if self._dataset_rename_listener is None:
    #         self._dataset_rename_listener = DatasetRenamedListener(self, _rename_dataset_event_fun)
    #         self._java_object.getDatasets().addRenamedListener(self._dataset_rename_listener)
    #
    #     if self._datasource_refreshed_listener is None:
    #         self._datasource_refreshed_listener = DatasourceRefreshedListener(self, _refreshed_datasource_event_fun)
    #         self._java_object.addRefreshedListener(self._datasource_refreshed_listener)
    #
    #     if self._datasource_disconnected_listener is None:
    #         self._datasource_disconnected_listener = DatasourceDisconnectedListener(self,
    #                                                                                 _disconnected_datasource_event_fun)
    #         self._java_object.addDisconnectedListener(self._datasource_disconnected_listener)

    # def _unregister_listeners(self):
    #     if self._java_object is not None:
    #         self._java_object.getDatasets().removeCreatedListener(self._dataset_created_listener)
    #         self._java_object.getDatasets().removeDeletedListener(self._dataset_deleted_listener)
    #         self._java_object.getDatasets().removeDeletedAllListener(self._dataset_deleted_all_listener)
    #         self._java_object.getDatasets().removeRenamedListener(self._dataset_rename_listener)
    #         self._java_object.removeRefreshedListener(self._datasource_refreshed_listener)
    #         self._java_object.removeDisconnectedListener(self._datasource_disconnected_listener)
    #     close_callback_server()
    ...
    def connection_info(self):
        ...
        # else:
        #     if self._java_object is not None:
        #         self._conninfo = DatasourceConnectionInfo._from_java_object(self._java_object.getConnectionInfo())
        #     return self.
        if self._jobject is None:
            raise ObjectDisposedError('Datasource')
        try:
    ...
    def _from_java_object(java_datasource, workspace=None):
        ...
        if workspace is not None:
            ...
            from .ws import Workspace
            ds._workspace = Workspace()
        # ds._alias = java_datasource.getAlias()
        # ds._refresh_datasets()
        # ds._register_listeners()
        return ds
    ...
    # def _remove_dataset(self, item):
    #     if isinstance(item, int):
    #         if item <= 0:
    #             item += len(self._datasets)
    #         del self._datasets[item]
    #     elif isinstance(item, str):
    #         i = -1
    ...
    #         for i in range(jdatasets.getCount()):
    #             dt = jdatasets.get(i)
    #             _dt = Dataset._from_java_object(dt, self)
    #             if _dt is not None:
    #                 self._datasets.append(_dt)
    ...
    def create_dataset(self, dataset_info, adjust_name=False):
        ...
        try:
            if not self.is_available_dataset_name(dataset_info.name) and adjust_name:
                dataset_info.set_name(self.get_available_dataset_name(dataset_info.name))
            if isinstance(dataset_info, (DatasetVectorInfo, DatasetImageInfo, DatasetGridInfo)):
                jdt = self._jobject.getDatasets().create(dataset_info._jobject)
            # elif isinstance(dataset_info, DatasetGridCollectionInfo):
            #     dt = self._jobject.getDatasets().createGridCollection(dataset_info._jobject)
            # elif isinstance(dataset_info, DatasetImageCollectionInfo):
            #     dt = self._jobject.getDatasets().createImageCollectionInfo(dataset_info._jobject)
            else:
                raise ValueError('invalid datasetInfo, required DatasetVectorInfo,'
                                 'DatasetImageInfo, DatasetGridInfo, but now is ' + str(type(dataset_info)))
    ...
    def create_vector_dataset(self, name, dataset_type, adjust_name=False):
        ...
        if self._jobject is None:
            raise ObjectDisposedError('Datasource')
        try:
            if not self.is_available_dataset_name(name) and adjust_name:
    ...
    # def create_grid_collection_dataset(self, name, pixel_format, adjust_name=False):
    #     """
    #     根据指定的数据集名称和像素格式，创建栅格数据集集合。
    #
    #     :param str name: 数据集名称
    #     :param pixel_format: 可以为像素格式枚举值或名称
    ...
    #         if not self.is_available_dataset_name(name) and adjust_name:
    #             name = self.get_available_dataset_name(name)
    #         return Dataset._from_java_object(self._jobject.getDatasets().createGridCollection(
    #             name, PixelFormat._make(pixel_format)._jobject), self)
    #     except Exception as e:
    #         log_error(e)
    #     return None

    # def create_image_collection_dataset(self, name, adjust_name=False):
    #     """
    #     根据指定的数据集名称，创建影像数据集集合。
    #
    #     :param str name: 数据集名称
    #     :param bool adjust_name: 当数据集名称不合法时，是否自动调整数据集名称，使用一个合法的数据集名称。默认为 False。
    #     :return: 创建成功则返回结果数据集对象，否则返回None
    ...
    #         if not self.is_available_dataset_name(name) and adjust_name:
    #             name = self.get_available_dataset_name(name)
    #         return Dataset._from_java_object(self._jobject.getDatasets().createImageCollection(name), self)
    #     except Exception as e:
    #         log_error(e)
    #     return None
    def create_mosaic_dataset(self, name, prj_coordsys=None, adjust_name=False):
        ...
        if name is None:
            raise ValueError('name is None')
        if not isinstance(prj_coordsys, (PrjCoordSysType, PrjCoordSys, int, str)):
            if prj_coordsys is not None:
                raise TypeError('The type of prj_coordsys must be int or str or PrjCoordSys or PrjCoordSysType object, '
                                'but now is: ' + str(type(prj_coordsys)))
        if self._jobject is None:
            raise ObjectDisposedError('Datasource')
        try:
            ...
            for i in range(select_random_count):
                ...
                for k in range(len(row_values)):
                    ...
                    if k > len(fieldTypes) - 1:
                        ...
                        if fieldTypes[k] is None:
                            fieldTypes[k] = _fieldType
                        else:
                            if _fieldType is not None and fieldTypes[k] != _fieldType:
            ...
            for i in range(select_random_count):
                ...
                if row_data is not None:
                    if isinstance(row_data, (Point2D, GeoPoint)):
                        ...
                        elif datasetType != DatasetType.POINT:
                            datasetType = DatasetType.CAD
                    elif isinstance(row_data, (Point3D, GeoPoint3D)):
                        if datasetType is None:
                            datasetType = DatasetType.POINT3D
                        elif datasetType != DatasetType.POINT3D:
                            datasetType = DatasetType.CAD
                    elif isinstance(row_data, GeoLine):
                        if datasetType is None:
                            datasetType = DatasetType.LINE
                        elif datasetType != DatasetType.LINE:
                            datasetType = DatasetType.CAD
                    elif isinstance(row_data, (Rectangle, GeoRegion)):
                        if datasetType is None:
                            datasetType = DatasetType.REGION
                        elif datasetType != DatasetType.REGION:
                            datasetType = DatasetType.CAD
                    else:
                        datasetType = DatasetType.CAD
                if values is not None:
            ...
            for item in data:
                ...
                try:
                    if isinstance(item, Rectangle):
                        ...
                        isAdd = rd.add(item)
                    if isAdd and values is not None:
                        row_data = values[i]
                        if row_data is not None:
    ...
    def copy_dataset(self, source, out_dataset_name=None, encode_type=None, progress=None):
        ...
        if isinstance(source, str):
            if source.find('/') == -1 and source.find('\\') == -1 and source.find('|') == -1:
    ...
    #     """
    #     返回此数据源是否有改动。true 表示数据源已改动，false 表示数据源还未改动。数据源负责管理数据集，因此当其中的数据集个数等内容发生改变
    #     时，此属性会返回 true。当数据源是以事务方式打开时，关闭数据源时可通过此属性判断是否需要提示用户保存修改内容。
    #
    #     :return: 此数据源是否有改动
    #     :rtype: bool
    #     """
            ...
            if isinstance(item, int):
                ...
                for handle, dt in self._dict_dt.items():
                    if i == item:
                        ...
                        break
                    i += 1
                if target_handle and target_dt:
