...
from ..enums import (DatasetType,
                     ...
                     FieldSign,
                     PyramidResampleType)
from .._utils import (datetime_to_java_date,
                      java_date_to_datetime,
                      split_input_list_from_str,
                      split_input_dict_from_str,
                      ...
                      java_color_to_tuple,
                      oj)
from ..env import get_show_features_count

__all__ = ['Dataset', 'DatasetVector', 'DatasetVectorInfo', 'DatasetImageInfo', 'DatasetGridInfo',
           'DatasetImage', 'DatasetGrid', 'DatasetTopology', 'DatasetMosaic', 'DatasetVolume', 'Colors', 'JoinItem',
    ...
    def _from_java_object(java_dataset, datasource):
        if java_dataset is not None:
            ...
            if data_type is DatasetType.IMAGE:
                dt = DatasetImage()
            elif data_type is DatasetType.GRID:
                dt = DatasetGrid()
            elif data_type is DatasetType.VOLUME:
                dt = DatasetVolume()
            elif data_type is DatasetType.MOSAIC:
                dt = DatasetMosaic()
            elif data_type in [DatasetType.CAD, DatasetType.LINE, DatasetType.POINT, DatasetType.REGION,
                ...
                dt = DatasetVector()
            elif data_type is DatasetType.TOPOLOGY:
                dt = DatasetTopology()
            else:
...
class LinkItem(object):
    ...
    def __init__(self, foreign_keys=None, name=None, foreign_table=None, primary_keys=None, link_fields=None,
                 link_filter=None, connection_info=None):
        """
        构造 LinkItem 对象
...
class QueryParameter(object):
    ...
    def __init__(self, attr_filter=None, cursor_type=CursorType.DYNAMIC, has_geometry=True, result_fields=None,
    ...
    def _from_java_object(java_obj):
        ...
        if spatial_query_object is not None:
            ...
            elif classname == 'com.supermap.data.Recordset':
            ...
            elif classname == 'com.supermap.data.Rectangle2D':
                obj.set_spatial_query_object(Rectangle._from_java_object(spatial_query_object))
            elif classname == 'com.supermap.data.Point2D':
        ...
        if timeConds is not None and len(timeConds) > 0:
        ...
        if java_join_items is not None and java_join_items.getCount() > 0:
            joins = []
        ...
        if java_link_items is not None and java_link_items.getCount() > 0:
            links = []
    ...
    def to_json(self):
        ...
        if 'spatial_query_object' in d.keys():
            ...
            if isinstance(obj, Point2D):
                dp = {'type': 'P2', 'data': obj.to_json()}
            elif isinstance(obj, Rectangle):
                dp = {'type': 'RC', 'data': obj.to_json()}
            elif isinstance(obj, Geometry):
                dp = {'type': 'G', 'data': obj.to_json()}
            elif isinstance(obj, Recordset):
                dp = {'type': 'RD', 'data': obj.to_json()}
            elif isinstance(obj, DatasetVector):
                dp = {'type': 'DT', 'data': obj.to_json()}
            else:
                dp = None
            if dp is not None:
...
class DatasetVector(Dataset):
    ...
    def __str__(self):
        ...
        all_field_infos = self.field_infos
        ignore_fields = set(
            ['SmSdriW', 'SmSdriN', 'SmSdriE', 'SmSdriS', 'SmGeometrySize', 'SmGeoPosition', 'SmGeometry'])
        fields = list(filter(lambda field: field.name not in ignore_fields, all_field_infos))
        names = list(field.name for field in fields)
        rd = self.get_recordset(cursor_type='STATIC', fields=names)
        ...
        top_10_values = []
        i = 0
        top_count = get_show_features_count()
        while rd.has_next() and i < top_count:
            values = list(rd.get_value(name) for name in names)
            values_str = []
    ...
    def get_recordset(self, is_empty=False, cursor_type=CursorType.DYNAMIC, fields=None):
        ...
        try:
            self.open()
            if fields is None:
                rd = Recordset._from_java_object(
                    self._jobject.getRecordset(is_empty, CursorType._make(cursor_type, CursorType.DYNAMIC)._jobject),
                    self)
                if rd is not None:
                    d = {'func': 'get_recordset',
                         'params': {'is_empty': is_empty, 'cursor_type': CursorType._make(cursor_type,
                                                                                          CursorType.DYNAMIC).name}}
    ...
    def query_with_ids(self, ids, id_field_name='SmID', cursor_type=CursorType.DYNAMIC):
        ...
        try:
            ...
            java_recordset = self._jobject.query(to_java_int_array(ids), _idFieldName,
                                                 CursorType._make(cursor_type, CursorType.DYNAMIC)._jobject)
            if java_recordset is not None:
                rd = Recordset._from_java_object(java_recordset, self)
                if rd is not None:
                    rd._set_custom_params({'func': 'query_with_ids',
                                           'params': {'ids': ids,
                                                      'id_field_name': id_field_name,
                                                      'cursor_type': CursorType._make(
                                                          cursor_type,
                                                          CursorType.DYNAMIC).name}})
                return rd
            else:
    ...
    def query_with_bounds(self, bounds, attr_filter=None, cursor_type=CursorType.DYNAMIC):
        ...
        try:
            ...
            if bounds is None:
                bounds = self.bounds
            java_recordset = self._jobject.query(bounds._jobject, attr_filter,
                                                 CursorType._make(cursor_type, CursorType.DYNAMIC)._jobject)
            if java_recordset is not None:
                rd = Recordset._from_java_object(java_recordset, self)
                if rd is not None:
                    rd._set_custom_params({'func': 'query_with_bounds',
                                           'params': {'bounds': bounds.to_json(),
                                                      'attr_filter': attr_filter,
                                                      'cursor_type': CursorType._make(cursor_type,
                                                                                      CursorType.DYNAMIC).name}})
                return rd
            else:
                return None
    ...
    def query_with_distance(self, geometry, distance, unit=None, attr_filter=None, cursor_type=CursorType.DYNAMIC):
        ...
        try:
            ...
            if bufferGeo is not None:
                ...
                queryParam.set_spatial_query_object(bufferGeo). \
                    set_spatial_query_mode(SpatialQueryMode.INTERSECT). \
                    set_attribute_filter(attr_filter). \
    ...
    def query_with_filter(self, attr_filter=None, cursor_type=CursorType.DYNAMIC, result_fields=None,
                          has_geometry=True):
        """
        根据指定的属性过滤条件查询记录集
        ...
        try:
            self.open()
            queryParam = QueryParameter().set_attribute_filter(attr_filter). \
                set_cursor_type(cursor_type). \
                set_result_fields(result_fields). \
                set_has_geometry(has_geometry). \
                set_order_by(['SmID ASC'])
            java_recordset = self._jobject.query(queryParam._jobject)
            if java_recordset is not None:
                return Recordset._from_java_object(java_recordset, self)
            else:
        ...
        try:
            if isinstance(item, int):
                if item < 0:
                    item += len(self.field_infos)
            self.open()
            return get_jvm().com.supermap.jsuperpy.DatasetUtils.updateFieldsEx(self._jobject, item, express,
                                                                               attr_filter)
        except Exception as e:
            log_error(e)
            return None
    ...
    def update_field(self, item, value, attr_filter=None):
        ...
        try:
            ...
            return get_jvm().com.supermap.jsuperpy.DatasetUtils.updateFields(self._jobject, item, _value,
                                                                             attr_filter)
        except Exception as e:
            log_error(e)
            return None
    ...
    def build_field_index(self, field_names, index_name):
        ...
        try:
            self.open()
            return self._jobject.buildFieldIndex(to_java_string_array(split_input_list_from_str(field_names)),
                                                 index_name)
        except Exception as e:
            log_error(e)
        return False
    ...
    def append(self, data, fields=None):
        """
        往当前数据集中追加记录。写入的数据可以：
        ...
        try:
            if isinstance(fields, str):
                fields = split_input_dict_from_str(fields)
            ...
            if isinstance(data, Recordset):
                if source is not None and len(source) > 0 and target is not None and len(source) == len(target):
                    is_success = self._jobject.append(data._jobject, to_java_string_array(source),
                                                      to_java_string_array(target))
                else:
                    is_success = self._jobject.append(data._jobject)
            elif isinstance(data, DatasetVector):
                if source is not None and len(source) > 0 and target is not None and len(source) == len(target):
                    rd = data.get_recordset(False, CursorType.STATIC, source)
                    is_success = self._jobject.append(data._jobject, to_java_string_array(source),
                                                      to_java_string_array(target))
                    rd.close()
                else:
                    rd = data.get_recordset(False, CursorType.STATIC)
                    is_success = self._jobject.append(rd._jobject)
                    rd.close()
            elif isinstance(data, Geometry):
                srd = self.get_recordset(True, CursorType.DYNAMIC, ['SmID'])
                srd.batch_edit()
                srd.add(data)
                srd.batch_update()
                srd.close()
                ...
                srd = self.get_recordset(True, CursorType.DYNAMIC, ['SmID'])
                srd.batch_edit()
                srd.add(GeoRegion(data))
                srd.batch_update()
                srd.close()
                is_success = True
            elif isinstance(data, Feature):
                if source is not None and len(source) > 0 and target is not None and len(source) == len(target):
                    srd = self.get_recordset(True, CursorType.DYNAMIC, source)
                    srd.batch_edit()
                    srd.add(data.geometry)
                    for _s, _t in fields.items():
                        srd.set_value(_t, data.get_value(_s))
                    srd.batch_update()
                    srd.close()
                    is_success = True
                else:
                    srd = self.get_recordset(True, CursorType.DYNAMIC)
                    srd.batch_edit()
                    values = data.get_values(True, True)
                    srd.add(data.geometry, values)
                    srd.batch_update()
                    srd.close()
                    is_success = True
            elif isinstance(data, Point2D):
                srd = self.get_recordset(True, CursorType.DYNAMIC, ['SmID'])
                srd.batch_edit()
                srd.add(GeoPoint(data))
                srd.batch_update()
                srd.close()
                is_success = True
                ...
                for item in data:
                    try:
                        if isinstance(item, (Recordset, DatasetVector)):
                            rds.append(item)
                        else:
                            if isinstance(item, Geometry):
                                srd.add(item)
                            elif isinstance(item, Feature):
                                if source is not None and len(source) > 0 and target is not None and len(source) == len(
                                        target):
                                    srd.add(item.geometry)
                                    for _s, _t in fields.items():
                                        srd.set_value(_t, item.get_value(_s))
                                else:
                                    values = item.get_values(True, True)
                                    srd.add(item.geometry, values)
                            elif isinstance(item, Rectangle):
                                srd.add(GeoRegion(item))
                            elif isinstance(item, Point2D):
                                srd.add(GeoPoint(item))
                    except Exception as e:
                        log_warning(e)
                srd.batch_update()
                srd.close()
                for rd in rds:
                    try:
                        self.append(rd, fields)
                    except Exception as e:
                        log_warning(e)
                is_success = True
            else:
                log_error('Unsupported data type ' + str(data))
                return False
            return is_success
        except Exception as e:
            log_error(e)
    ...
    def append_fields(self, source, source_link_field, target_link_field, source_fields, target_fields=None):
        ...
        try:
            ...
            if target_fields is not None:
                ...
                result = self._jobject.appendFields(source_dt._jobject, str(source_link_field),
                                                    str(target_link_field), source_names, target_names)
            else:
                result = self._jobject.appendFields(source_dt._jobject, str(source_link_field),
                                                    str(target_link_field), source_names)
            return result
        except Exception as e:
            log_error(e)
            return False
...
class SpatialIndexInfo(object):
    ...
    def make_tile(tile_width, tile_height):
        ...
        :rtype: SpatialIndexInfo
        """
        return SpatialIndexInfo().set_type(SpatialIndexType.TILE).set_tile_width(tile_width).set_tile_height(
            tile_height)
    ...
    def make_mgrid(center, grid_size0, grid_size1, grid_size2):
        ...
        :rtype: SpatialIndexInfo
        """
        return SpatialIndexInfo().set_type(SpatialIndexType.MULTI_LEVEL_GRID).set_grid_center(center).set_grid_size0(
            grid_size0).set_grid_size1(grid_size1).set_grid_size2(grid_size2)
...
class Colors(object):
    ...
    def make_gradient(count, gradient_type, reverse=False, gradient_colors=None):
        ...
        if _type is None and gradient_colors is None:
        ...
        if _type is None:
            ...
            java_obj = jvm.com.supermap.data.Colors.makeGradient(count,
                                                                 to_java_color_array(gradient_colors.values()))
        else:
    ...
    def make_random(count, colors=None):
        ...
        :rtype: Colors
        """
        if colors is not None and colors is not Colors:
...
class DatasetGridInfo(object):
    ...
    def __init__(self, name=None, width=None, height=None, pixel_format=None, encode_type=None,
                 block_size_option=BlockSizeOption.BS_256):
        """
        构造栅格数据集信息对象
        ...
        (self.set_name(name).set_width(width).set_height(height).set_pixel_format(pixel_format).
         set_encode_type(encode_type).set_block_size_option(block_size_option))
...
class DatasetGrid(Dataset):
    ...
    def build_value_table(self, out_data=None, out_dataset_name=None):
        ...
        if self.pixel_format is PixelFormat.DOUBLE or self.pixel_format is PixelFormat.SINGLE:
    ...
    def xy_to_grid(self, point):
        ...
        if pnt is not None:
            return int(pnt.getY()), int(pnt.getX())
        else:
            return None
...
# class DatasetGridCollectionInfo(object):
#     """
#     栅格数据集集合信息
#     """
#
#     def __init__(self, name=None, block_size_option=None, pixel_format=None, encode_type=None):
#         self._name = None
#         self._block_size_option = None
#         self._pixel_format = None
#         self._encode_type = None
#
#         self.set_name(name).set_block_size_option(block_size_option).set_pixel_format(pixel_format). \
#             set_encode_type(encode_type)
#
#     @property
#     def name(self):
#         """str: 栅格数据集集合名字"""
...
#     @property
#     def block_size_option(self):
#         """BlockSizeOption: 栅格数据集集合的栅格块大小类型"""
#         return self._block_size_option
#
#     @property
#     def pixel_format(self):
...
#         设置数据集集合名称
#
#         :param str value: 数据集合名称
#         :return: self
#         :rtype: DatasetGridCollectionInfo
#         """
#         if value is not None:
#             self._name = value
#         return self
...
#     @property
#     def _jobject(self):
#         java_obj = get_jvm().com.supermap.data.DatasetGridCollectionInfo()
#         if self.name is not None:
#             java_obj.setName(self.name)
#         if self.block_size_option is not None:
#             java_obj.setBlockSizeOption(self.block_size_option._jobject)
...
#     def make_from_dict(values):
#         """
#         从 dict 中读取信息构造 DatasetGridCollectionInfo 对象
#
#         :param values: DatasetGridCollectionInfo 信息
#         :type values: dict
#         :rtype: DatasetGridCollectionInfo
#         """
#         return DatasetGridCollectionInfo().from_dict(values)
#
#     def from_dict(self, values):
#         """
#         从 dict 中读取信息
...
# class DatasetGridCollection(Dataset):
#     """
#     栅格数据集集合类型的数据集类。
#
#     栅格数据集集合是一种数据集类型，派生于 Dataset，对该类型的数据集可以创建、更新和删除影像金字塔、重命名数据集等；它又具有
#     集合的功能，能对集合中栅格数据集进行添加、删除和追加等操作。
#
...
#         Dataset.__init__(self)
#
#     def add(self, name, alias, is_top=False):
#         """
#         将栅格数据添加到栅格数据集集合的最顶层或尾部。
#
#         使用该方法有以下四点注意事项：
#
#             * 目前只支持和栅格数据集集合在同一数据源下的栅格数据集的添加。
#             * 所添加的栅格数据集的投影必须与栅格数据集集合一致，否则不能加入集合。
#             * 不限制添加栅格数据集的像素格式。但是建议一个集合中的各子数据集像素格式保持一致。
#             * 所指定的别名必须合法且在集合中是唯一的，否添加失败。
#
#
#         :param name:  栅格数据集的名称。
#         :type name: str
#         :param alias:  添加到栅格数据集集合中的栅格数据集的别名，别名需合法且唯一
#         :type alias: str
#         :param is_top: 是否添加到最顶层，为 True，则添加在最顶层，其索引值为0，反之显示在尾部
#         :type is_top: bool
#         :return: 添加是否成功
#         :rtype: bool
#         """
...
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.add(name, alias, is_top)
#
#     def add_all(self, values):
#         """
#         以映射方式向栅格数据集集合中添加栅格数据集。由于别名唯一，设置为键值，即键值为栅格数据集的别名，值为栅格数据集的名称。
#
#         使用该方法有以下四点注意事项：
#
#             * 目前只支持和栅格数据集集合在同一数据源下的栅格数据集的添加。
#             * 所添加的栅格数据集的投影必须与栅格数据集集合一致，否则不能加入集合。
#             * 不限制添加栅格数据集的像素格式。但是建议一个集合中的各子数据集像素格式保持一致。
#             * 所指定的别名必须合法且在集合中是唯一的，否则添加失败。
#
#         :param values:  需要添加的栅格数据集别名和名称组成的映射对象，键值为栅格数据集的别名，值为栅格数据集的名称。
#         :type values: dict[str,str]
#         :return: 添加是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
#             log_error('DatasetGridCollection object has been disposed')
#         if not isinstance(values, dict):
#             raise ValueError('values must be dict')
#         self.open()
#         return self._jobject.add(values)
#
#     def insert(self, name, alias, index):
#         """
#         将栅格数据集插入到栅格数据集集合指定的位置处。
#         使用该方法有以下几点注意事项：
#
#          * 目前只支持和栅格数据集集合在同一数据源下的栅格数据集的插入。
#          * 所插入的栅格数据的投影必须与栅格数据集集合一致，否则不能插入到集合中。
#          * 不限制插入的栅格数据的像素格式。但是建议一个集合中的各子数据集像素格式保持一致。
#          * 所指定的别名必须合法且在集合中是唯一的，否则插入失败。
#
#         :param name:  栅格数据集的名称。
#         :type name: str
#         :param alias: 添加到栅格数据集集合中的栅格数据集的别名，别名需合法且唯一。
...
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.insert(name, alias, int(index))
#
#     def build_pyramid(self, progress=None):
#         """
#         为栅格数据集集合对象创建影像金字塔。目的是加快全幅显示速度
#
...
#         栅格数据集集合是否创建影像金字塔
#
#         :return: 返回 True 表示已经创建，False 表示未创建。
#         :rtype: bool
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
...
#         删除栅格数据集集合对象的影像金字塔
#
#         :return: 删除是否成功，成功返回 True，否则为 False
#         :rtype: bool
#         """
#         if self._jobject is None:
#             log_error('DatasetGridCollection object has been disposed')
...
#     def update_pyramid(self, rect):
#         """
#         根据指定范围更新栅格数据集集合的影像金字塔。 当对栅格数据集集合执行以下操作后，需要调用该方法：
#
#             * 修改子数据集的裁剪区域；
#             * 调整子数据集的顺序，执行添加、删除子数据集等操作；
#             * 当添加的子数据集的范围在已有金字塔范围之外。
#
#         :param rect:  指定的范围
#         :type rect: Rectangle
#         :return: 更新是否成功
...
#     def set_clip_region(self, region):
#         """
#         设置栅格数据集集合的裁剪区域。栅格数据集集合的裁剪区域用来控制所有子数据集的显示范围。
#         调用该方法无法对集合内的子数据集设置裁剪区域。子数据集裁剪区域的需子栅格数据集对象通过 :py:meth:`.DatasetGrid.set_clip_region`
#         方法设置。
#
#         注意：
#
#         * 当重新设置栅格数据集集合的地理范围，不自动修改栅格数据集集合的裁剪区域。
#
#         :param region: 栅格数据集集合的裁剪区域
#         :type region: GeoRegion or Rectangle
#         :return: self
#         :rtype: DatasetGridCollection
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
...
#     @property
#     def no_value(self):
#         """float: 返回集合的无值，对所有子数据集有效，主要用于创建集合金字塔时，忽略子数据集无值"""
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.getNoValue()
...
#     def set_no_value(self, value):
#         """
#         设置集合的无值，对所有子数据集有效，主要用于创建集合金字塔时，忽略子数据集无值。
#
...
#     def set_color_table(self, colors):
#         """
#         设置栅格数据集集合的颜色表。 无论是否对子数据集对象设置颜色表，栅格数据集集合显示时使用集合的颜色表。
#
#         :param colors: 颜色表对象，即颜色集合。
#         :type colors: Colors
#         :return: self
#         :rtype: DatasetGridCollection
#         """
...
#         return self
#
#     def append(self, dataset_collection):
#         """
#         向栅格数据集集合对象中追加另外一个栅格数据集集合。 若追加成功，则所追加的栅格数据集集合中的所有栅格数据集按照其原有顺
#         序添加到被追加的集合的尾部。
#
#         :param dataset_collection: 待追加的栅格数据集集合对象
#         :type dataset_collection: DatasetGridCollection
#         :return: 追加是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
...
#         self.open()
#         from ._util import get_input_dataset
#         dt = get_input_dataset(dataset_collection)
#         return self._jobject.append(dt._jobject)
#
#     def change_order(self, alias, index):
#         """
#         改变栅格数据集在栅格数据集集合中的位置。调整栅格数据集集合中子数据集的顺序，可以改善集合的显示效果。
#         注意：该方法建议在对栅格数据集集合创建影像金字塔之前调用，以减少对集合金字塔的创建和更新。
#
#         :param str alias: 需要改变位置的栅格数据集的别名
#         :param int index: 栅格数据集移动到集合中的索引值。
#         :return: 位置改变是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.changeOrder(alias, index)
#
#     def get_attribute_dataset(self):
#         """
#         返回记录栅格数据集合信息的矢量数据集。
#         返回的矢量数据集是一个面数据集，不允许用户修改。
#         该面数据集的一个面对象对应一个子数据集。如果子数据集含有裁剪区域，则面对象即是每个子数据集的裁剪区域，如果子数据集不含
#         有裁剪区域，则面对象是每个子数据集最小外接矩形。
#         这个面数据集的字段除了包含系统字段外，还包含其他描述影像子数据集信息的字段，字段名称和字段中存储的信息含义如下表所示
#
#         .. image:: ../image/AttributeDataset.png
#
#
#         :return: 记录影像数据集合信息的面数据集。
#         :rtype: DatasetVector
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return Dataset._from_java_object(self._jobject.getAttributeDataset(), self.datasource)
#
#     def get_available_alias_name(self, alias):
#         """
#         根据传入字符串获取一个合法的栅格数据集集合的别名
#
#         :param str alias: 输入的字符串
#         :return: 一个合法的栅格数据集集合的别名
#         :rtype: str
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.getAvailableAliasName(alias)
#
#     def is_available_alias_name(self, alias):
#         """
#         判断传入字符串是否是一个合法的栅格数据集集合的别名
#
#         :param str alias: 输入的字符串
#         :return: 传入的字符串合法返回 True，不合法返回 False
#         :rtype: bool
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.isAvailableAliasName(alias)
#
#     def get_child_dataset_alias(self, index):
#         """
#         返回栅格数据集集合中指定序号的栅格数据集对象的别名
#
#         :param int index: 指定的序号
#         :return: 指定序号的栅格数据集对象的别名。
#         :rtype: str
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.get(int(index))
#
#     def get_child_dataset(self, alias):
#         """
#         返回栅格数据集集合中指定别名的栅格数据集对象
#
#         :param str alias: 栅格数据集的别名
#         :return: 指定的别名对应的栅格数据集对象
#         :rtype: DatasetGrid
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return Dataset._from_java_object(self._jobject.getDataset(alias), self.datasource)
#
#     def index_of_child_dataset(self, alias):
#         """
#         返回指定别名对应的子数据集在栅格数据集集合中的序号。
#
#         :param str alias:
#         :return: 指定别名对应的子数据集在栅格数据集集合中的序号
#         :rtype: int
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.getDatasets().indexOf(alias)
#
#     def remove(self, item):
#         """
#         根据指定的别名或序号从栅格数据集集合中移除栅格数据集。
#         注意：删除的只是栅格数据集在集合中的信息，与子数据集关联的同一数据源下的栅格数据集不会被删除。
#
#         :param item:  指定的栅格数据集别名或序号
#         :type item: int or str
#         :return: 移除是否成功
#         :rtype: bool
#         """
...
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.remove(item)
#
#     def remove_all(self):
#         """
...
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.removeAll()
#
#     def rename_sub(self, alias, new_alias):
#         """
#         重命名栅格数据集集中数据集的别名。重命名不区分大小写。
#         如果数据源以只读的方式打开，将不能重命名。
#
#         :param str alias: 原始的别名
#         :param str new_alias: 新的别名
#         :return: 重命名是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.rename(alias, new_alias)
#
...
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.synchronize()
#
#     @property
#     def count(self):
#         """int: 栅格数据集集合中栅格数据集的个数"""
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
...
class DatasetImageInfo(object):
    ...
    def __init__(self, name=None, width=None, height=None, pixel_format=None, encode_type=None,
                 block_size_option=BlockSizeOption.BS_256, band_count=None):
        """
        构造影像数据集信息对象
        ...
        self.set_name(name).set_width(width).set_height(height).set_pixel_format(pixel_format). \
            set_encode_type(encode_type).set_block_size_option(block_size_option).set_band_count(band_count)
...
class DatasetImage(Dataset):
    ...
    def set_no_value(self, value, band):
        ...
        :param value: 指定的无值。
        :type value: float or tuple[int,int,int] or tuple[int,int,int,int]
        :param int band: 指定的波段索引号，从 0 开始。
        """
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
    ...
    #         return None
    #     self.open()
    #     return ColorSpaceType._make(self._jobject.getColorSpace().name())
    #
    ...
    #
    #     :param value: 色彩显示模式
    #     :type value: ColorSpaceType or str
    #     """
    #     if self._jobject is None:
    #         raise ObjectDisposedError(type(self).__name__)
    #     self.open()
    #     self._jobject.setColorSpace(ColorSpaceType._make(value)._jobject)


# class DatasetImageCollectionInfo:
#     """
#     影像数据集集合信息类。该类用于设置影像数据集集合的创建信息，包括名称、像素格式、像素分块类型、波段数和编码方式等。
#     """
#
#     def __init__(self, name=None, block_size_option=None, pixel_format=None, encode_type=None, band_count=None):
#         self._name = None
#         self._block_size_option = None
#         self._pixel_format = None
#         self._encode_type = None
#         self._band_count = None
#
#         self.set_name(name).set_block_size_option(block_size_option).set_pixel_format(pixel_format).set_encode_type(
#             encode_type).set_band_count(band_count)
#
#     @property
#     def name(self):
#         """str: 数据集名称"""
...
#     @property
#     def block_size_option(self):
#         """BlockSizeOption: 影像数据集集合的像素分块类型"""
#         return self._block_size_option
#
#     @property
#     def pixel_format(self):
#         """PixelFormat: 影像数据集集合的像素格式"""
#         return self._pixel_format
#
#     @property
#     def encode_type(self):
#         """EncodeType: 影像数据集集合的编码方式"""
#         return self._encode_type
#
#     @property
#     def band_count(self):
...
#         设置影像数据集集合的名称
#
#         :param str value: 影像数据集集合的名称
#         :return: self
#         :rtype: DatasetImageCollectionInfo
#         """
#         self._name = value
#         return self
#
...
#         """
#         设置影像数据集集合的波段数
#         创建影像数据集集合时，波段数可以设置为 0，此时，像素格式（pixel_format）和编码格式（encode_type）的设置是无效的，因为
#         这些信息是针对波段而言，因此波段为 0 时无法保存。此影像数据集集合的波段数、像素格式和编码格式，将以向其中添加的第一个子
#         数据集的相关信息为准。
#
#         :param int value: 影像数据集集合的波段数
#         :return: self
#         :rtype: DatasetImageCollectionInfo
#         """
...
#     @property
#     def _jobject(self):
#         java_obj = get_jvm().com.supermap.data.DatasetImageCollectionInfo()
#         if self.name is not None:
#             java_obj.setName(self.name)
#         if self.block_size_option is not None:
#             java_obj.setBlockSizeOption(self.block_size_option._jobject)
...
#     @staticmethod
#     def _from_java_object(java_obj):
#         obj = DatasetImageCollectionInfo()
#         obj.set_name(java_obj.getName())
#         obj.set_block_size_option(java_obj.getBlockSizeOption().name())
#         obj.set_pixel_format(java_obj.getPixelFormat().name())
#         obj.set_band_count(java_obj.getBandCount())
#         obj.set_encode_type(java_obj.getEncodeType().name())
#         return obj
#
#     def to_dict(self):
#         """
...
#     def make_from_dict(values):
#         """
#         从 dict 中构建 DatasetImageCollectionInfo 对象
#
#         :param values: 包含 DatasetImageCollectionInfo 信息的 dict 对象
#         :type values: dict
#         :rtype: DatasetImageCollectionInfo
#         """
#         return DatasetImageCollectionInfo().from_dict(values)
#
#     def from_dict(self, values):
#         """
#         从 dict 中读取 DatasetImageCollectionInfo 信息
#
#         :param values: 包含 DatasetImageCollectionInfo 信息的 dict 对象
#         :type values: dict
#         :return: self
...
# class DatasetImageCollection(Dataset):
#     """
#
#     影像数据集集合类型的数据集类。
#
#     影像数据集集合是一种数据集类型，派生于 Dataset，对该类型的数据集可以创建、更新和删除影像金字塔、重命名数据集等；它又具有集
#     合的功能，能对集合中影像数据集进行添加、删除和追加等操作。
#
#     能添加到影像数据集集合中影像数据有两种类型，一种是影像数据包括影像数据集 DatasetImage，另一种是影像插件所支持的影像文件
#     （如 *.sit）。
#
#     注意：
#
...
#      * 只有当集合中无子数据集时，对其设置投影才有效；有子数据集时，不允许修改集合的投影信息，即设置投影无效。
#      * 当添加的子数据集行列数大于5000的时候，添加到地图窗口，数据集和集合都没有金字塔的情况下，该子数据集在原始分辨率下才能显示。
#      * 在对影像数据集集合创建金字塔时，所指定的（NoData）无值会被忽略，因此，对于大量需要背景透明的影像数据，可以通过将背景色
#        设置为无值，然后构建金字塔的方式来处理。
#
#     """
#
...
#         使用该方法有以下几点注意事项：
#
#             * 目前只支持和影像数据集集合在同一数据源下的影像数据集的添加。
#             * 所添加的影像数据的投影必须与影像数据集集合一致，否则不能加入集合。
#             * 不限制添加影像数据的像素格式。但是建议一个集合中的各子数据集像素格式保持一致。
#             * 若添加的影像数据具有多波段，则每个影像数据的波段数必须一致。目前不支持添加多波段的影像文件，仅支持添加同一数据源下的
#               多波段影像数据集。
#             * 所指定的别名必须合法且在集合中是唯一的，否则添加失败。
#
#         :param str name:  影像数据的名称。若影像数据为 DatasetImage 时，该参数为数据集的名称；若为影像文件时，该参数为影像文件的全路径。
#         :param str alias: 添加到影像数据集集合中的影像数据集的别名，别名需合法且唯一。
#         :param bool is_top: 是否添加到最顶层，为 True，则添加在最顶层，其索引值为0，反之显示在尾部。
#         :return: 添加是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
...
#             * 所添加的影像数据的投影必须与影像数据集集合一致，否则不能加入集合。
#             * 不限制添加影像数据的像素格式。但是建议一个集合中的各子数据集像素格式保持一致。
#             * 若添加的影像数据具有多波段，则每个影像数据的波段数必须一致。目前不支持添加多波段的影像文件，仅支持添加同一数据源
#               下的多波段影像数据集。
#             * 所指定的别名必须合法且在集合中是唯一的，否则添加失败。
#
#         :param dict values: 需要添加的影像数据别名和名称组成的映射对象，键值为影像数据集的别名，值为影像数据的名称。
#         :return: dict[str,str]
#         :rtype: bool
#         """
#         if self._jobject is None:
...
#         使用该方法有以下几点注意事项：
#
#             * 目前只支持和影像数据集集合在同一数据源下的影像数据集的插入。
#             * 所插入的影像数据的投影必须与影像数据集集合一致，否则不能插入到集合中。
#             * 不限制插入的影像数据的像素格式。但是建议一个集合中的各子数据集像素格式保持一致。
#             * 若插入的影像数据具有多波段，则每个影像数据的波段数必须一致。目前不支持插入多波段的影像文件，仅支持插入同一数据源
#               下的多波段影像数据集。
#             * 所指定的别名必须合法且在集合中是唯一的，否则插入失败。
#
#         :param str name: 影像数据的名称。若影像数据为 DatasetImage 时，该参数为数据集的名称；若为影像文件时，该参数为影像文件的全路径。
#         :param str alias: 插入到影像数据集集合中的影像数据集的别名，别名需合法且唯一。
#         :param int index: 指定的索引值。
#         :return: 插入是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
...
#         返回影像数据集集合是否创建金字塔。
#
#         :return: True 表示已经创建，False 表示未创建
#         :rtype: bool
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
...
#     def update_pyramid(self, rect):
#         """
#         根据指定范围更新影像数据集集合的影像金字塔。当对影像数据集集合执行以下操作后，需要调用该方法：
#
#             * 修改子数据集的调色板、裁剪区域；
#
#             * 调整子数据集的顺序，执行添加、删除子数据集等操作；
...
#
#         注意：当重新设置影像数据集集合的地理范围，不自动修改影像数据集集合的裁剪区域
#
#         :param region: 裁剪区域
#         :type region: GeoRegion or Rectangle
#         :return: self
#         :rtype: DatasetImageCollection
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
...
#         return self
#
#     def get_no_value(self, band):
#         """
#         返回影像数据集集合指定波段的无值。
#
#         :param int band:  指定的波段索引号，从 0 开始
#         :return: 影像数据集集合指定波段的无值
#         :rtype: float
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.getNoValue(int(band))
#
#     def set_no_value(self, value, band):
#         """
...
#             return
#         self.open()
#         self._jobject.setNoValue(float(value), int(band))
#         return self
#
#     @property
#     def pixel_format(self):
...
#     @property
#     def palette(self):
#         """Clolors: 影像数据集集合的颜色调色板。 """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return Colors._from_java_object(self._jobject.getPalette())
#
#     def set_palette(self, colors):
#         """
#         设置影像数据集集合的颜色调色板。 影像数据集集合显示时使用子数据集自身的调色板。
#         调用该方法无法设置集合内子数据集的调色板。
#
#         :param Colors colors: 颜色调色板对象，即颜色集合
#         :return: self
#         :rtype: DatasetImageCollection
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         self._jobject.setPalette(colors._jobject)
#         return self
#
#     def append(self, dataset_collection):
#         """
#         向影像数据集集合对象中追加另外一个影像数据集集合。若追加成功，则所追加的影像数据集集合中的所有影像数据按照其原有顺序添
#         加到被追加的集合尾部。
#
#         :param dataset_collection:  待追加的影像数据集集合对象
#         :type dataset_collection: DatasetImageCollection
#         :return: 追加是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
#             log_error('DatasetImageCollection object has been disposed')
#             return None
#         self.open()
#         from ._util import get_input_dataset
#         dt = get_input_dataset(dataset_collection)
...
#
#         :param str alias: 需要改变位置的影像数据集的别名
#         :param int index: 影像数据移动到集合中的索引值。
#         :return: 位置改变是否成功
#         :rtype: bool
#         """
#         if self._jobject is None:
...
#         返回记录影像数据集合信息的矢量数据集。
#         返回的矢量数据集是一个面数据集，不允许用户修改。
#         该面数据集的一个面对象对应一个子数据集。如果子数据集含有裁剪区域，则面对象即是每个子数据集的裁剪区域，如果子数据集不
#         含有裁剪区域，则面对象是每个子数据集最小外接矩形。
#         这个面数据集的字段除了包含系统字段外，还包含其他描述影像子数据集信息的字段，字段名称和字段中存储的信息含义如下表所示
#
#         .. image:: ../image/AttributeDataset.png
#
...
#
#         :param str alias: 影像数据集的别名
#         :return: 指定的别名对应的影像数据集对象
#         :rtype: DatasetImage
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
...
#
#         :param str alias: 指定的别名
#         :return: 指定别名对应的子数据集在影像数据集集合中的序号
#         :rtype: int
#         """
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
...
#     def remove(self, item):
#         """
#         根据指定的别名或序号从影像数据集集合中移除影像数据集。
#         注意：删除的只是影像数据集和影像文件在集合中的信息，与子数据集关联的同一数据源下的影像数据集和存放在磁盘上的影像文
#         件不会被删除。
#
#         :param item: 指定的影像数据集别名或序号
#         :type item: str or int
#         :return: 移除是否成功
#         :rtype: bool
#         """
...
#         注意：删除的只是影像数据集和影像文件在集合中的信息，与子数据集关联的同一数据源下的影像数据集和存放在磁盘上的影像文件不
#         会被删除。
#
#         :return: 移除是否成功
#         :rtype: bool
...
#     @property
#     def band_count(self):
#         """int: 影像数据集集合的波段个数。影像数据集集合中所有子数据集的具有相同的波段数"""
#         if self._jobject is None:
#             raise ObjectDisposedError(type(self).__name__)
#         self.open()
#         return self._jobject.getBandCount()
...
class DatasetMosaic(Dataset):
    ...
    def add_files(self, directory_paths, extension=None, clip_file_extension=None):
        ...
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        if clip_file_extension is not None and not isinstance(clip_file_extension, str):
        ...
        try:
            ...
            elif isinstance(directory_paths, list):
...
def combine_band(red_dataset, green_dataset, blue_dataset, out_data=None, out_dataset_name=None):
    ...
    try:
        ...
        # CombineBand 有缺陷，创建结果数据集后没有刷新 Java 组件的 Datasets
        result = get_jvm().com.supermap.data.Toolkit.CombineBand(_out_dataset_name,
                                                                 out_datasource._jobject,
                                                                 _red_data._jobject,
                                                                 _green_data._jobject,
                                                                 _blue_data._jobject)
        if result:
...
class Recordset(JVMBase):
    ...
    def set_value(self, item, value):
        ...
        try:
            ...
            if value is None:
                ...
                elif field_type == FieldType.WTEXT or field_type == FieldType.TEXT:
                    result = self._jobject.setString(index, str(value))
                elif field_type == FieldType.BYTE:
                ...
                elif field_type == FieldType.SINGLE:
                ...
                elif field_type == FieldType.JSONB:
                    result = self._jobject.setJsonB(index, str(value))
                elif field_type == FieldType.LONGBINARY:
    ...
    def from_json(value):
        ...
        if d1['type'] == 'pre':
            ...
            elif _method == 'query_with_ids':
                return dataset.query_with_ids(ids=params['ids'], id_field_name=params['id_field_name'],
                                              cursor_type=params['cursor_type'])

            elif _method == 'query_with_bounds':
                return dataset.query_with_bounds(bounds=Rectangle.from_json(params['bounds']),
                                                 attr_filter=params['attr_filter'],
                                                 cursor_type=params['cursor_type'])
            else:
