...
__all__ = ['GeometriesRelation', 'aggregate_points_geo', 'can_contain', 'has_area_intersection', 'has_cross',
           ...
           'orthogonal_polygon_fitting', 'zoom_geometry', 'divide_polygon']
...
class GeometriesRelation(JVMBase):
    ...
    def __init__(self, tolerance=1.0e-10, gridding_level='NONE'):
        """
    ...
    def set_gridding(self, gridding_level):
        ...
        if gridding_level is not None:
            gridding_level = GriddingLevel._make(gridding_level, 'NONE')
            self._jobject.setGridding(oj(gridding_level))
        return self
    ...
    def insert(self, data, value):
        ...
        if isinstance(data, (Point2D, Rectangle, Geometry)):
        ...
        elif isinstance(data, (Recordset, DatasetVector)):
            if isinstance(data, DatasetVector):
                ...
                get_jvm().com.supermap.jsuperpy.Utils.GeometriesRelation_InsertRecordset(self._jobject, rd._jobject,
                                                                                         field_name)
                rd.close()
                del rd
            else:
                if data.index_of_field(value) != -1 and data.get_field_info(value).type in (
                    ...
                    field_name = None
                get_jvm().com.supermap.jsuperpy.Utils.GeometriesRelation_InsertRecordset(self._jobject, data._jobject,
                                                                                         field_name)
            return True
        else:
            log_warning(
                'Failed to insert, geometry required Point2D, Rectangle and Geometry, but now is ' + str(type(data)))
            return False
    ...
    def intersect_extents(self, rc):
        ...
        if isinstance(rc, Rectangle):
            java_result = self._jobject.intersectExtents(oj(rc))
    ...
    def is_match(self, data, src_value, mode):
        ...
        if isinstance(data, (Point2D, Rectangle, Geometry)):
            if isinstance(data, Rectangle):
                data = data.to_region()
            return self._jobject.isMatch(data._jobject, int(src_value), oj(spatial_mode))
        else:
    ...
    def matches(self, data, mode, excludes=None):
        ...
        if isinstance(data, Rectangle):
            data = data.to_region()
        if excludes is not None:
            java_result = self._jobject.matches(data._jobject, oj(spatial_mode), excludes)
        else:
            java_result = self._jobject.matches(data._jobject, oj(spatial_mode))
        if java_result is not None:
            return list(java_result)
        else:
            return None
...
def aggregate_points_geo(points, min_pile_point_count, distance, unit='Meter', prj=None, as_region=False):
    ...
    if as_region:
        regions = get_jvm().com.supermap.data.Geometrist.aggregatePointsToRegions(to_java_point2ds(points), java_prj,
                                                                                  float(distance),
                                                                                  oj(unit),
        ...
        results = get_jvm().com.supermap.data.Geometrist.aggregatePoints(to_java_point2ds(points), java_prj,
                                                                         float(distance),
                                                                         oj(unit),
                                                                         int(min_pile_point_count))
        return list(results)
...
def can_contain(geo_search, geo_target):
    ...
    elif isinstance(geo_search, Rectangle):
        geo_search = geo_search.to_region()
    if isinstance(geo_target, Point2D):
        geo_target = GeoPoint(geo_target)
    elif isinstance(geo_target, Rectangle):
        geo_target = geo_target.to_region()
    return get_jvm().com.supermap.data.Geometrist.canContain(oj(geo_search), oj(geo_target))


def has_intersection(geo_search, geo_target, tolerance=None):
    """
    ...
    * 该关系适合的几何对象类型：
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return get_jvm().com.supermap.data.Geometrist.hasIntersection(oj(geo_search), oj(geo_target),
                                                                  tolerance)
...
def has_area_intersection(geo_search, geo_target, tolerance=None):
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return get_jvm().com.supermap.data.Geometrist.hasAreaIntersection(oj(geo_search), oj(geo_target),
                                                                      tolerance)
...
def has_cross(geo_search, geo_target):
    ...
    * 该关系适合的几何对象类型：
    ...
    if isinstance(geo_target, Rectangle):
        geo_target = geo_target.to_region()
    return get_jvm().com.supermap.data.Geometrist.hasCross(oj(geo_search), oj(geo_target))


def has_overlap(geo_search, geo_target, tolerance=None):
    """
    ...
    * 该关系适合的几何对象类型：
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return get_jvm().com.supermap.data.Geometrist.hasOverlap(oj(geo_search), oj(geo_target), tolerance)


def has_touch(geo_search, geo_target, tolerance=None):
    """
    ...
    * 该关系适合的几何对象类型：
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return get_jvm().com.supermap.data.Geometrist.hasTouch(oj(geo_search), oj(geo_target), tolerance)
...
def has_common_point(geo_search, geo_target):
    ...
    if isinstance(geo_target, Point2D):
        ...
        geo_target = geo_target.to_region()
    return get_jvm().com.supermap.data.Geometrist.hasCommonPoint(oj(geo_search), oj(geo_target))
...
def has_common_line(geo_search, geo_target):
    ...
    if isinstance(geo_target, Rectangle):
        geo_target = geo_target.to_region()
    return get_jvm().com.supermap.data.Geometrist.hasCommonLine(oj(geo_search), oj(geo_target))
...
def has_hollow(geometry):
    ...
    :rtype: bool
    """
    return get_jvm().com.supermap.data.Geometrist.hasHollow(oj(geometry))
...
def is_disjointed(geo_search, geo_target):
    ...
    * 该关系适合的几何对象类型：
    ...
    if isinstance(geo_target, Point2D):
        ...
        geo_target = geo_target.to_region()
    return get_jvm().com.supermap.data.Geometrist.isDisjointed(oj(geo_search), oj(geo_target))
...
def is_identical(geo_search, geo_target, tolerance=None):
    ...
    * 该关系适合的几何对象类型：
    ...
    if isinstance(geo_target, Point2D):
        ...
        geo_target = geo_target.to_region()
    if tolerance is not None:
        return get_jvm().com.supermap.data.Geometrist.isIdentical(oj(geo_search), oj(geo_target),
                                                                  float(tolerance))
    else:
        return get_jvm().com.supermap.data.Geometrist.isIdentical(oj(geo_search), oj(geo_target))
...
def is_within(geo_search, geo_target, tolerance=None):
    ...
    *  该关系适合的几何对象类型：
    ...
    if isinstance(geo_target, Point2D):
        ...
        geo_target = geo_target.to_region()
    if tolerance is not None:
        return get_jvm().com.supermap.data.Geometrist.isWithin(oj(geo_search), oj(geo_target),
                                                               float(tolerance))
    else:
        return get_jvm().com.supermap.data.Geometrist.isWithin(oj(geo_search), oj(geo_target))
...
def is_left(point, start_point, end_point):
    ...
    return get_jvm().com.supermap.data.Geometrist.isLeft(oj(point), oj(start_point), oj(end_point))
...
def is_right(point, start_point, end_point):
    ...
    start_point = Point2D.make(start_point)
    end_point = Point2D.make(end_point)
    return get_jvm().com.supermap.data.Geometrist.isRight(oj(point), oj(start_point), oj(end_point))
...
def is_on_same_side(point1, point2, start_point, end_point):
    ...
    start_point = Point2D.make(start_point)
    end_point = Point2D.make(end_point)
    return get_jvm().com.supermap.data.Geometrist.isOnSameSide(oj(point1), oj(point2), oj(start_point),
                                                               oj(end_point))
...
def is_parallel(start_point1, end_point1, start_point2, end_point2):
    ...
    return get_jvm().com.supermap.data.Geometrist.isParallel(oj(start_point1), oj(end_point1),
                                                             oj(start_point2), oj(end_point2))
...
def is_perpendicular(start_point1, end_point1, start_point2, end_point2):
    ...
    start_point2 = Point2D.make(start_point2)
    end_point2 = Point2D.make(end_point2)
    return get_jvm().com.supermap.data.Geometrist.isPerpendicular(oj(start_point1), oj(end_point1),
                                                                  oj(start_point2), oj(end_point2))
...
def is_point_on_line(point, start_point, end_point, is_extended=True):
    ...
    start_point = Point2D.make(start_point)
    end_point = Point2D.make(end_point)
    return get_jvm().com.supermap.data.Geometrist.isPointOnLine(oj(point), oj(start_point),
                                                                oj(end_point), bool(is_extended))
...
def is_project_on_line_segment(point, start_point, end_point):
    ...
    start_point = Point2D.make(start_point)
    end_point = Point2D.make(end_point)
    return get_jvm().com.supermap.data.Geometrist.isProjectOnLineSegment(oj(point), oj(start_point),
                                                                         oj(end_point))
...
def nearest_point_to_vertex(vertex, geometry):
    ...
    elif isinstance(geometry, GeoRegion):
        geometry = geometry.convert_to_line()
    if not isinstance(geometry, GeoLine):
        raise ValueError('geometry required Rectangle, GeoRegion or GeoLine, but now is ' + str(type(geometry)))
    return get_jvm().com.supermap.data.Geometrist.nearestPointToVertex(oj(vertex), oj(geometry))


def erase(geometry, erase_geometry, tolerance=None):
    """
    ...
    * 该操作适合的几何对象类型：
    ...
    :param float tolerance: 节点容限。只被操作对象为面对象时容限才有效，即面擦除线和点时，容限无效。
    :return: 擦除操作后的几何对象。
    :rtype: Geometry
    """
    ...
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.erase(oj(geometry), oj(erase_geometry), tolerance))


def clip(geometry, clip_geometry, tolerance=None):
    """
    ...
    * 该操作适合的几何对象类型：
    ...
    :param float tolerance: 节点容限。只被操作对象为面对象时容限才有效，即面裁剪线时，容限无效。
    :return: 裁剪结果对象
    :rtype: Geometry
    """
    if isinstance(geometry, Rectangle):
    ...
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.clip(oj(geometry), oj(clip_geometry), tolerance))


def identity(geometry, identity_geometry, tolerance=None):
    """
    ...
    * 该操作适合的几何对象类型：
    ...
    :param float tolerance: 节点容限。只有被操作几何对象为面时，容限才有效。
    :return: 同一操作后的几何对象
    :rtype: Geometry
    """
    if isinstance(geometry, Rectangle):
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.identity(oj(geometry), oj(identity_geometry), tolerance))
...
def intersect(geometry1, geometry2, tolerance=None):
    ...
    :param float tolerance: 节点容限，仅线线相交或面面相交时容限才有效。
    :return: 求交操作后的几何对象。
    :rtype: Geometry
    """
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.intersect(oj(geometry1), oj(geometry2), float(tolerance)))
...
def intersect_line(start_point1, end_point1, start_point2, end_point2, is_extended):
    ...
    start_point2 = Point2D.make(start_point2)
    end_point2 = Point2D.make(end_point2)
    return Point2D._from_java_object(
        get_jvm().com.supermap.data.Geometrist.intersectLine(oj(start_point1), oj(end_point1),
                                                             oj(start_point2), oj(end_point2),
                                                             bool(is_extended)))
...
def union(geometry1, geometry2, tolerance=None):
    """
    ...
    * 该操作适合的几何对象类型：
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.union(oj(geometry1), oj(geometry2), tolerance))


def update(geometry, update_geometry, tolerance=None):
    """
    ...
    if tolerance is not None:
        ...
        tolerance = 1.0e-10
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.update(oj(geometry), oj(update_geometry), tolerance))


def xor(geometry1, geometry2, tolerance=None):
    """
...
def compute_concave_hull(points, angle=45.0):
    ...
    """
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.computeConcaveHull(to_java_point2ds(points), float(angle)))
...
def compute_convex_hull(points):
    ...
    if isinstance(points, Geometry):
        return Geometry._from_java_object(
            get_jvm().com.supermap.data.Geometrist.computeConvexHull(oj(points)))
    else:
        return Geometry._from_java_object(
            get_jvm().com.supermap.data.Geometrist.computeConvexHull(to_java_point2ds(points)))


# def compute_fillet(start_point1, end_point1, start_point2, end_point2, radius):
#     pass
...
def compute_geodesic_area(geometry, prj):
    ...
    * 使用该方法计算经纬度面积，在通过 prj 参数指定投影坐标系类型对象（PrjCoordSys）时，必须通过该对象的 set_type 方法设置投影坐
    ...
    if prj is not None:
        ...
        java_prj = None
    return get_jvm().com.supermap.data.Geometrist.computeGeodesicArea(oj(geometry), java_prj)
...
def compute_geodesic_distance(points, major_axis, flatten):
    ...
    :rtype: float
    """
    return get_jvm().com.supermap.data.Geometrist.computeGeodesicDistance(to_java_point2ds(points), float(major_axis),
                                                                          float(flatten))
...
def compute_geodesic_line(start_point, end_point, prj, segment=18000):
    ...
    prj = PrjCoordSys.make(prj)
    if prj is not None:
        java_parameter.setPrjCoordSys(oj(prj))
    ...
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.computeGeodesicLine(oj(start_point), oj(end_point),
                                                                   java_parameter))
...
def compute_geodesic_line2(start_point, angle, distance, prj, segment=18000):
    ...
    java_parameter.setLineType(get_jvm().com.supermap.data.GeodesicLineType.GEODESIC)
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.computeGeodesicLine(oj(start_point), float(angle), float(distance),
                                                                   java_parameter))
...
def compute_parallel(geo_line, distance):
    ...
    """
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.computeParallel(oj(geo_line), float(distance)))
...
def compute_parallel2(point, start_point, end_point):
    ...
    end_point = Point2D.make(end_point)
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.computeParallel(oj(point), oj(start_point),
                                                               oj(end_point)))
...
def compute_perpendicular(point, start_point, end_point):
    ...
    end_point = Point2D.make(end_point)
    return Geometry._from_java_object(
        get_jvm().com.supermap.data.Geometrist.computePerpendicular(oj(point), oj(start_point), oj(end_point)))
...
def compute_perpendicular_position(point, start_point, end_point):
    ...
    end_point = Point2D.make(end_point)
    return Point2D._from_java_object(
        get_jvm().com.supermap.data.Geometrist.computePerpendicularPosition(oj(point), oj(start_point),
                                                                            oj(end_point)))
...
def compute_distance(geometry1, geometry2):
    ...
    if isinstance(geometry1, Point2D):
        p1 = geometry1
    elif isinstance(geometry1, GeoPoint):
        p1 = geometry1.point
    else:
        p1 = None
    if isinstance(geometry2, Point2D):
        p2 = geometry2
    elif isinstance(geometry2, GeoPoint):
        p2 = geometry2.point
    else:
        p2 = None
    ...
    elif isinstance(geometry1, Rectangle):
        geometry1 = geometry1.to_region()
    if isinstance(geometry2, Point2D):
    ...
    return get_jvm().com.supermap.data.Geometrist.distance(oj(geometry1), oj(geometry2))
...
def resample(geometry, distance, resample_type='RTBEND'):
    ...
    """
    resample_type = make_enum(VectorResampleType, resample_type, 'RTBEND')
    java_resample_fun = get_jvm().com.supermap.data.Geometrist.resample
    if isinstance(geometry, Geometry):
        return Geometry._from_java_object(
            java_resample_fun(oj(geometry), oj(resample_type), float(distance)))
    else:
...
def smooth(points, smoothness):
    ...
    if isinstance(points, GeoLine):
        ...
        if result.get_part_count() > 0:
            ...
            del result
            return None
    elif isinstance(points, (GeoRegion, Rectangle)):
...
def compute_default_tolerance(prj):
    ...
    return get_jvm().com.supermap.data.Geometrist.getTolerance(oj(prj))


def split_line(source_line, split_geometry, tolerance=1.0e-10):
    """
    ...
    * 面对象分割线对象。面对象分割线对象与线分割线类似，会在分割面和操作线的所有交点处将操作线打断，然后分别将位于奇数和偶数位置的线合并，产生两个线对象。
    ...
    elif isinstance(split_geometry, Rectangle):
        split_geometry = split_geometry.to_region()
    results = get_jvm().com.supermap.data.Geometrist.splitLine(oj(source_line), oj(split_geometry),
                                                               float(tolerance))
...
def split_region(source_region, split_geometry):
    ...
    split_result = get_jvm().com.supermap.data.Geometrist.splitRegion(oj(source_region), oj(split_geometry),
                                                                      oj(result_geo1), oj(result_geo2))
    if split_result:
        return result_geo1, result_geo2
    else:
        return None
...
def georegion_to_center_line(source_region, pnt_from=None, pnt_to=None):
    ...
    if pnt_from is not None and pnt_to is not None:
        if isinstance(pnt_from, GeoPoint):
            pnt_from = pnt_from.point
        if isinstance(pnt_to, GeoPoint):
            pnt_to = pnt_to.point
        return Geometry._from_java_object(
            get_jvm().com.supermap.analyst.spatialanalyst.Generalization.regionToCenterLine(oj(source_region),
                                                                                            oj(pnt_from),
                                                                                            oj(pnt_to)))
    else:
        return Geometry._from_java_object(
...
def orthogonal_polygon_fitting(geometry, width_threshold, height_threshold):
    ...
    result_geo = get_jvm().com.supermap.data.Geometrist.orthogonalPolygonFitting(oj(geometry),
...
def zoom_geometry(geometry, center_point, scale_x, scale_y):
    """
    ...
    :return: 成功返回缩放后的几何对象，失败返回None
    :rtype: Geometry
    """
    if not isinstance(geometry, (GeoRegion, GeoLine, GeoPoint)):
        raise ValueError('geometry required (GeoRegion, GeoLine, GeoPoint), but now is ' + str(type(geometry)))
    center_point = Point2D.make(center_point)
    result_geo = get_jvm().com.supermap.data.Geometrist.zoom(oj(geometry), oj(center_point), float(scale_x),
                                                             float(scale_y))
    if result_geo is not None:
        return Geometry._from_java_object(result_geo)
    return None
...
def divide_polygon(polygon, divide_type='PART', orientation='NORTH', angle=90.0, divide_parts=3,
                   divide_area=0.0, divide_area_unit='SQUAREMETER', remainder_area=None, prj=None):
    """
    ...
    if isinstance(polygon, Rectangle):
        polygon = polygon.to_region()

    if not isinstance(polygon, GeoRegion):
        raise ValueError('polygon required GeoRegion, but now is ' + str(type(polygon)))

    parameter = get_jvm().com.supermap.data.DividePolygonParameter()
    divide_type = DividePolygonType._make(divide_type, 'PART')

    if divide_type is DividePolygonType.PART:
        divide_parts = int(divide_parts)
        if divide_parts < 2:
            raise ValueError('divide parts must be greater than 1 if divide_type is PART')
        parameter.setDivideParts(int(divide_parts))
    else:
        divide_area_unit = AreaUnit._make(divide_area_unit, 'SQUAREMETER')
        divide_area = float(divide_area)
        if divide_area <= 0:
            raise ValueError('divide area must be greater than 0')
        if divide_parts < 1:
            raise ValueError('divide parts must be greater than 0 if divide_type is AREA')
        parameter.setDivideArea(float(divide_area))
        parameter.setUnitArea(oj(divide_area_unit))
        parameter.setDivideParts(int(divide_parts))

    parameter.setDivideType(oj(divide_type))
    parameter.setOrientation(oj(DividePolygonOrientation._make(orientation, 'NORTH')))
    parameter.setAngle(float(angle))

    if remainder_area:
        remainder_area = float(remainder_area)
    else:
        remainder_area = 0
    if remainder_area > 0:
        parameter.setMergeRemainder(True)
        parameter.setRemainderArea(float(remainder_area))
    else:
        parameter.setMergeRemainder(False)

    if not prj:
        prj = PrjCoordSys.make(prj)
    result_geos = get_jvm().com.supermap.data.Geometrist.dividePolygon(oj(polygon), parameter, oj(prj))

    if result_geos:
        results = []
        for geo in java_array_to_list(result_geos):
            results.append(Geometry._from_java_object(geo))
        return results
    return None
