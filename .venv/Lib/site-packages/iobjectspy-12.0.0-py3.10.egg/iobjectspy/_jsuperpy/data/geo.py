...
from .._utils import (java_color_to_tuple,
                      to_java_color,
                      tuple_to_java_color,
                      color_to_tuple,
                      to_java_double_array,
                      split_input_list_from_str,
                      oj)
from ._jvm import JVMBase
from ._util import (field_default_value_to_java,
                    convert_value_to_python)
from .style import *
from .ex import ObjectDisposedError

__all__ = ['FieldInfo', 'Point2D', 'Point3D', 'Rectangle', 'Geometry', 'GeoPoint', 'GeoPoint3D', 'GeoLine',
           'GeoRegion',
           'TextPart', 'GeoText', 'TextStyle', 'GeoStyle3D', 'Feature', 'Geometry3D', 'GeoBox', 'GeoModel3D',
           'GeoCylinder', 'GeoCircle3D', 'GeoRegion3D', 'GeoLine3D', 'Plane', 'Matrix', 'PointM', 'GeoLineM',
           'GeoConstructiveSolid', 'CSGNode', 'CSGEntity', 'CSGBooleanNode', 'CSGSimpleNode', 'Box', 'Sphere',
            'Cylinder', 'Cone', 'Ellipsoid', 'Torus', 'TruncatedCone', 'Table3D', 'Wedge', 'Pyramid', 'EllipticRing',
           'RectangularRing', 'SlopedCylinder', 'BendingCylinder']
...
class FieldInfo:
    ...
    def __init__(self, name=None, field_type=None, max_length=None, default_value=None, caption=None,
                 is_required=False, is_zero_length_allowed=True):
        """
        构造字段信息对象
        ...
        self.set_name(name). \
            ...
            set_caption(caption). \
            set_required(is_required). \
            set_zero_length_allowed(is_zero_length_allowed)
    ...
    def clone(self):
        ...
        :rtype: FieldInfo
        """
        return FieldInfo(self.name, self.type, self.max_length, self.default_value, self.caption,
                         self.is_required, self.is_zero_length_allowed)
    ...
    def set_caption(self, value):
        ...
        if value is not None:
            ...
            if self._java_object:
                self._java_object.setCaption(str(value))
        return self
    ...
    def __getstate__(self):
        return self.name, self.caption, self.type.name, self.max_length, \
    ...
    def to_dict(self):
        ...
        if self.max_length is not None and self.type in [FieldType.TEXT, FieldType.WTEXT, FieldType.CHAR]:
...
class Point2D:
    ...
    def equal(self, other, tolerance=0.0):
        ...
        if not is_equal(self.x, other.x, d) or not is_equal(self.y, other.y, d):
            return False
        else:
...
class Rectangle:
    ...
    def _jobject(self):
        ...
        Py4J 映射的 Java 对象
        """
        return get_jvm().com.supermap.data.Rectangle2D(self.left, self.bottom, self.right,
                                                       self.top)
    ...
    def __eq__(self, other):
        ...
        import math
        if (math.isclose(self.left, other.left) and math.isclose(self.right, other.right)
                and math.isclose(self.top, other.top) and math.isclose(self.bottom, other.bottom)):
            return True
        else:
            return False
    ...
    def _set_left_right(self, left, right):
        if left is not None and right is not None:
            self._min_x = min(float(left), float(right))
            self._max_x = max(float(left), float(right))
        else:
            if left is not None:
                self._min_x = float(left)
            else:
                self._min_x = None
            if right is not None:
                self._max_x = float(right)
            else:
                self._max_x = None
        return self
    ...
    def _set_top_bottom(self, top, bottom):
        if top is not None and bottom is not None:
            self._min_y = min(float(top), float(bottom))
            self._max_y = max(float(top), float(bottom))
        else:
            if bottom is not None:
                self._min_y = float(bottom)
            else:
                self._min_y = None
            if top is not None:
                self._max_y = float(top)
            else:
                self._max_y = None
    ...
    def set_left(self, value):
        ...
        :rtype: Rectangle
        """
        self._min_x = float(value) if value is not None else None
        if self.right is not None and self.left is not None:
            if self.right < self.left:
                self._max_x, self._min_x = self._min_x, self._max_x
        return self
    ...
    def set_right(self, value):
        ...
        """
        self._max_x = float(value) if value is not None else None
        if self.right is not None and self.left is not None:
            if self.right < self.left:
                self._max_x, self._min_x = self._min_x, self._max_x
        return self
    ...
    def set_top(self, value):
        ...
        """
        self._max_y = float(value) if value is not None else None
        if self.top is not None and self.bottom is not None:
            if self.top < self.bottom:
                self._max_y, self._min_y = self._min_y, self._max_y
        return self
    ...
    def set_bottom(self, value):
        ...
        :rtype: Rectangle
        """
        self._min_y = float(value) if value is not None else None
        if self.top is not None and self.bottom is not None:
            if self.top < self.bottom:
                self._max_y, self._min_y = self._min_y, self._max_y
        return self
    ...
    def contains(self, item):
        ...
        if isinstance(item, Rectangle):
            points = item.points
            if self.contains(points[0]) and self.contains(points[1]) and self.contains(points[2]) and self.contains(
                    points[3]):
                return True
            else:
                return False
    ...
    def has_intersection(self, item):
        ...
        if isinstance(item, Point2D):
            ...
            if self.left <= item.left <= self.right and \
                    self.bottom <= item.top <= self.top:
                return True
            elif self.left <= item.left <= self.right and \
                    self.bottom <= item.bottom <= self.top:
                return True
            elif self.left <= item.right <= self.right and \
                    self.bottom <= item.top <= self.top:
                return True
            elif self.left <= item.right <= self.right and \
                    self.bottom <= item.bottom <= self.top:
            ...
            return get_jvm().com.supermap.data.Geometrist.has_intersection(self.to_region()._jobject,
                                                                           item._jobject)
    ...
    def is_empty(self):
        ...
        :rtype: bool
        """
        if self.left is None or self.right is None or self.top is None or self.bottom is None:
            return True
        if -1.7976931348623157e+308 in (self.left, self.right, self.top, self.bottom):
            return True
        return False
...
class Geometry(JVMBase):
    ...
    def _make_geo_instance(geo_type):
        ...
        if _geoType is GeometryType.GEOPOINT:
            geo = GeoPoint()
        elif _geoType is GeometryType.GEOLINE:
            geo = GeoLine()
        elif _geoType is GeometryType.GEOREGION:
            geo = GeoRegion()
        elif _geoType is GeometryType.GEOREGION3D:
            geo = GeoRegion3D()
        elif _geoType is GeometryType.GEOTEXT:
            geo = GeoText()
        elif _geoType is GeometryType.GEOPOINT3D:
            geo = GeoPoint3D()
        elif _geoType is GeometryType.GEOLINE3D:
            geo = GeoLine3D()
        elif _geoType is GeometryType.GEOBOX:
            geo = GeoBox()
        elif _geoType is GeometryType.GEOCIRCLE3D:
            geo = GeoCircle3D()
        elif _geoType is GeometryType.GEOCYLINDER:
            geo = GeoCylinder()
        elif _geoType is GeometryType.GEOMODEL3D:
            geo = GeoModel3D()
        elif _geoType is GeometryType.GEOLINEM:
            geo = GeoLineM()
        else:
            geo = Geometry()
        return geo
    ...
    def linear_extrude(self, height=0.0, twist=0.0, scaleX=1.0, scaleY=1.0, bLonLat=False):
        ...
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        if (self.type != GeometryType.GEOREGION and
                self.type != GeometryType.GEOREGION3D and
                self.type != GeometryType.GEOCIRCLE and
                self.type != GeometryType.GEOCIRCLE3D and
                self.type != GeometryType.GEORECTANGLE):
            raise TypeError("require type GeoRegion or GeoRegion3D")
        if height < 0:
            raise ValueError("height should greater than 0")
        geo = self._jvm.com.supermap.realspace.threeddesigner.ModelBuilder3D.linearExtrude(
            self._jobject, bLonLat, float(height), float(twist), float(scaleX), float(scaleY))
        if geo != None:
    ...
    def rotate_extrude(self, angle, slices):
        ...
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        if (self.type != GeometryType.GEOREGION and self.type != GeometryType.GEOLINE):
        ...
        geo = self._jvm.com.supermap.realspace.threeddesigner.ModelBuilder3D.rotateExtrude(
            self._jobject, rotateExtrudeP)
        if geo != None:
            geo = GeoModel3D._from_java_object(geo)
        return geo
    ...
    def _from_pb_bytes(bys):
        ...
        :rtype: Geometry
        """
        return Geometry._from_java_object(
            get_jvm().com.supermap.jsuperpy.serializer.SerializerUtil.deserializeGeometry(bys))
...
class GeoPoint3D(Geometry):
    ...
    def __init__(self, point=None):
        ...
        elif isinstance(point, GeoPoint3D):
            self._jobject.setX(point.get_x())
            self._jobject.setY(point.get_y())
            self._jobject.setZ(point.get_z())
        elif isinstance(point, (tuple, list)) and len(point) > 2:
            self._jobject.setX(float(point[0]))
            self._jobject.setY(float(point[1]))
            self._jobject.setZ(float(point[2]))
    ...
    def set_z(self, z):
        ...
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setZ(float(z) if z is not None else None)
        return self
...
class GeoLine(Geometry):
    ...
    def __init__(self, points=None):
        ...
        if points is not None:
            ...
            elif isinstance(points, Rectangle):
                java_point2ds = self._jvm.com.supermap.data.Point2Ds()
    ...
    def __getstate__(self):
        return self.get_parts(), self.id, self.bounds
...
class TextStyle(object):
    ...
    def __init__(self):
        ...
        # self._shadowOffsetX = None
        self._fontHeight = None
        self._italic = None
        # self._shadowOffsetY = None
    ...
    #
    # def set_shadow_offset_x(self, value):
    ...
    #
    # def set_shadow_offset_y(self, value):
    ...
    #
    # def set_shadow_color(self, value):
...
class TextPart(object):
    ...
    def __getstate__(self):
        return self.text, self.anchor_point, self.rotation
...
class GeoText(Geometry):
    ...
    def __getstate__(self):
        return self.get_parts(), self.text_style, self.id
    ...
    def add_part(self, text_part):
        ...
        if isinstance(text_part, TextPart):
            return self._jobject.addPart(oj(text_part))
        return -1
...
class Feature(object):
    ...
    def set_value(self, item, value):
        ...
        try:
            ...
            elif fieldInfo.type in (FieldType.CHAR, FieldType.TEXT, FieldType.WTEXT, FieldType.JSONB):
                self._values[item] = str(value)
            elif fieldInfo.type in (FieldType.BYTE, FieldType.INT16, FieldType.INT32, FieldType.INT64):
                self._values[item] = int(value)
            elif fieldInfo.type in (FieldType.SINGLE, FieldType.DOUBLE):
                self._values[item] = float(value)
            elif fieldInfo.type == FieldType.LONGBINARY:
                if isinstance(value, bytes) or isinstance(value, bytearray):
                    self._values[item] = value
                else:
                    self._values[item] = bytes(str(value), encoding='utf-8')
            elif fieldInfo.type == FieldType.DATETIME:
    ...
    def get_values(self, exclude_system=True, is_dict=False):
        ...
        for field in self.field_infos:
            i += 1
            if exclude_system and field.is_system_field():
                continue
            if is_dict:
    ...
    #         name = fields[i].name
    #         if not name.lower().startswith('sm'):
    #             _fields[i] = fields[i]
    #         else:
    #             _system_fields[i] = fields[i].clone().set_name('T' + name)
    #     return self.geometry, _fields, _system_fields, self._values, self.feature_id
    #
    # def __setstate__(self, state):
    #     if len(state) != 5:
    #         raise Exception('state length required 4 but ' + str(len(state)))
    #     fields = state[1]
    #     system_fields = state[2]
    #     all_fields = list(range(len(fields) + len(system_fields)))
    #     for index, field in fields.items():
    #         all_fields[index] = field
    #     for index, field in system_fields.items():
    #         all_fields[index] = field.clone().set_name(field.name[1:])
    #     self.__init__(state[0], all_fields, state[3], state[4])
    ...
    def to_json(self):
        ...
        if self.field_infos is not None and len(self.field_infos) > 0:
...
class GeoLine3D(Geometry3D):
    ...
    def __init__(self, points=None):
        ...
        if points is not None:
            ...
            elif isinstance(points, GeoRegion3D):
...
class GeoModel3D(Geometry3D):
        ...
        if isinstance(poin3dValues, (list)):
            if isinstance(faceIndices, (list)):
                ...
                if len(models) >= 2:
                    jModels = []
                    for pm in models:
                        jModels.append(pm._jobject)
                    result = self._jvm.com.supermap.realspace.threeddesigner.ModelTools.compose(jModels)
                else:
                    result = models[0]._jobject
    ...
    def translate(self, x=0, y=0, z=0):
        ...
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        geoArray = [self._jobject]
        result = self._jvm.com.supermap.realspace.threeddesigner.ModelTools.translate(
            geoArray, float(x), float(y), float(z))
        if len(result) > 0:
    ...
    def set_color(self, value):
        ...
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jvm.com.supermap.jsuperpy.threeddesigner.ModelBuilder3DTools.setModelColor(
            self._jobject, tuple_to_java_color(value))
    ...
    def IsLonLat(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.isLonLat()

    def set_IsLonLat(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setIsLonLat(value)

    # 单独封装BoolOperation3D类
    # def union(self, that):
    #     """
    #     三维模型对象进行布尔运算-求并运算
    #     :param that: 求并的GeoModel3D
    #     :return:
    #     """
    #     if self._jobject is None:
    #         raise ObjectDisposedError(type(self).__name__)
    #     if not isinstance(self, GeoModel3D):
    #         raise TypeError("require type GeoModel3D")
    #     if that._jobject is None:
    #         raise ObjectDisposedError(type(that).__name__)
    #     geo = get_jvm().com.supermap.realspace.threeddesigner.BooleanOperator3D.union(
    #         self._jobject, that._jobject)
    #     if geo != None:
    #         geo = GeoModel3D._from_java_object(geo)
    #     return geo
    #
    ...
    #     """
    #     两个指定三维几何对象的差运算
    #     :param that: 求差的GeoModel3D
    #     :return: 返回相减后的GeoModel3D
    #     """
    #     if self._jobject is None:
    #         raise ObjectDisposedError(type(self).__name__)
    #     if not isinstance(self, GeoModel3D):
    ...
    def Mirror(self, plane):
        ...
        if isinstance(plane, (Plane)):
            result = self._jvm.com.supermap.realspace.threeddesigner.ModelBuilder3D.Mirror(self._java_object,
                                                                                           plane._java_object)
            return result._java_object
        return None

    def mergeSkeleton(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)

        jModel = self._jvm.com.supermap.data.Model()
        bSuccessed = self._jvm.com.supermap.realspace.threeddesigner.ModelBuilder3D.mergeSkeleton(
            self._jobject.getModel(), jModel, False)
        if bSuccessed is True:
            self._jobject.setModel(jModel)
...
class GeoBox(Geometry3D):
    ...
    def __str__(self):
        return 'GeoBox: ' + 'length=' + self._len + \
...
class GeoCylinder(Geometry3D):
    ...
    def __str__(self):
        return 'GeoCylinder: ' + 'topRadius=' + self._topR + \
               ',bottomRadius=' + self._bottomR + ',height=' + self._height
...
class GeoConstructiveSolid(Geometry3D):
    """
    构造实体几何对象类
    """
    _nodes = []
    def __init__(self, csg_nodes=None):
        Geometry3D.__init__(self)

        if self._jobject is not None:
            if isinstance(csg_nodes, CSGNode):
                nodes = [csg_nodes._java_object]
                _nodes = nodes
                self._java_object.setCSGNodes(nodes)
            elif isinstance(csg_nodes, list):
                if isinstance(csg_nodes[0], CSGNode):
                    j_nodes = []
                    _nodes = csg_nodes
                    for node in _nodes:
                        j_nodes.append(node._java_object)
                    self._java_object.setCSGNodes(j_nodes)


    def _create_java_object(self):
        self._java_object = self._jvm.com.supermap.data.GeoConstructiveSolid()
        return self
    ...
    def set_IsLonLat(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._java_object.setIsLonLat(value)
    ...
    def set_csgNodes(self, value):
        ...
        for node in self._nodes:
            j_nodes.append(node._java_object)
        self._java_object.setCSGNodes(self._nodes)
...
class Plane(JVMBase):
    ...
    def __init__(self, points=None):
        if points is not None:
            if isinstance(points, (list, tuple)):
                if len(points) == 3:
                    p1 = Point3D.make(points[0])
                    p2 = Point3D.make(points[1])
                    p3 = Point3D.make(points[2])
                    self._java_object = get_jvm().com.supermap.data.Plane(p1._jobject, p2._jobject, p3._jobject)
            elif isinstance(points, PlaneType):
                self._java_object = get_jvm().com.supermap.data.Plane(points._jobject)
        else:
            self._create_java_object()
...
class Matrix(JVMBase):
    ...
    # 先注掉吧  有点没太明白
    # def toAnglesXYZ(self, point3d):
    #     """
    #     :根据矩阵获取旋转
    #     :param point3d:传出参数
    #     :return:返回为true时，获取成功；返回为False时，获取失败
    #     """
    #     anglesP=[point3d.x, point3d.y, point3d.z]
    #     if self._java_object.toAnglesXYZ(to_java_double_array(anglesP)):
    #         point3d.x = anglesP[0]
    #         point3d.Y = anglesP[1]
    #         point3d.Z = anglesP[2]
    #     else:
    ...
    @staticmethod
    def invert(matrix):
        mat = matrix._java_object.invert(matrix._java_object)
        return mat

    @staticmethod
    def identity():
        mat = get_jvm().com.supermap.data.Matrix()
        return Matrix._from_java_object(mat)


class CSGNode(JVMBase):
    """
    CSGNode基类
    """
    def __init__(self, other = None):
        JVMBase.__init__(self)
        if (other is not None) and isinstance(other, CSGNode):
            if other.type == CSGNodeType.CSG_SIMPLENODE:
                self._java_object = get_jvm().com.supermap.data.CSGSimpleNode(other._java_object)
            else:
                self._java_object = get_jvm().com.supermap.data.CSGBooleanNode(other._java_object)

    @property
    def matrix(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        matrix = self._java_object.getMatrix()
        return Matrix._from_java_object(matrix)

    def set_Matrix(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self.self._jobject.setMatrix(value._jobject)
    ...
    def type(self):
        ...
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return CSGNodeType._make(self._java_object.getType().name())

    def to_json(self):
        """输出为json"""
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.toJson()
    ...
    def from_json(str_json):
        ...
        try:
            csg_j_node = get_jvm().com.supermap.data.CSGNode.fromJson(str_json)
            if csg_j_node is None:
                return None
            return CSGNode._from_java_object(csg_j_node)
        except Exception as e:
            log_error(e)
            raise e
    ...
    def __add__(self, other):
        """
        :布尔并，other可以是CSGNode、CSGEntity、Point3D
        :当other是CSGNode时，构造新的CSGBoolNode
        :当other是CSGEntity时，构造新的CSGBoolNode
        :当other是Point3D时，平移当前node
        """
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        if other is None:
        ...
        if not (isinstance(other, Point3D) or isinstance(other, CSGEntity) or isinstance(other, CSGNode)):
            print('Just support Point3D,CSGEntity and CSGNode!')
            return None

        if isinstance(other, Point3D):
            j_result = self._java_object.translate(other._jobject)
        else:
            j_result = self._java_object.union(other._java_object)

        if isinstance(other, Point3D):
            self._java_object = j_result
            return self
        else:
            new_csg_node = CSGBooleanNode()
            new_csg_node._java_object = j_result
            return new_csg_node

    def __sub__(self, other):
        """
        :布尔差，other可以是CSGNode、CSGEntity
        :构造新的CSGBoolNode
        """
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        ...
        if not (isinstance(other, CSGEntity) or isinstance(other, CSGNode)):
            print('Just support CSGEntity and CSGNode!')
            return None

        j_result = self._java_object.difference(other._java_object)
    ...
    def __mul__(self, other):
        """
       :布尔差，other可以是CSGNode、CSGEntity、Point3D、Matrix
       :当other是CSGNode时，构造新的CSGBoolNode
       :当other是CSGEntity时，构造新的CSGBoolNode
       :当other是Point3D时，当前node平移
       :当other是Matrix时，矩阵变换当前Node
       """
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        if other is None:
        ...
        if not (isinstance(other, Point3D) or isinstance(other, CSGEntity) or isinstance(other,
                                                                                         CSGNode) or isinstance(
            other, Matrix)):
            print('Just support Point3D,CSGEntity CSGNode and Matrix!')
            return None

        if isinstance(other, Point3D):
            j_result = self._java_object.scale(other._jobject)
        elif isinstance(other, Matrix):
            j_result = self._java_object.multiply(other._java_object)
        else:
            j_result = self._java_object.intersect(other._java_object)

        if isinstance(other, Point3D) or isinstance(other, Matrix):
            self._java_object = j_result
            return self
        else:
            new_csg_node = CSGBooleanNode()
    ...
    def rotate(self, _rotate):
        """
        :旋转
        """
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        if not isinstance(_rotate, Point3D):
            return

        self._java_object.rotate(_rotate._jobject)
        return self
    ...
    @staticmethod
    def _make_csgnode_instance(csg_node_type):
        _csg_type = CSGNodeType._make(csg_node_type)
        if _csg_type is CSGNodeType.CSG_SIMPLENODE:
            csg_node = CSGSimpleNode()
        else:
            csg_node = CSGBooleanNode()
        return csg_node

    @staticmethod
    def _from_java_object(j_csg_node):
        if j_csg_node is None:
            return None
        csg_node = CSGNode._make_csgnode_instance(j_csg_node.getType().name())
        csg_node._java_object = j_csg_node
        return csg_node


class CSGEntity(JVMBase):

    def __init__(self, other=None):
        JVMBase.__init__(self)
        if (other is not None) and(isinstance(other, CSGEntity)):
            self._java_object = other.copy_j_object()
    ...
    def type(self):
        ...
        j_type = self._java_object.getType()
        return CSGEntityType._make(j_type.name())

    @property
    def area(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.getArea()

    @property
    def volume(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.getVolume()

    def __add__(self, other):
        """
        :布尔并，other可以是CSGNode、CSGEntity、Point3D
        :当other是CSGNode时，构造CSGBoolNode并返回
        :当other是CSGEntity时，构造CSGBoolNode并返回
        :当other是Point3D时，构造SimpleNode平移并返回
        """
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)

        if other is None:
            raise ObjectDisposedError(type(other).__name__)

        if not (isinstance(other, Point3D) or isinstance(other, CSGNode) or isinstance(other, CSGEntity)):
            print('Just support Point3D CSGEntity and CSGNode')
            return None

        if isinstance(other, Point3D):
            j_csg_node = self._java_object.translate(other._jobject)
            return CSGNode._from_java_object(j_csg_node)

        j_csg_node = self._java_object.union(other._java_object)
        return CSGNode._from_java_object(j_csg_node)
    ...
    def __sub__(self, other):
        ...
        if not (isinstance(other, CSGNode) or isinstance(other, CSGEntity)):
            print('Just support CSGEntity and CSGNode')

        j_csg_node = self._java_object.difference(other._java_object)
        return CSGNode._from_java_object(j_csg_node)

    def __mul__(self, other):
        """
        :布尔交，other可以是CSGNode、CSGEntity、Point3D、Matrix
        :当other是CSGNode时，构造CSGBoolNode并返回
        :当other是CSGEntity时，构造CSGBoolNode并返回
        :当other是Point3D时，构造SimpleNode缩放并返回
        :当other是Matrix时，构造SimpleNode变换并返回
        """
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        ...
        if not (isinstance(other, CSGNode) or isinstance(other, CSGEntity) or isinstance(other,
                                                                                         Point3D) or isinstance(
                other, Matrix)):
            print('Just support CSGEntity and CSGNode')
            return None

        if isinstance(other, Point3D):
            j_csg_node = self._java_object.scale(other._jobject)
            return CSGNode._from_java_object(j_csg_node)

        if isinstance(other, Matrix):
            j_csg_node = self._java_object.multiply(other._java_object)
            return CSGNode._from_java_object(j_csg_node)


        j_csg_node = self._java_object.intersect(other._java_object)
        return CSGNode._from_java_object(j_csg_node)
    ...
    def rotate(self, _rotate):
        ...
        j_csg_node = self._java_object.rotate(_rotate._java_object)
        return CSGNode._from_java_object(j_csg_node)

    @staticmethod
    def _make_entity_instance(entity_type):
        """
        根据java类型创建实例
        :param entity_type:
        :return:
        """
        _csg_type = CSGEntityType._make(entity_type)
        if _csg_type is CSGEntityType.CSG_Box:
            entity = Box()
        elif _csg_type is CSGEntityType.CSG_Sphere:
            entity = Sphere()
        elif _csg_type is CSGEntityType.CSG_Cylinder:
            entity = Cylinder()
        elif _csg_type is CSGEntityType.CSG_Cone:
            entity = Cone()
        elif _csg_type is CSGEntityType.CSG_Ellipsoid:
            entity = Ellipsoid()
        elif _csg_type is CSGEntityType.CSG_Torus:
            entity = Torus()
        elif _csg_type is CSGEntityType.CSG_TruncatedCone:
            entity = TruncatedCone()
        elif _csg_type is CSGEntityType.CSG_Table3D:
            entity = Table3D()
        elif _csg_type is CSGEntityType.CSG_Wedge:
            entity = Wedge()
        elif _csg_type is CSGEntityType.CSG_Pyramid:
            entity = Pyramid()
        elif _csg_type is CSGEntityType.CSG_EllipticRing:
            entity = EllipticRing()
        elif _csg_type is CSGEntityType.CSG_RectangularRing:
            entity = RectangularRing()
        elif _csg_type is CSGEntityType.CSG_SlopedCylinder:
            entity = SlopedCylinder()
        elif _csg_type is CSGEntityType.CSG_BendingCylinder:
            entity = BendingCylinder()

        return entity

    @staticmethod
    def _from_java_object(j_csg_entity):
        if j_csg_entity is None:
            return None
        entity = CSGEntity._make_entity_instance(j_csg_entity.getType().name())
        entity._java_object = j_csg_entity
        return entity

    def copy_j_object(self):
        """
        根据当前实体类型创建java对象
        :return:java实例对象
        """
        ...
        _csg_type = CSGEntityType._make(self._java_object.getType().name())
        if _csg_type is CSGEntityType.CSG_Box:
            _new_jobject = get_jvm().com.supermap.data.Box(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Sphere:
            _new_jobject = get_jvm().com.supermap.data.Sphere(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Cylinder:
            _new_jobject = get_jvm().com.supermap.data.Cylinder(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Cone:
            _new_jobject = get_jvm().com.supermap.data.Cone(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Ellipsoid:
            _new_jobject = get_jvm().com.supermap.data.Ellipsoid(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Torus:
            _new_jobject = get_jvm().com.supermap.data.Torus(self._java_object)
        elif _csg_type is CSGEntityType.CSG_TruncatedCone:
            _new_jobject = get_jvm().com.supermap.data.TruncatedCone(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Table3D:
            _new_jobject = get_jvm().com.supermap.data.Table3D(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Wedge:
            _new_jobject = get_jvm().com.supermap.data.Wedge(self._java_object)
        elif _csg_type is CSGEntityType.CSG_Pyramid:
            _new_jobject = get_jvm().com.supermap.data.Pyramid(self._java_object)
        elif _csg_type is CSGEntityType.CSG_EllipticRing:
            _new_jobject = get_jvm().com.supermap.data.EllipticRing(self._java_object)
        elif _csg_type is CSGEntityType.CSG_RectangularRing:
            _new_jobject = get_jvm().com.supermap.data.RectangularRing(self._java_object)
        elif _csg_type is CSGEntityType.CSG_SlopedCylinder:
            _new_jobject = get_jvm().com.supermap.data.SlopedCylinder(self._java_object)
        elif _csg_type is CSGEntityType.CSG_BendingCylinder:
            _new_jobject = get_jvm().com.supermap.data.BendingCylinder(self._java_object)
        else:
            _new_jobject = None
    ...
    def clone(self):
        ...
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return CSGEntity(self)


class CSGBooleanNode(CSGNode):

    def __init__(self, left=None, right=None, boolType = CSGBooleanType.BOOL_UNION):
        """
        :用左右两个节点构造,
        left,right可以同时为CSGNode、CSGEntity，也可一个是CSGNode、一个是CSGEntity
        """
        JVMBase.__init__(self)
        self._java_object = get_jvm().com.supermap.data.CSGBooleanNode()

        bo_type = get_jvm().com.supermap.data.BooleanOperatorType.INTERSECT
        if boolType == CSGBooleanType.BOOL_UNION:
            bo_type = get_jvm().com.supermap.data.BooleanOperatorType.UNION
        elif  boolType == CSGBooleanType.BOOL_DIFFERENCE:
            bo_type = get_jvm().com.supermap.data.BooleanOperatorType.DIFFERENCE
        # 左右节点需要拷贝
        self._java_object.setBoolType(bo_type)
        # left为CSGNode
        if isinstance(left, CSGNode):
            _left = left.clone()
            self.set_left_node(_left)
        # left为CSGEntity
        elif isinstance(left, CSGEntity):
            simple = CSGSimpleNode(left)# 构造simpleNode会拷贝
            self.set_left_node(simple)

        self._rightNode = None
        # right为CSGNode
        if isinstance(right, CSGNode):
            _right = right.clone()
            self.set_right_node(_right)
        # right为CSGEntity
        elif isinstance(right, CSGEntity):
            simple = CSGSimpleNode(right)
            self.set_right_node(simple)

    def type(self):
        return CSGNodeType.CSG_BOOLEANNODE

    @property
    def left_node(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return CSGNode._from_java_object(self._java_object.getLeftNode())

    @property
    def right_node(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return CSGNode._from_java_object(self._java_object.getRightNode())

    @property
    def bool_type(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return CSGNodeType._make(self._java_object.getBoolType())

    def set_left_node(self, value):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        if isinstance(value, CSGNode):
            self._java_object.setLeftNode(value._java_object)
    ...
    def set_bool_type(self, value):
        if isinstance(value, CSGBooleanType):
            bo_type = get_jvm().com.supermap.data.BooleanOperatorType.INTERSECT
            if value == CSGBooleanType.BOOL_UNION:
                bo_type = get_jvm().com.supermap.data.BooleanOperatorType.UNION
            elif value == CSGBooleanType.BOOL_DIFFERENCE:
                bo_type = get_jvm().com.supermap.data.BooleanOperatorType.DIFFERENCE
            # 左右节点需要拷贝
            self._java_object.setBoolType(bo_type)
    ...
    def clone(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        new_csg_node = CSGBooleanNode()
        new_csg_node._java_object = get_jvm().com.supermap.data.CSGBooleanNode(self._jobject)
        return new_csg_node


class CSGSimpleNode(CSGNode):
    ...
    def __init__(self, data=None):
        ...
        """
        JVMBase.__init__(self)
        self._java_object = get_jvm().com.supermap.data.CSGSimpleNode()

        if isinstance(data, CSGEntity):
            _data = data.clone()
            self.set_entity(_data)

        elif isinstance(data, CSGSimpleNode):
            self._java_object = get_jvm().com.supermap.data.CSGSimpleNode(data._java_object)

    def type(self):
        return CSGNodeType.CSG_SIMPLENODE

    def get_entity(self):
        return CSGEntity._from_java_object(self._java_object)

    def set_entity(self, entity):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        if isinstance(entity, CSGEntity):
            self._java_object.setEntity(entity._java_object)

    def clone(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        new_csg_node = CSGSimpleNode()
        new_csg_node._java_object = get_jvm().com.supermap.data.CSGSimpleNode(self._java_object)
        return new_csg_node
...
class Box(CSGEntity):
    ...
    def __init__(self, _length=10, _width=10, _height=10):
        """
        :length width ,height 长宽高
        """
        self._java_object = get_jvm().com.supermap.data.Box(float(_length), float(_width), float(_height))
    ...
    def set_length(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setLength(float(value))
...
class Sphere(CSGEntity):
    """
    球体
    """
    def __init__(self, dradius=10.0):
        """
        :半径
        """
        self._java_object = get_jvm().com.supermap.data.Sphere(float(dradius))
...
class Cylinder(CSGEntity):
    """
    圆柱体(上下半径可以不一致)
    """
    def __init__(self, top_radius=10, bottom_radius = 10, height = 50):
        """
        :上下底面半径可不同
        """
        self._java_object = get_jvm().com.supermap.data.Cylinder(float(top_radius), float(bottom_radius), float(height))
    ...
    def set_bottom_radius(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setBottomRadius(float(value))
...
class Cone(CSGEntity):
    """
    圆锥体
    """

    def __init__(self, radius=10, height=50):
        """
        :param radius:底面半径
        :param height: 高
        """
        self._java_object = get_jvm().com.supermap.data.Cone(float(radius), float(height))
...
class Ellipsoid(CSGEntity):
    """
    椭球体
    """
    def __init__(self, semiAxis_x=10, semiAxis_y=10, semiAxis_z=10):
        """
        :param semiAxis_x:x半轴
        :param semiAxis_y:y半轴
        :param semiAxis_z:z半轴
        """
        self._java_object = get_jvm().com.supermap.data.Ellipsoid(float(semiAxis_x), float(semiAxis_y), float(semiAxis_z))

    @property
    def semiAxis_X(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getSemiAxisX()

    def set_semiAxis_X(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setSemiAxisX(float(value))

    @property
    def semiAxis_Y(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getSemiAxisY()

    def set_semiAxis_Y(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setSemiAxisY(float(value))

    @property
    def semiAxis_Z(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getSemiAxisZ()

    def set_semiAxis_Z(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setSemiAxisZ(float(value))


class Torus(CSGEntity):
    """
    圆环体
    """
    def __init__(self, _ring_radius=10, _pipe_radius=1, _sweep_angle=360, _start_angle=0):
        """
        :param ring_radius:环半径
        :param pipe_radius:管半径
        :param sweep_angle:扫略角度
        :param start_angle:起始角度
        """
        self._java_object = get_jvm().com.supermap.data.Torus(float(_ring_radius), float(_pipe_radius),
                                                              float(_sweep_angle), float(_start_angle))

    @property
    def ring_radius(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getRingRadius()

    def set_ring_radius(self,value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setRingRadius(float(value))

    @property
    def pipe_radius(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getPipeRadius()

    def set_pipe_radius(self,value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setPipeRadius(float(value))

    @property
    def sweep_angle(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getSweepAngle()

    def set_sweep_angle(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setSweepAngle(float(value))
...
class TruncatedCone(CSGEntity):
    """
    圆台体
    """
    def __init__(self, _top_radius=10, _bottom_radius=20, _height=20, _top_offset=Point2D(0.0, 0.0)):
        """
        :param _top_radius:顶部半径
        :param _bottom_radius:底部半径
        :param _height:高度
        :param _top_offset:顶面圆心偏移(Point2D)
        """
        self._java_object = get_jvm().com.supermap.data.TruncatedCone(float(_top_radius), float(_bottom_radius),
                                                                  float(_height),
                                                                  _top_offset._jobject)
    ...
    def top_offset(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return Point2D._from_java_object(self._jobject.getTopOffset())
    ...
    def set_top_offset(self, value):
        ...
        if not isinstance(value,Point2D):
            return False
        self._jobject.setTopOffset(value._jobject)


class Table3D(CSGEntity):
    """
    棱台体
    """
    def __init__(self, _bottom_length=10, _bottom_length1=10, _top_length=5, _top_length1=5, _height=6):
        """
        :param _bottom_length:底面对角线长度1(与X轴重合)
        :param _bottom_length1:底面对角线长度2(与Y轴重合)
        :param _top_length:顶面对角线长度1(与X轴平行)
        :param _top_length1:顶面对角线长度2(与Y轴平行)
        :param _height:高度
        """
        self._java_object = get_jvm().com.supermap.data.Table3D(float(_bottom_length), float(_bottom_length1),
                                                            float(_top_length),
                                                            float(_top_length1), float(_height))

    @property
    def bottom_length(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getBottomLength()

    def set_bottom_length(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.setBottomLength(float(value))

    @property
    def bottom_length1(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getBottomLength1()

    def set_bottom_length1(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setBottomLength1(float(value))

    @property
    def top_length(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getTopLength()

    def set_top_length(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setTopLength(float(value))

    @property
    def top_length1(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getTopLength1()

    def set_top_length1(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setTopLength1(float(value))
...
class Wedge(CSGEntity):
    """
    楔形体
    """

    def __init__(self, _bottom_length=10, _bottom_Width=8, _top_length=8, _topWidth=0, _height=15,
                 _top_offset=Point2D(1.0, 4.0)):
        """
        :param _bottom_length:底面对角线长度1(与X轴重合)
        :param _bottom_length1:底面对角线长度2(与Y轴重合)
        :param _top_length:顶面对角线长度1(与X轴平行)
        :param _top_length1:顶面对角线长度2(与Y轴平行)
        :param _top_offset:顶面偏移
        :param _height:高度
        """
        self._java_object = get_jvm().com.supermap.data.Wedge(float(_bottom_length), float(_bottom_Width),
                                                          float(_top_length), float(_topWidth),
                                                          float(_height), _top_offset._jobject)
    ...
    def set_bottom_length(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setBottomLength(float(value))

    @property
    def bottom_width(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getBottomWidth()

    def set_bottom_width(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setBottomWidth(float(value))
    ...
    def top_width(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getTopWidth()

    def set_top_width(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setTopWidth(float(value))
...
class Pyramid(CSGEntity):
    ...
    def __init__(self, _length=10, _width=10, _height=10):
        ...
        :length width ,height 长宽高
        """
        self._java_object = get_jvm().com.supermap.data.Pyramid(float(_length), float(_width), float(_height))
...
class EllipticRing(CSGEntity):
    """
    椭圆环
    """
    def __init__(self, _semi_major_axis=10, _semi_minor_axis=8, _pipe_padius=1):
        """
        :param semi_major_axis:长半轴
        :param _semi_minor_axis:短半轴
        :param _pipe_padius:管半径
        """
        self._java_object = get_jvm().com.supermap.data.EllipticRing(float(_semi_major_axis), float(_semi_minor_axis),
                                                                 float(_pipe_padius))
    ...
    def semi_major_axis(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getSemiMajorAxis()

    def set_semi_major_axis(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setSemiMajorAxis(float(value))

    @property
    def semi_minor_axis(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._jobject.getSemiMinorAxis()

    def set_semi_minor_axis(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setSemiMinorAxis(float(value))
...
class RectangularRing(CSGEntity):
    """
    矩形环
    """
    def __init__(self, _length=10, _width=8, _pipe_radius=1):
        """
        :param _length:长
        :param _width:宽
        :param _pipe_radius:管半径
        """
        self._java_object = get_jvm().com.supermap.data.RectangularRing(float(_length), float(_width),
                                                                        float(_pipe_radius))
...
class SlopedCylinder(CSGEntity):
    """
    斜口圆柱
    """
    def __init__(self, _bottom_radius=10, _top_radius=10, _height=50, _top_slope=Point2D(0.0, 0.0),
                 _bottom_slope=Point2D(0.0, 0.0)):
        """
        :param _bottom_radius:底部半径
        :param _top_radius:顶部半径
        :param _height:高
        :param _top_angle:顶部坡度(与X、Y轴角度)
        :param _bottom_angle:底部坡度(与X、Y轴角度)
        """
        self._java_object = get_jvm().com.supermap.data.SlopedCylinder(float(_bottom_radius), float(_top_radius),
                                                                       float(_height),
                                                                       _top_slope._jobject, _bottom_slope._jobject)

    @property
    def bottom_radius(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.getBottomRadius()

    def set_bottom_radius(self, value):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        self._java_object.setBottomRadius(float(value))
    ...
    def top_slope(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return Point2D._from_java_object(self._jobject.getTopSlope())

    def set_top_slope(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setTopSlope(value._jobject)

    @property
    def bottom_slope(self):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        return Point2D._from_java_object(self._jobject.getBottomSlope())

    def set_bottom_slope(self, value):
        if self._jobject is None:
            raise ObjectDisposedError(type(self).__name__)
        self._jobject.setBottomSlope(value._jobject)


class BendingCylinder(CSGEntity):
    """
    弯折圆柱
    """
    def __init__(self, _radius=3, _angle=30, _length=10):
        """
        :param _radius:半径
        :param _angle:角度
        :param _length:长度
        """
        self._java_object = get_jvm().com.supermap.data.BendingCylinder(float(_radius), float(_angle), float(_length))

    @property
    def radius(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.getRadius()

    def set_radius(self, value):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        self._java_object.setRadius(float(value))

    @property
    def angle(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.getAngle()

    def set_angle(self, value):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        self._java_object.setAngle(float(value))

    @property
    def length(self):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        return self._java_object.getLength()

    def set_length(self, value):
        if self._java_object is None:
            raise ObjectDisposedError(type(self).__name__)
        self._java_object.setLength(float(value))
