...
__all__ = ['is_auto_close_output_datasource', 'set_auto_close_output_datasource', 'is_use_analyst_memory_mode',
           'set_analyst_memory_mode', 'get_omp_num_threads', 'set_omp_num_threads', 'set_iobjects_java_path',
           'get_iobjects_java_path', 'get_show_features_count', 'set_show_features_count']
...
SHOW_FEATURES_COUNT = 'show_features_count'
...
class Env:
    ...
    def set_iobjects_java_bin_path(cls, iobjects_java_bin_path):
        ...
        if iobjects_java_bin_path is not None:
            if os.path.isdir(iobjects_java_bin_path) and os.path.exists(
                    iobjects_java_bin_path):
                cls._prepare_set()
                cls._lock.acquire()
    ...
    @classmethod
    def get_show_features_count(cls):
        """
        矢量数据集打印最多显示的记录数

        :rtype: int
        """
        if SHOW_FEATURES_COUNT in cls._settings:
            try:
                return int(cls._settings[SHOW_FEATURES_COUNT])
            except:
                return 10
        else:
            return 10
    ...
    @classmethod
    def set_show_features_count(cls, number_features):
        """
        设置矢量数据集打印最多显示的记录数

        :param int number_features: 矢量数据集打印最多显示的记录数
        """
        if number_features and int(number_features) > 0:
            cls._prepare_set()
            cls._lock.acquire()
            cls._settings[SHOW_FEATURES_COUNT] = int(number_features)
            cls._write()
            cls._lock.release()
...
def set_iobjects_java_path(bin_path, is_copy_jars=True):
    ...
    """
    if bin_path is not None and is_copy_jars:
...
def get_show_features_count():
    """
    获取打印矢量数据集时显示的记录数目。

    :rtype: int
    """
    return Env.get_show_features_count()


def set_show_features_count(features_count):
    """
    设置打印矢量数据集时显示的记录数目
    ...
    :param int features_count: 打印矢量数据集时显示的记录数目
    """
    Env.set_show_features_count(int(features_count))
