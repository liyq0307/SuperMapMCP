...
from iobjectspy._jsuperpy.data._util import (to_java_recordset_array,
                                             get_input_dataset,
...
__all__ = ['ProcessingOptions', 'topology_processing', 'topology_build_regions', 'pickup_border', 'PreprocessOptions',
           'preprocess', 'topology_validate', 'split_lines_by_regions', 'integrate']
...
class ProcessingOptions(object):
    ...
    def __init__(self, pseudo_nodes_cleaned=False,
                 overshoots_cleaned=False,
                 redundant_vertices_cleaned=False,
                 undershoots_extended=False,
                 duplicated_lines_cleaned=False,
                 lines_intersected=False,
                 adjacent_endpoints_merged=False,
                 overshoots_tolerance=1.0e-10,
                 undershoots_tolerance=1.0e-10,
                 vertex_tolerance=1.0e-10,
                 filter_vertex_recordset=None,
                 arc_filter_string=None,
                 filter_mode=None):
        """
        构造拓扑处理参数类
        ...
        self.set_pseudo_nodes_cleaned(pseudo_nodes_cleaned). \
            set_overshoots_cleaned(overshoots_cleaned). \
            set_overshoots_tolerance(overshoots_tolerance). \
            set_redundant_vertices_cleaned(redundant_vertices_cleaned). \
            set_undershoots_extended(undershoots_extended). \
            set_undershoots_tolerance(undershoots_tolerance). \
            set_duplicated_lines_cleaned(duplicated_lines_cleaned). \
            set_lines_intersected(lines_intersected). \
            set_adjacent_endpoints_merged(adjacent_endpoints_merged). \
            set_vertex_tolerance(vertex_tolerance). \
            set_filter_vertex_recordset(filter_vertex_recordset). \
            set_arc_filter_string(arc_filter_string). \
...
def topology_processing(input_data,
                        pseudo_nodes_cleaned=True,
                        overshoots_cleaned=True,
                        redundant_vertices_cleaned=True,
                        undershoots_extended=True,
                        duplicated_lines_cleaned=True,
                        lines_intersected=True,
                        adjacent_endpoints_merged=True,
                        overshoots_tolerance=1.0e-10,
                        undershoots_tolerance=1.0e-10,
                        vertex_tolerance=1.0e-10,
                        filter_vertex_recordset=None,
                        arc_filter_string=None,
                        filter_mode=None, options=None, progress=None):
    """
    ...
    if options is not None:
        ...
        java_option = ProcessingOptions(pseudo_nodes_cleaned, overshoots_cleaned, redundant_vertices_cleaned,
                                        undershoots_extended, duplicated_lines_cleaned, lines_intersected,
                                        adjacent_endpoints_merged, overshoots_tolerance, undershoots_tolerance,
                                        vertex_tolerance, filter_vertex_recordset, arc_filter_string,
                                        filter_mode)._jobject
    listener = None
    ...
    try:
        is_success = _jvm.com.supermap.data.topology.TopologyProcessing.clean(_source_input._jobject,
                                                                              java_option)
    except Exception as e:
        log_error(e)
        is_success = False
...
def topology_build_regions(input_data, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        java_result_dt = _jvm.com.supermap.data.topology.TopologyProcessing.buildRegions(_source_input._jobject,
                                                                                         _ds._jobject,
                                                                                         _outDatasetName)
    except Exception as e:
        log_error(e)
...
def pickup_border(input_data, is_preprocess=True, extract_ids=None, out_data=None, out_dataset_name=None,
                  progress=None):
    """
    ...
    try:
        ...
        if extract_ids is None:
            ...
            java_result_dt = pickupBorder(oj(_source_input), oj(_ds), _outDatasetName, extract_ids,
                                          bool(is_preprocess))
    except Exception as e:
        log_error(e)
        java_result_dt = None
...
class PreprocessOptions(object):
    ...
    def __init__(self, arcs_inserted=False, vertex_arc_inserted=False, vertexes_snapped=False, polygons_checked=False,
                 vertex_adjusted=False):
        """
        构造拓扑预处理参数类对象
        ...
        """
        self.set_arcs_inserted(arcs_inserted).set_polygons_checked(polygons_checked).set_vertex_adjusted(
            vertex_adjusted). \
            set_vertex_arc_inserted(vertex_arc_inserted).set_vertexes_snapped(vertexes_snapped)
...
def preprocess(inputs, arcs_inserted=True, vertex_arc_inserted=True, vertexes_snapped=True, polygons_checked=True,
               vertex_adjusted=True, precisions=None, tolerance=1.0e-10, options=None, progress=None):
    """
    对给定的拓扑数据集进行拓扑预处理。
    ...
    if options is not None and isinstance(options, PreprocessOptions):
        ...
        parameters = PreprocessOptions(arcs_inserted, vertex_arc_inserted, vertexes_snapped, polygons_checked,
                                       vertex_adjusted)
    _jvm = get_jvm()
    listener = None
    if progress is not None and safe_start_callback_server():
    ...
    try:
        ...
        is_success = _jvm.com.supermap.data.topology.TopologyValidator.preprocess(
            to_java_recordset_array(_all_recordsets),
            to_java_int_array(_all_orders),
            parameters._jobject,
            float(tolerance))
    except:
        import traceback
...
def topology_validate(source_data, validating_data, rule, tolerance, validate_region=None,
    ...
    # rule -> check with, need preprocess
    ...
    if isinstance(_source_input, DatasetVector):
        if _validate_input is not None:
            if isinstance(_validate_input, Recordset):
    ...
    try:
        java_result_dt = _jvm.com.supermap.data.topology.TopologyValidator.validate(_source_input._jobject,
                                                                                    java_validate_input,
                                                                                    rule._jobject,
                                                                                    tolerance,
                                                                                    javaValidateRegion,
...
def split_lines_by_regions(line_input, region_input, progress=None):
    ...
    if progress is not None and safe_start_callback_server():
        try:
            ...
            listener = None
    try:
        is_success = _jvm.com.supermap.data.topology.TopologyProcessing.splitLinesByRegions(_line_rd._jobject,
                                                                                            _region_rd._jobject)
    except Exception as e:
        log_error(e)
        is_success = False
...
def integrate(source_dataset, tolerance, unit=None, precision_orders=None, progress=None):
    """
    对数据集进行数据整合，整合的过程包括节点捕捉和插点操作。与 py:func:`.preprocess`: 的具有相似的功能，能将数据中的拓扑错误处理掉，与
    ...
    True
    >>> integrate([ds['street'], ds['poi']], 1.0, 'meter', [0,1])
    True

    :param source_dataset: 被处理的数据集或数据集几何
    :type source_dataset: DatasetVector or str or list[DatasetVector] or list[str]
    :param float tolerance: 结点容限
    :param unit: 节点容限单位，当为 None 时使用数据集坐标系单位。如果数据集坐标系为投影坐标系，禁止使用角度单位。
    :type unit: Unit or str
    :param precision_orders: 数据精度等级数组。精度等级的值越小，代表对应数据集的精度越高，数据质量越好。在进行顶点捕捉时，低精度的数据集中
                             的点将被捕捉到高精度数据集中的点的位置上。精度等级数组必须与要与输入的数据集集合元素数量相同并一一对应。如果
                             为 None，则每个数据集的精度都相同。
    :type precision_orders: list[int] or tuple[int]
    :param function progress: 进度信息处理函数，具体参考 :py:class:`.StepEvent`
    :return: 成功返回 True，否则返回False
    :rtype: bool
    """
    if source_dataset is None:
        raise ValueError('source_dataset is None')
    if isinstance(source_dataset, str):
        source_dataset = split_input_list_from_str(source_dataset)
    elif isinstance(source_dataset, DatasetVector):
        source_dataset = [source_dataset]
    datasets = []
    for dt in source_dataset:
        dt1 = get_input_dataset(dt)
        if not isinstance(dt1, DatasetVector):
            raise ValueError('source dataset required DatasetVector, but now is ' + str(type(dt)))
        if dt1.type is not DatasetType.LINE and dt1.type is not DatasetType.REGION and dt1.type is not DatasetType.POINT:
            raise ValueError(
                'source dataset only support Dataset Point, Line or Region, but now is ' + str(dt1.type))
        datasets.append(dt1)
    if not datasets:
        raise ValueError('Have no valid datasets')

    if precision_orders:
        if isinstance(precision_orders, str):
            precision_orders = split_input_int_list_from_str(precision_orders)
        elif not isinstance(precision_orders, (list, tuple)):
            precision_orders = [int(precision_orders)]
        precision_orders = to_java_int_array(precision_orders)

    unit = make_enum(Unit, unit)
    if unit is None:
        unit = datasets[0].prj_coordsys.coord_unit

    listener = None
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'integrate')
            get_jvm().com.supermap.data.topology.TopologyValidator.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
            listener = None
    try:
        result = get_jvm().com.supermap.data.topology.TopologyValidator.integrate(to_java_datasetvector_array(datasets),
                                                                                  precision_orders,
                                                                                  float(tolerance),
                                                                                  oj(unit),
                                                                                  None)
        ...
        if listener is not None:
            try:
                get_jvm().com.supermap.data.topology.TopologyValidator.removeSteppedListener(listener)
            except Exception as e1:
                log_error(e1)
            close_callback_server()
        return result
