...
__all__ = ['measure_central_element', 'measure_directional', 'measure_linear_directional_mean', 'measure_mean_center',
           ...
           'build_weight_matrix', 'weight_matrix_file_to_table', 'GWR', 'GTWR', 'GWRSummary', 'OLSSummary',
           'ordinary_least_squares', 'InteractionDetectorResult', 'RiskDetectorMean', 'RiskDetectorResult',
           'GeographicalDetectorResult', 'geographical_detector',
           'density_based_clustering', 'hierarchical_density_based_clustering', 'ordering_density_based_clustering',
           'spa_estimation', 'bshade_estimation', 'bshade_sampling', 'BShadeSamplingResult', 'BShadeEstimationResult',
...
def _measure_param(group_field=None, weight_field=None, self_weight_field=None, distance_method='EUCLIDEAN',
                   stats_fields=None, ellipse_size=None, is_orientation=None):
...
def measure_central_element(source, group_field=None, weight_field=None, self_weight_field=None,
                            distance_method='EUCLIDEAN', stats_fields=None, out_data=None, out_dataset_name=None,
                            progress=None):
    """
    关于空间度量：
...
def measure_directional(source, group_field=None, ellipse_size='SINGLE', stats_fields=None,
...
def measure_linear_directional_mean(source, group_field=None, weight_field=None, is_orientation=False,
                                    stats_fields=None,
                                    out_data=None, out_dataset_name=None, progress=None):
    """
...
def measure_mean_center(source, group_field=None, weight_field=None, stats_fields=None, out_data=None,
...
def measure_median_center(source, group_field, weight_field, stats_fields=None, out_data=None, out_dataset_name=None,
                          progress=None):
    """
...
def measure_standard_distance(source, group_field, weight_field, ellipse_size='SINGLE', stats_fields=None,
                              out_data=None, out_dataset_name=None, progress=None):
    """
...
def _patterns_param(assessment_field=None, concept_model='INVERSEDISTANCE', distance_method='EUCLIDEAN',
                    distance_tolerance=-1.0, exponent=1.0, weight_file_path=None, k_neighbors=1,
                    is_standardization=False, is_FDR_adj=False, self_weight_field=None):
...
def auto_correlation(source, assessment_field, concept_model='INVERSEDISTANCE', distance_method='EUCLIDEAN',
                     distance_tolerance=-1.0, exponent=1.0, k_neighbors=1,
                     is_standardization=False, weight_file_path=None, progress=None):
    """
    分析模式介绍：
    ...
    try:
        ...
        java_param = _patterns_param(assessment_field, concept_model, distance_method, distance_tolerance, exponent,
                                     weight_file_path, k_neighbors, is_standardization)
...
def high_or_low_clustering(source, assessment_field, concept_model='INVERSEDISTANCE', distance_method='EUCLIDEAN',
                           distance_tolerance=-1.0, exponent=1.0,
                           k_neighbors=1, is_standardization=False, weight_file_path=None, progress=None):
    """
...
def average_nearest_neighbor(source, study_area, distance_method='EUCLIDEAN', progress=None):
    ...
    * 距离计算方法类型可以指定相邻要素之间的距离计算方式(参阅 :py:class:`.DistanceMethod` )。如果输入数据集为地理坐标系，则会采用弦测量方法来
    ...
    try:
        ...
        java_result = averageNearestNeighbor(oj(source_dt), float(study_area), oj(DistanceMethod._make(distance_method,
                                                                                                       'EUCLIDEAN')))
    except:
        import traceback
        log_error(traceback.format_exc())
...
def incremental_auto_correlation(source, assessment_field, begin_distance=0.0, distance_method='EUCLIDEAN',
                                 incremental_distance=0.0, incremental_number=10, is_standardization=False,
                                 progress=None):
    """
...
def cluster_outlier_analyst(source, assessment_field, concept_model='INVERSEDISTANCE', distance_method='EUCLIDEAN',
                            distance_tolerance=-1.0, exponent=1.0, is_FDR_adjusted=False,
                            k_neighbors=1, is_standardization=False, weight_file_path=None,
                            out_data=None, out_dataset_name=None, progress=None):
    """
    聚类分布介绍：
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                get_jvm().com.supermap.analyst.spatialstatistics.ClusteringDistributions.addSteppedListener(
                    listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
        ...
        java_param = _patterns_param(assessment_field, concept_model, distance_method, distance_tolerance, exponent,
                                     weight_file_path, k_neighbors, is_standardization, is_FDR_adjusted)
...
def hot_spot_analyst(source, assessment_field, concept_model='INVERSEDISTANCE', distance_method='EUCLIDEAN',
                     distance_tolerance=-1.0, exponent=1.0, is_FDR_adjusted=False,
                     k_neighbors=1, is_standardization=False, self_weight_field=None, weight_file_path=None,
                     out_data=None, out_dataset_name=None, progress=None):
    """
    热点分析，返回结果矢量数据集。
...
def optimized_hot_spot_analyst(source, assessment_field=None, aggregation_method='NETWORKPOLYGONS',
                               aggregating_polygons=None, bounding_polygons=None, out_data=None,
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                get_jvm().com.supermap.analyst.spatialstatistics.StatisticsUtilities.addSteppedListener(
                    listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
...
def build_weight_matrix(source, unique_id_field, file_path, concept_model='INVERSEDISTANCE',
                        distance_method='EUCLIDEAN', distance_tolerance=-1.0, exponent=1.0,
                        k_neighbors=1, is_standardization=False, progress=None):
    """
    构建空间权重矩阵。
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                get_jvm().com.supermap.analyst.spatialstatistics.WeightsUtilities.addSteppedListener(
                    listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
        ...
        java_param = _patterns_param(None, concept_model, distance_method, distance_tolerance, exponent, None,
                                     k_neighbors, is_standardization, None, None)
...
def weight_matrix_file_to_table(file_path, out_data, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result = get_jvm().com.supermap.analyst.spatialstatistics.WeightsUtilities.converToTableDataset(
            str(file_path),
            oj(out_datasource),
            _outDatasetName)
    except:
...
def _set_gwr_parameter(java_param, explanatory_fields, model_field, kernel_function='GAUSSIAN',
                       band_width_type='AICC', distance_tolerance=0.0, kernel_type='FIXED', neighbors=2,
                       prediction_dataset=None, explanatory_fields_matching=None,
                       out_predicted_name=None, out_datasource=None):
    if explanatory_fields is not None:
        explanatory_fields = split_input_list_from_str(explanatory_fields)
        java_param.setExplanatoryFields(to_java_string_array(explanatory_fields))
    if model_field is not None:
        java_param.setModelFeild(str(model_field))
    if kernel_function is not None:
        java_param.setKernelFunction(oj(make_enum(KernelFunction, kernel_function)))
    if kernel_type is not None:
        java_param.setKernelType(oj(make_enum(KernelType, kernel_type)))
    if band_width_type is not None:
        java_param.setBandWidthType(oj(make_enum(BandWidthType, band_width_type)))
    if distance_tolerance is not None:
        java_param.setDistanceTolerance(float(distance_tolerance))
    if neighbors is not None:
        java_param.setNeighbors(int(neighbors))

    prediction_dataset = get_input_dataset(prediction_dataset)
    if isinstance(prediction_dataset, DatasetVector):
        java_param.setPredictionDataset(oj(prediction_dataset))
        java_param.setTargetPredictedDatasource(oj(out_datasource))
        if out_predicted_name is None:
            out_predicted_name = prediction_dataset.name + '_predicted'
        out_predicted_name = out_datasource.get_available_dataset_name(out_predicted_name)
        java_param.setTargetPredictedDatasetName(out_predicted_name)
        if explanatory_fields_matching:
            explanatory_fields_matching = split_input_dict_from_str(explanatory_fields_matching)
            java_param.setExplanatoryFieldsMatching(explanatory_fields_matching)
    return java_param


def GWR(source, explanatory_fields, model_field, kernel_function='GAUSSIAN',
        band_width_type='AICC', distance_tolerance=0.0, kernel_type='FIXED', neighbors=2,
        out_data=None, out_dataset_name=None, progress=None, prediction_dataset=None, explanatory_fields_matching=None,
        out_predicted_name=None):
    """
    空间关系建模介绍：
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                get_jvm().com.supermap.analyst.spatialstatistics.SpatialRelModeling.addSteppedListener(
                    listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
        ...
        _set_gwr_parameter(java_param, explanatory_fields, model_field, kernel_function, band_width_type,
                           distance_tolerance, kernel_type, neighbors, prediction_dataset, explanatory_fields_matching,
                           out_predicted_name, out_datasource)
        ...
        if java_result is None:
            ...
            java_predicted_result = java_result.getPredictedResultDataset()
            if java_predicted_result:
                predicted_dt = out_datasource[java_predicted_result.getName()]
            else:
                predicted_dt = None
        return gwr_summary, gwr_dt, predicted_dt


def GTWR(source, explanatory_fields, model_field, time_field, time_distance_unit='DAYS',
         kernel_function='GAUSSIAN', band_width_type='AICC', distance_tolerance=0.0, kernel_type='FIXED', neighbors=2,
         out_data=None, out_dataset_name=None, progress=None, prediction_dataset=None, prediction_time_field=None,
         explanatory_fields_matching=None, out_predicted_name=None):
    """
    时空地理加权回归。
    ...
    :type explanatory_fields: list[str] or str
    :param str model_field: 建模字段的名称
    :param str time_field: 数据集时间字段名称
    :param time_distance_unit: 时间距离单位。时间距离是 time_field 字段内两条记录的时间插值，时间距离需要统一转换到指定的时间距离单位下，比
                               如，时间间隔为60分钟，则相应的时间距离可以为：60 Minutes, 或1 Hours，或 1/24 Days等。不同的时间距离单位，
                               在不同的核函数中，权重可能不同。
    ...
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        _outDatasetName = source_dt.name + '_gtwr'
    else:
        _outDatasetName = out_dataset_name
    _outDatasetName = out_datasource.get_available_dataset_name(_outDatasetName)
    listener = None
    try:
        if progress is not None and safe_start_callback_server():
            try:
                listener = ProgressListener(progress, 'GTWR')
                get_jvm().com.supermap.analyst.spatialstatistics.SpatialRelModeling.addSteppedListener(
                    listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
                listener = None
        java_param = get_jvm().com.supermap.analyst.spatialstatistics.GTWRParameter()
        _set_gwr_parameter(java_param, explanatory_fields, model_field, kernel_function, band_width_type,
                           distance_tolerance, kernel_type, neighbors, prediction_dataset, explanatory_fields_matching,
                           out_predicted_name, out_datasource)
        java_param.setTimeField(str(time_field))
        if prediction_time_field:
            java_param.setPredictionTimeField(str(prediction_time_field))
        time_distance_unit = make_enum(TimeDistanceUnit, time_distance_unit, 'DAYS')
        java_param.setTimeSpanUnits(oj(time_distance_unit))
        gtwr = (get_jvm().com.supermap.analyst.spatialstatistics.SpatialRelModeling.
                geographicallyAndTemporallyWeightedRegression)
        java_result = gtwr(oj(source_dt), oj(out_datasource), _outDatasetName, java_param)
        del java_param
    except:
        import traceback
        log_error(traceback.format_exc())
...
def ordinary_least_squares(input_data, explanatory_fields, model_field, out_data=None, out_dataset_name=None,
                           progress=None):
    """
    普通最小二乘法。
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                _jvm.com.supermap.analyst.spatialstatistics.SpatialRelModeling.addSteppedListener(
                    listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
        ...
        if java_result is not None:
            return result_dt, OLSSummary(java_result.getOLSSummary())
        else:
            return None


def geographical_detector(input_data, model_field, explanatory_fields, is_factor_detector=True,
                          is_ecological_detector=True, is_interaction_detector=True, is_risk_detector=True,
                          progress=None):
    """
    ...
    try:
        ...
        java_result = geographicalDetector(oj(source_dt), str(model_field),
                                           to_java_string_array(split_input_list_from_str(explanatory_fields)),
                                           bool(is_factor_detector), bool(is_ecological_detector),
                                           bool(is_interaction_detector), bool(is_risk_detector), None)
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
class RiskDetectorResult:
    ...
    def __init__(self, java_object):
        if java_object is None:
            ...
            self._values = [RiskDetectorResult._get_risk_detector_values(values) for values in
                            java_object.getRiskDetectorValues()]
...
def density_based_clustering(input_data, min_pile_point_count, search_distance, unit,
                             out_data=None, out_dataset_name=None, progress=None):
    """
    密度聚类的DBSCAN实现
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialstatistics.ClusteringDistributions. \
            densityBasedClustering(_source_input._jobject, _ds._jobject, _outDatasetName,
                                   int(min_pile_point_count), float(search_distance), Unit._make(unit)._jobject)
    except Exception as e:
        log_error(e)
        java_result_dt = None
    finally:
...
def hierarchical_density_based_clustering(input_data, min_pile_point_count,
                                          out_data=None, out_dataset_name=None, progress=None):
    """
    密度聚类的HDBSCAN实现
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialstatistics.ClusteringDistributions. \
            hierarchicalDensityBasedClustering(_source_input._jobject, _ds._jobject, _outDatasetName,
                                               int(min_pile_point_count))
    except Exception as e:
        log_error(e)
        java_result_dt = None
    finally:
...
def ordering_density_based_clustering(input_data, min_pile_point_count, search_distance, unit, cluster_sensitivity,
                                      out_data=None, out_dataset_name=None, progress=None):
    """
    密度聚类的OPTICS实现
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialstatistics.ClusteringDistributions. \
            orderingDensityBasedClustering(_source_input._jobject, _ds._jobject, _outDatasetName,
                                           int(min_pile_point_count), float(search_distance),
                                           Unit._make(unit)._jobject, int(cluster_sensitivity))
    except Exception as e:
        log_error(e)
        java_result_dt = None
    finally:
...
def spa_estimation(source_dataset, reference_dataset, source_unique_id_field, source_data_field,
                   reference_unique_id_field, reference_data_fields, out_data=None, out_dataset_name=None,
                   progress=None):
    """
    单点地域估计(SPA)
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialstatistics.SamplingInference. \
            SPA(oj(_source_input), oj(_ref_input), str(source_unique_id_field), str(source_data_field),
                str(reference_unique_id_field), to_java_string_array(reference_data_fields),
                oj(_ds), str(_dest_dt_name))
    except Exception as e:
        log_error(e)
        java_result_dt = None
    finally:
...
class BShadeEstimationResult:
    ...
    def to_dict(self):
        ...
        """
        return {'dataset': self.dataset, 'variance': self.variance, 'weights': self.weights}


def bshade_estimation(source_dataset, historical_dataset, source_data_fields,
                      historical_fields, estimate_method='TOTAL', out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        java_result = _jvm.com.supermap.analyst.spatialstatistics.SamplingInference. \
            BShadeEstimation(oj(_source_input), oj(_historical_input), to_java_string_array(source_data_fields),
                             to_java_string_array(historical_fields), oj(estimate_method), oj(_ds), str(_dest_dt_name))
    except Exception as e:
        log_error(e)
        java_result = None
    finally:
        ...
        if java_result is not None:
            ...
            return BShadeEstimationResult()._from_result(result_dt, java_result.getEstimatedVariance(),
                                                         java_array_to_list(java_result.getWeights()))
        return None
...
class BShadeSamplingResult:
    ...
    def to_dict(self):
        ...
        :rtype: dict[str, object]
        """
        return {'sample_number': self.sample_number, 'estimate_variance': self.estimate_variance,
...
class BShadeSamplingParameter:
    ...
    def __init__(self):
        ...
        self._min_temperature = 1.0e-8
        self._min_energy = -1.0e38
        self._cool_rate = 0.9
...
def bshade_sampling(historical_dataset, historical_fields, parameter, progress=None):
    ...
    try:
        java_result = _jvm.com.supermap.analyst.spatialstatistics.SamplingInference. \
            BShadeSampling(oj(_historical_input), to_java_string_array(historical_fields), oj(parameter))
    except Exception as e:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
        ...
        if java_result is not None:
            ...
            for item in java_result:
                result = BShadeSamplingResult()._from_result(item.getSampleNumber(),
                                                             item.getEstimatedVariance(),
                                                             java_array_to_list(item.getWeights()),
                                                             java_array_to_list(item.getSolutionNames()))
