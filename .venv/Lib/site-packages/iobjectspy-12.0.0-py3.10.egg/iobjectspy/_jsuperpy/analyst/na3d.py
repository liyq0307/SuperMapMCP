...
__all__ = ['NetworkSplitMode3D', 'build_network_dataset_known_relation_3d', 'build_facility_network_directions_3d',
...
def build_network_dataset_3d(line, point=None, split_mode='NO_SPLIT', tolerance=0.0, line_saved_fields=None,
    ...
    if point is not None and point.type is not DatasetType.POINT3D:
    ...
    try:
        ...
        if point is not None:
            java_result = network_builder_func(oj(line), oj(point),
                                               ...
                                               oj(NetworkSplitMode3D._make(split_mode, 'NO_SPLIT')),
                                               float(tolerance))
        else:
            java_result = network_builder_func(oj(line),
                                               to_java_string_array(split_input_list_from_str(line_saved_fields)),
                                               oj(out_datasource), str(dest_name),
                                               oj(NetworkSplitMode3D._make(split_mode, 'NO_SPLIT')),
                                               float(tolerance))
...
def build_network_dataset_known_relation_3d(line, point, edge_id_field, from_node_id_field, to_node_id_field,
...
def build_facility_network_directions_3d(network_dataset, source_ids, sink_ids, direction_field='Direction',
...
class FacilityAnalyst3D(JVMBase):
    ...
    def find_sink(self, edge_or_node_id, weight_name=None, is_uncertain_direction_valid=False, is_edge_id=True):
        ...
        java_result = func(int(edge_or_node_id), str(weight_name) if weight_name is not None else None,
                           parse_bool(is_uncertain_direction_valid))
        if java_result is not None:
            return FacilityAnalystResult3D(java_result)
        return None
...
class TransportationAnalystSetting3D:
    ...
    def set_network_dataset(self, dataset):
        ...
        if dataset is not None:
            ...
            if isinstance(dataset, DatasetVector) and dataset.type is DatasetType.NETWORK3D:
