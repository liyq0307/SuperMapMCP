...
from iobjectspy._jsuperpy._utils import (split_input_list_from_str,
                                         oj,
                                         parse_bool,
                                         java_array_to_list)
...
__all__ = ['build_address_indices', 'AddressItem', 'AddressSearch']


def get_default_dictionary_file(is_traditional=False):
    curr_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    if is_traditional:
        return curr_dir + '/address_t.dct'
    else:
        return curr_dir + '/address.dct'


def build_address_indices(output_directory, datasets, index_fields, save_fields=None,
                          top_group_field=None, secondary_group_field=None,
                          lowest_group_field=None, is_build_reverse_matching_indices=False,
                          bin_distance=0.0, dictionary_file=None, is_append=False, is_traditional=False):
    """
    构建地址匹配的索引文件
    ...
    if not os.path.exists(output_directory):
        ...
        if os.listdir(output_directory):
            if not os.path.exists(address_gp):
                ...
                if not is_append:
                    log_warning(
                        'output directory "' + output_directory + '" have address indices, may be overwritten. You can '
                                                                  'use is_append=True for append data to this indices')
        else:
            is_append = False
    ...
    dictionary_file = dictionary_file if dictionary_file else get_default_dictionary_file(is_traditional)
    ...
    java_setting.setTraditional(bool(is_traditional))
...
class _AddressSearchParameter:
    def __init__(self, is_address_segmented=False,
                 is_location_return=True,
                 max_result_count=10,
                 top_group_name=None,
                 secondary_group_name=None,
                 lowest_group_name=None):
        ...
        self.set_address_segmented(is_address_segmented).set_location_return(is_location_return).set_max_result_count(
            max_result_count).set_top_group_name(top_group_name).set_secondary_group_name(
            secondary_group_name).set_lowest_group_name(lowest_group_name)
...
class AddressSearch(JVMBase):
    ...
    def match(self, address, is_address_segmented=False, is_location_return=True,
              max_result_count=10, top_group_name=None, secondary_group_name=None, lowest_group_name=None):
        """
        地址匹配。该方法支持多线程。
        ...
        parameter = _AddressSearchParameter(is_address_segmented, is_location_return, max_result_count,
                                            top_group_name, secondary_group_name, lowest_group_name)
