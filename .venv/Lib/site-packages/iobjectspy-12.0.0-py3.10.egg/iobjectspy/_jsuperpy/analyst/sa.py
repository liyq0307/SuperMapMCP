...
from iobjectspy._jsuperpy._gateway import (get_gateway, get_jvm,
...
from iobjectspy._jsuperpy.data._util import (to_java_point2d_array,
                                             get_input_dataset,
                                             get_output_datasource,
                                             check_output_datasource,
                                             try_close_output_datasource,
                                             create_result_datasaet,
                                             to_java_stattype_array,
                                             to_java_geometry_array,
                                             to_java_point3ds,
                                             to_java_datasetgrid_array,
                                             to_java_datasetvector_array,
                                             to_java_dataset_array)
...
__all__ = ['create_buffer', 'overlay', 'multilayer_overlay', 'dissolve', 'aggregate_points', 'smooth_vector',
           'resample_vector', 'create_random_points', 'regularize_building_footprint', 'tabulate_area',
           'auto_compute_project_points', 'compute_natural_breaks', 'erase_and_replace_raster',
           'create_thiessen_polygons', 'summary_points', 'clip_vector', 'update_attributes', 'simplify_building',
           'resample_raster', 'ReclassSegment', 'ReclassMappingTable',
           ...
           'compute_range_distance', 'eliminate', 'eliminate_specified_regions', 'edge_match',
           ...
           'grid_neighbour_statistics', 'altitude_statistics',
           'GridHistogram', 'thin_raster', 'build_lake', 'build_terrain', 'area_solar_radiation_days',
           'area_solar_radiation_hours', 'raster_mosaic',
           'zonal_statistics_on_raster_value', 'calculate_profile', 'CutFillResult', 'inverse_cut_fill',
           ...
           'majority_filter', 'expand', 'shrink', 'nibble', 'region_group', 'boundary_clean',
           'CellularAutomataParameter', 'PCACellularAutomataParameter', 'PCAEigenValue',
           'PCAEigenResult', 'PCACellularAutomata', 'CellularAutomataFlushedEvent',
           'ANNTrainResult', 'ANNCellularAutomataParameter', 'ANNCellularAutomata', 'ANNCellularAutomataResult',
           'ANNParameter', 'MCECellularAutomataParameter', 'MCECellularAutomata']
...
def create_buffer(input_data, distance_left, distance_right=None, unit=None, end_type=None, segment=24,
                  is_save_attributes=True, is_union_result=False, out_data=None, out_dataset_name='BufferResult',
                  progress=None):
    """
    创建矢量数据集或记录集的缓冲。
    ...
    * 对“负半径”的说明
    ...
    try:
        result = _jvm.com.supermap.analyst.spatialanalyst.BufferAnalyst.createBuffer(_input._jobject,
                                                                                     result_dt._jobject,
                                                                                     buffer_param,
                                                                                     is_union_result,
                                                                                     is_save_attributes)
...
def create_line_one_side_multi_buffer(input_data, radius, is_left, unit=None, segment=24, is_save_attributes=True,
                                      is_union_result=False, is_ring=True, out_data=None,
                                      out_dataset_name='BufferResult', progress=None):
    """
    ...
    if isinstance(radius, (float, int)):
        radiuses.append(float(radius))
    elif isinstance(radius, (list, tuple)):
        for d in radius:
            radiuses.append(float(d))
    elif isinstance(radius, str):
        ds = split_input_list_from_str(radius)
        for d in ds:
            radiuses.append(float(d))
    if len(radiuses) < 1:
        raise ValueError('radius have no valid value.')

    _jvm = get_jvm()
    _dist_unit = BufferRadiusUnit._make(unit, BufferRadiusUnit.METER)
    ...
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'create_line_one_side_multi_buffer')
            _jvm.com.supermap.analyst.spatialanalyst.BufferAnalyst.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
    ...
    try:
        buffer_func = _jvm.com.supermap.analyst.spatialanalyst.BufferAnalyst
        result = buffer_func.createLineOneSideMultiBuffer(_input._jobject,
                                                          result_dt._jobject,
                                                          to_java_double_array(radiuses),
                                                          _dist_unit._jobject,
                                                          int(segment),
                                                          bool(is_left),
                                                          bool(is_union_result),
                                                          bool(is_save_attributes),
                                                          bool(is_ring))
    except Exception as e:
        log_error(e)
        result = False
...
def create_multi_buffer(input_data, radius, unit=None, segment=24, is_save_attributes=True,
                        is_union_result=False, is_ring=True, out_data=None,
                        out_dataset_name='BufferResult', progress=None):
    """
    ...
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'create_multi_buffer')
            _jvm.com.supermap.analyst.spatialanalyst.BufferAnalyst.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
    ...
    try:
        buffer_func = _jvm.com.supermap.analyst.spatialanalyst.BufferAnalyst
        result = buffer_func.createMultiBuffer(_input._jobject,
                                               result_dt._jobject,
                                               to_java_double_array(radiuses),
                                               _dist_unit._jobject,
                                               int(segment),
...
def overlay(source_input, overlay_input, overlay_mode, source_retained=None, overlay_retained=None,
            tolerance=1.0e-10, out_data=None, out_dataset_name='OverlayOutput', progress=None,
            output_type=OverlayAnalystOutputType.INPUT, is_support_overlap_in_layer=False):
    """
    ...
    :param bool is_support_overlap_in_layer: 是否支持数据集内存在面重叠的对象。默认为 False，即不支持，如果面数据集内存在交叠情形，则叠加
                                             分析结果可能会存在错误。如果设置为 True，将使用新的算法进行计算。
    :return: 结果数据集或数据集名称
    :rtype: DatasetVector or str
    """
    check_lic()
    _source_input = get_input_dataset(source_input)
    _overlay_input = get_input_dataset(overlay_input)
    if _source_input is None:
        raise ValueError('source input_data is None')
    if _overlay_input is None:
        raise ValueError('overlay input_data is None')

    if not isinstance(_source_input, (DatasetVector, Recordset, list, tuple)):
        raise ValueError(
            'source_input required DatasetVector, Recordset, list, tuple, but now is : ' + str(type(_source_input)))
    if not isinstance(_overlay_input, (DatasetVector, Recordset, list, tuple)):
        raise ValueError(
            'overlay_input required DatasetVector, Recordset, list, tuple, but now is : ' + str(type(_overlay_input)))
    if isinstance(_source_input, list):
        _source_input = list(filter(lambda x: isinstance(x, Geometry), _source_input))
    if isinstance(_overlay_input, list):
        _overlay_input = list(filter(lambda x: isinstance(x, Geometry), _overlay_input))
    ...
    if out_data is not None:
        ...
        if hasattr(_source_input, 'datasource'):
            _ds = getattr(_source_input, 'datasource')
        elif hasattr(_overlay_input, 'datasource'):
            _ds = getattr(_overlay_input, 'datasource')
        else:
            raise ValueError('must set valid out_datasource datasource')
    check_output_datasource(_ds)
    if out_dataset_name is None:
        _outDatasetName = OverlayMode._make(overlay_mode).name + '_output'
    else:
        _outDatasetName = out_dataset_name

    geo_type = None
    if isinstance(_source_input, DatasetVector):
        result_dt_type = _source_input.type
    elif isinstance(_source_input, Recordset):
        result_dt_type = _source_input.dataset.type
    else:
        geo_type = _source_input[0].type
        if geo_type == GeometryType.GEOPOINT:
            result_dt_type = DatasetType.POINT
        elif geo_type == GeometryType.GEOLINE:
            result_dt_type = DatasetType.LINE
        elif geo_type == GeometryType.GEOREGION:
            result_dt_type = DatasetType.REGION
        else:
            raise ValueError('invalid geometry type : ' + geo_type.name)

    if geo_type == GeometryType.GEOREGION:
        if isinstance(_overlay_input, list) and _overlay_input[0].type == GeometryType.GEOREGION:
            return _overlay_geometrys(_source_input, _overlay_input, overlay_mode, _ds, _outDatasetName, tolerance,
                                      progress)

    result_dt = create_result_datasaet(_ds, _outDatasetName, result_dt_type)
    if isinstance(_source_input, DatasetVector):
        result_dt.set_prj_coordsys(_source_input.prj_coordsys)
    elif isinstance(_source_input, Recordset):
        result_dt.set_prj_coordsys(_source_input.dataset.prj_coordsys)
    elif isinstance(_overlay_input, DatasetVector):
        result_dt.set_prj_coordsys(_overlay_input.prj_coordsys)
    elif isinstance(_overlay_input, Recordset):
        result_dt.set_prj_coordsys(_overlay_input.dataset.prj_coordsys)

    temp_ds = None
    listener = None
    source_recordset = None
    overlay_recordset = None
    java_source = None
    java_overlay = None
    java_result_dt = oj(result_dt)
    try:
        if isinstance(_source_input, DatasetVector):
            if isinstance(_overlay_input, DatasetVector):
                java_source = _source_input._jobject
                java_overlay = _overlay_input._jobject
            elif isinstance(_overlay_input, Recordset):
                source_recordset = _source_input.get_recordset()
                java_source = source_recordset._jobject
                java_overlay = _overlay_input._jobject
            else:
                java_source = _source_input._jobject
                java_overlay = to_java_geometry_array(_overlay_input)
                overlay_retained = None
        elif isinstance(_source_input, Recordset):
            if isinstance(_overlay_input, DatasetVector):
                java_source = _source_input._jobject
                overlay_recordset = _overlay_input.get_recordset()
                java_overlay = overlay_recordset._jobject
            elif isinstance(_overlay_input, Recordset):
                java_source = _source_input._jobject
                java_overlay = _overlay_input._jobject
            else:
                ...
                dt = temp_ds.write_spatial_data(_overlay_input)
                java_source = _source_input._jobject
                overlay_recordset = dt.get_recordset()
                java_overlay = overlay_recordset._jobject
                overlay_retained = None
        else:
            if temp_ds is None:
                temp_ds = Datasource.create(':memory:')
            dt = temp_ds.write_spatial_data(_source_input)
            source_retained = None
            if isinstance(_overlay_input, DatasetVector):
                java_overlay = _overlay_input._jobject
                dt.set_prj_coordsys(_overlay_input.prj_coordsys)
                java_source = dt._jobject
            elif isinstance(_overlay_input, Recordset):
                dt.set_prj_coordsys(_overlay_input.dataset.prj_coordsys)
                source_recordset = dt.get_recordset()
                java_source = oj(source_recordset)
                java_overlay = _overlay_input._jobject
            else:
                if temp_ds is None:
                    temp_ds = Datasource.create(':memory:')
                dt_overlay = temp_ds.write_spatial_data(_overlay_input)
                java_overlay = dt_overlay._jobject
                java_source = dt._jobject
                overlay_retained = None

        param = _jvm.com.supermap.analyst.spatialanalyst.OverlayAnalystParameter()
        if source_retained is not None:
            param.setSourceRetainedFields(to_java_string_array(split_input_list_from_str(source_retained)))
        if overlay_retained is not None:
            param.setOperationRetainedFields(to_java_string_array(split_input_list_from_str(overlay_retained)))
        param.setTolerance(tolerance)

        if is_support_overlap_in_layer is not None:
            param.setSupportOverlapInLayer(bool(is_support_overlap_in_layer))

        output_type = OverlayAnalystOutputType._make(output_type, 'INPUT')
        param.setOutputType(oj(output_type))
        ...
        result = overlay_funs[_overlayMode](java_source, java_overlay, java_result_dt, param)
    except Exception as e:
        import traceback
        ...
        if source_recordset is not None:
            try:
                source_recordset.close()
                del source_recordset
            except:
                del source_recordset

        if overlay_recordset is not None:
            try:
                overlay_recordset.close()
                del overlay_recordset
            except:
                del overlay_recordset
...
def _overlay_geometrys(source_geos, overlay_geos, overlay_mode, ds, out_name, tolerance, progress):
    ...
    try:
        result = overlay_funs[_overlayMode](to_java_geometry_array(source_geos), to_java_geometry_array(overlay_geos),
                                            tolerance)
    except:
        import traceback
        log_error(traceback.format_exc())
        ...
        if result is None:
            ...
            for r in result:
                feature = Feature(Geometry._from_java_object(r.getGeometry()),
                                  [r.getSourceIndex(), r.getTargetIndex()], field_infos=field_infos)
                features.append(feature)
            dt = ds.write_features(features, out_name)
            return try_close_output_datasource(dt, ds)


def multilayer_overlay(inputs, overlay_mode, output_attribute_type='ONLYATTRIBUTES', tolerance=1.0e-10, out_data=None,
                       out_dataset_name='OverlayOutput', progress=None):
    """
    多图层叠加分析，支持多数据集或多记录集的叠加分析。只支持面数据集或面对象叠加分析。
    ...
    :param out_data: 结果数据保存的数据源。如果为空，则结果数据集保存到叠加分析源数据集所在的数据源。当 inputs 输入中全是几何对象数组时，必须
                     设置结果数据保存的数据源。
    :type out_data: Datasource or DatasourceConnectionInfo or str
    :param str out_dataset_name: 结果数据集名称
    :param function progress: 进度信息处理函数，具体参考 :py:class:`.StepEvent`
    ...
    if not isinstance(inputs, (list, tuple)):
        raise ValueError('inputs required list or tuple')
    overlay_mode = make_enum(OverlayMode, overlay_mode)
    if not (overlay_mode is OverlayMode.INTERSECT or overlay_mode is OverlayMode.UNION):
        raise ValueError('only support intersect and union, but now is ' + str(overlay_mode))
    output_attribute_type = make_enum(OverlayOutputAttributeType, output_attribute_type, 'ONLYATTRIBUTES')

    inputs = [get_input_dataset(value) for value in inputs]
    count_dt, count_rd, count_geos = 0, 0, 0
    for item in inputs:
        if isinstance(item, DatasetVector):
            count_dt += 1
    ...
    if count_rd > 0:
        for index, item in enumerate(inputs):
            if isinstance(item, DatasetVector):
                if not input_ds:
                    input_ds = item.datasource
                rd = item.get_recordset()
                need_close_rd.append(rd)
                inputs[index] = rd
            elif isinstance(item, (list, tuple)):
                if not memory_ds:
                    memory_ds = create_datasource(':memory:')
                if memory_ds:
                    dt = memory_ds.write_spatial_data(item, 'input_data')
                    rd = dt.get_recordset()
                    need_close_rd.append(rd)
                    inputs[index] = rd
            elif isinstance(item, Recordset):
                if not input_ds:
                    input_ds = item.datasource
        inputs = list(filter(lambda value: isinstance(value, Recordset), inputs))
        if len(inputs) < 2:
            raise ValueError('count of inputs is invalid, must be greater than 1')
        java_inputs = to_java_array(inputs, get_jvm().com.supermap.data.Recordset)
    else:
        for index, item in enumerate(inputs):
            if isinstance(item, (list, tuple)):
                ...
                if memory_ds:
                    dt = memory_ds.write_spatial_data(item, 'input_data')
                    inputs[index] = dt
            elif isinstance(item, DatasetVector):
                if not input_ds:
                    input_ds = item.datasource
        inputs = list(filter(lambda value: isinstance(value, DatasetVector), inputs))
        if len(inputs) < 2:
            raise ValueError('count of inputs is invalid, must be greater than 1')
        java_inputs = to_java_array(inputs, get_jvm().com.supermap.data.DatasetVector)

    if out_data is not None:
        out_datasource = get_output_datasource(out_data)
    else:
        out_datasource = input_ds
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        dest_name = 'MultilayerOverlay'
    else:
    ...
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'multilayer_overlay')
            _jvm.com.supermap.analyst.spatialanalyst.OverlayAnalyst.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
            listener = None
    try:
        if overlay_mode is OverlayMode.INTERSECT:
            java_result = _jvm.com.supermap.analyst.spatialanalyst.OverlayAnalyst.intersect(
                java_inputs, oj(output_attribute_type), float(tolerance), oj(out_datasource), str(dest_name))
        else:
            java_result = _jvm.com.supermap.analyst.spatialanalyst.OverlayAnalyst.union(
                java_inputs, oj(output_attribute_type), float(tolerance), oj(out_datasource), str(dest_name))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
...
class _DissolveParameter:
    ...
    def __init__(self, dissolve_type=None, fields=None, stats_fields=None, stats_types=None,
                 attr_filter=None, tolerance=1.0e-10, is_null_value_able=True, is_preprocess=True):
        """
        初始化融合参数对象
...
def dissolve(input_data, dissolve_type, dissolve_fields, field_stats=None, attr_filter=None,
             is_null_value_able=True, is_preprocess=True, tolerance=1.0e-10, out_data=None,
             out_dataset_name='DissolveResult', progress=None):
    """
    ...
    parameter = _DissolveParameter(). \
        set_dissolve_type(dissolve_type). \
        set_dissolve_fields(dissolve_fields). \
        set_statistics_field_names(stats_fields). \
        set_statistics_types(stats_types). \
        set_filter_string(attr_filter). \
        set_tolerance(tolerance). \
        set_null_value_able(is_null_value_able). \
        set_preprocess(is_preprocess)
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.Generalization.dissolve(_source_input._jobject,
                                                                                          _ds._jobject,
                                                                                          _outDatasetName,
                                                                                          _java_parameter)
    except Exception as e:
...
def aggregate_points(input_data, min_pile_point, distance, unit=None, class_field=None, out_data=None,
                     out_dataset_name='AggregateResult', progress=None):
    """
    ...
    if class_field is not None:
        ...
        if field_info is not None:
            if field_info.type not in (FieldType.INT16, FieldType.INT32, FieldType.INT64):
                raise ValueError(
                    'invalid class field type, required int16, int32 and int64, but is ' + str(field_info.type.name))
        else:
    ...
    try:
        is_success = _jvm.com.supermap.analyst.spatialanalyst.Generalization.aggregatePoints(
            oj(_source_input), float(distance), oj(Unit._make(unit)), min_pile_point, oj(_ds),
            _outDatasetName, class_field)
...
def resample_vector(input_data, distance, resample_type=VectorResampleType.RTBEND, is_preprocess=True,
                    tolerance=1.0e-10, is_save_small_geometry=False, out_data=None, out_dataset_name=None,
                    progress=None, is_save_topology=False):
    """
    ...
    try:
        ...
        is_success = _source_input._jobject.resample(resample_parameter, listener is not None,
                                                     bool(is_save_small_geometry))
...
def create_thiessen_polygons(input_data, clip_region, field_stats=None, out_data=None, out_dataset_name=None,
                             progress=None):
    """
    创建泰森多边形。
    ...
    elif isinstance(_source_input, (DatasetVector, Recordset)):
        pass
    else:
        raise ValueError(
            'Create thiessen polygon only support DatasetVector, Recordset and list points for input_data')
    ...
    if out_data is not None:
        ...
        try:
            if points is not None:
                java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.ProximityAnalyst. \
                    createThiessenPolygon(points, out_datasource._jobject, _outDatasetName,
                                          java_clip_region)
            else:
                if fields is None or statTypes is None:
                    java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.ProximityAnalyst. \
                        createThiessenPolygon(_source_input._jobject, out_datasource._jobject, _outDatasetName,
                                              java_clip_region)
                else:
                    java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.ProximityAnalyst. \
                        createThiessenPolygon(_source_input._jobject, out_datasource._jobject, _outDatasetName,
                                              java_clip_region, to_java_string_array(fields),
                                              to_java_stattype_array(statTypes))
        except Exception as e:
            log_error(e)
            java_result_dt = None
        finally:
        ...
        try:
            if points is not None:
                regions = _jvm.com.supermap.analyst.spatialanalyst.ProximityAnalyst. \
                    createThiessenPolygon(points, java_clip_region)
            else:
                regions = _jvm.com.supermap.analyst.spatialanalyst.ProximityAnalyst. \
                    createThiessenPolygon(_source_input._jobject, java_clip_region)
        except Exception as e:
            log_error(e)
            regions = None
        finally:
...
def summary_points(input_data, radius, unit=None, stats=None, is_random_save_point=False,
                   is_save_attrs=False, out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if stats is not None:
            if isinstance(stats, StatisticsField):
                ...
                raise ValueError(
                    'invalid stats type, required StatisticsField, (list,tuple,set of StatisticsField) or str')
        java_statFields = get_gateway().new_array(_jvm.com.supermap.analyst.spatialanalyst.StatisticsField,
                                                  len(statFields))
        i = 0
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst. \
            ProximityAnalyst.summaryPoints(_source_input._jobject,
                                           float(radius),
                                           Unit._make(unit, Unit.METER)._jobject,
                                           java_statFields,
                                           _ds._jobject,
                                           _outDatasetName,
                                           bool(is_random_save_point),
                                           bool(is_save_attrs))
    except Exception as e:
        log_error(e)
        java_result_dt = None
...
def clip_vector(input_data, clip_region, is_clip_in_region=True, is_erase_source=False, out_data=None,
                out_dataset_name=None, progress=None):
    """
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.VectorClip. \
            clipDatasetVector(_source_input._jobject,
                              clip_region._jobject, bool(is_clip_in_region),
                              bool(is_erase_source), _ds._jobject,
                              _outDatasetName)
    except Exception as e:
        log_error(e)
        java_result_dt = None
...
def simplify_building(source_data, width_threshold, height_threshold, save_failed=False, out_data=None,
    ...
    try:
        java_result_dt = get_jvm().com.supermap.jsuperpy.Utils.simplifyBuilding(oj(_source_input),
                                                                                oj(_ds),
                                                                                _outDatasetName,
...
def update_attributes(source_data, target_data, spatial_relation, update_fields, interval=1.0e-6):
    """
    ...
    try:
        result = get_jvm().com.supermap.jsuperpy.UpdateAttributes.updateAttributes(oj(_attribute_input),
                                                                                   oj(_updated_input),
                                                                                   oj(spatial_relation),
                                                                                   stat_fields,
                                                                                   stat_modes,
...
def resample_raster(input_data, new_cell_size, resample_mode, out_data=None, out_dataset_name=None,
                    progress=None):
    """
    栅格数据重采样，返回结果数据集。
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.GeneralizeAnalyst.resample(_source_input._jobject,
                                                                                             float(new_cell_size),
                                                                                             RasterResampleMode._make(
                                                                                                 resample_mode)._jobject,
                                                                                             _ds._jobject,
                                                                                             _outDatasetName)
    except Exception as e:
        log_error(e)
...
class ReclassSegment(object):
    ...
    def __init__(self, start_value=None, end_value=None, new_value=None, segment_type=None):
        ...
        self.set_start_value(start_value).set_end_value(end_value).set_new_value(new_value).set_segment_type(
            segment_type)
...
def _to_java_reclass_segment_array(values):
    if values is None:
        ...
        if isinstance(values, ReclassSegment):
            java_array = get_gateway().new_array(get_jvm().com.supermap.analyst.spatialanalyst.ReclassSegment,
                                                 1)
...
class ReclassMappingTable(object):
    ...
    def change_missing_value_to(self):
        ...
        return float(self._changeMissingValueTo) if self._changeMissingValueTo is not None else None
                ...
                for seg in segs:
                    ...
                    if len(items) == 4:
                        self._segments.append(
                            ReclassSegment(float(items[0]), float(items[1]), float(items[2]), str(items[3])))
                    else:
                        items = seg.split(' ')
                        if len(items) == 4:
                            self._segments.append(
    ...
    def _from_java_object(java_obj):
        ...
        if java_segments is not None:
            ...
            for java_seg in java_segments:
                segments.append(ReclassSegment(java_seg.getStartValue(), java_seg.getEndValue(),
                                               java_seg.getNewValue(), java_seg.getSegmentType().name()))
...
def reclass_grid(input_data, re_pixel_format, segments=None, reclass_type='UNIQUE', is_retain_no_value=True,
                 change_no_value_to=None, is_retain_missing_value=False, change_missing_value_to=None, reclass_map=None,
                 out_data=None, out_dataset_name=None, progress=None):
    """
    栅格数据重分级，返回结果栅格数据集。
    ...
    if reclass_map is not None:
        ...
        java_reclassMap = ReclassMappingTable().set_change_missing_value_to(
            change_missing_value_to).set_change_no_value_to(change_no_value_to). \
            set_reclass_type(reclass_type).set_retain_missing_value(is_retain_missing_value). \
            set_retain_no_value(is_retain_no_value).set_segments(segments)._jobject
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.GeneralizeAnalyst.reclass(_source_input._jobject,
                                                                                            java_reclassMap,
                                                                                            ReclassPixelFormat._make(
                                                                                                re_pixel_format)._jobject,
                                                                                            _ds._jobject,
                                                                                            _outDatasetName)
    except Exception as e:
        log_error(e)
...
def aggregate_grid(input_data, scale, aggregation_type, is_expanded, is_ignore_no_value, out_data=None,
                   out_dataset_name=None, progress=None):
    """
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.GeneralizeAnalyst.aggregate(_source_input._jobject,
                                                                                              int(scale),
                                                                                              AggregationType._make(
                                                                                                  aggregation_type)._jobject,
                                                                                              bool(is_expanded),
                                                                                              bool(is_ignore_no_value),
                                                                                              _ds._jobject,
                                                                                              _outDatasetName)
    except Exception as e:
        log_error(e)
...
def slice_grid(input_data, number_zones, base_output_zones, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.GeneralizeAnalyst.slice(_source_input._jobject,
                                                                                          _ds._jobject,
                                                                                          _outDatasetName,
                                                                                          int(number_zones),
                                                                                          int(base_output_zones))
    except Exception as e:
        log_error(e)
        java_result_dt = None
    finally:
...
def compute_range_raster(input_data, count, progress=None):
    ...
    :rtype: Array
    """
    warnings.warn('compute_range_raster will be removed in version 12. Please use compute_natural_breaks.',
                  DeprecationWarning)
    _source_input = get_input_dataset(input_data)
    if _source_input is None:
        raise ValueError('source input_data is None')
    ...
    try:
        java_result_arr = _jvm.com.supermap.analyst.spatialanalyst.GeneralizeAnalyst.ComputeRange(
            _source_input._jobject,
            int(count))
    except Exception as e:
        log_error(e)
...
def compute_range_vector(input_data, value_field, count, progress=None):
    ...
    :rtype: Array
    """
    warnings.warn('compute_range_vector will be removed in version 12. Please use compute_natural_breaks.',
                  DeprecationWarning)
...
def kernel_density(input_data, value_field, search_radius, resolution, bounds=None,
                   out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.DensityAnalyst.kernelDensity(param,
                                                                                               _source_input._jobject,
                                                                                               value_field,
                                                                                               _ds._jobject,
                                                                                               _outDatasetName)
...
def point_density(input_data, value_field, resolution, neighbour_shape, neighbour_unit='CELL', bounds=None,
                  out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if neighbour_shape is not None:
            if isinstance(neighbour_shape, NeighbourShape):
            ...
            elif isinstance(neighbour_shape, str):
                ...
                if len(tokens) > 1:
                    ...
                    if shapeType is NeighbourShapeType.RECTANGLE:
                        ...
                        if len(tokens) >= 4:
                            java_neighbourShape = NeighbourShapeWedge(float(tokens[1]),
                                                                      float(tokens[2]), float(tokens[3]))._jobject
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.DensityAnalyst.pointDensity(param,
                                                                                              _source_input._jobject,
                                                                                              value_field,
                                                                                              _ds._jobject,
                                                                                              _outDatasetName)
...
def clip_raster(input_data, clip_region, is_clip_in_region=True, is_exact_clip=False, out_data=None,
                out_dataset_name=None, progress=None):
    """
    ...
    if ext_name is not None and ext_name.lower() in set(['tif', 'tiff']):
        try:
            ...
            java_result = _jvm.com.supermap.analyst.spatialanalyst.RasterClip.clip(_source_input._jobject,
                                                                                   clip_region._jobject,
                                                                                   bool(is_clip_in_region),
                                                                                   targetFileName,
                                                                                   targetFileType._jobject)
        except Exception as e:
            log_error(e)
            java_result = False
        if java_result:
        ...
        _outDatasetName = _ds.get_available_dataset_name(_outDatasetName)
        try:
            java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.RasterClip.clip(_source_input._jobject,
                                                                                      clip_region._jobject,
                                                                                      bool(is_clip_in_region),
                                                                                      bool(is_exact_clip),
                                                                                      _ds._jobject,
...
def erase_and_replace_raster(input_data, replace_region, replace_value, out_data=None, out_dataset_name=None,
                             progress=None):
    """
    ...
    if source_input is None:
        raise ValueError('source input_data is None')
    if not isinstance(source_input, (DatasetGrid, DatasetImage)):
        raise ValueError('source input_data must be DatasetGrid or DatasetImage')
    _jvm = get_jvm()

    if isinstance(replace_region, Rectangle):
        replace_region = replace_region.to_region()

    if not isinstance(replace_region, GeoRegion):
        raise ValueError('replace region required GeoRegion, but now is ' + str(type(replace_region)))

    if out_data is not None:
        out_datasource = get_output_datasource(out_data)
    else:
        out_datasource = source_input.datasource
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        dest_name = source_input.name + '_replace'
    else:
        dest_name = out_dataset_name
    dest_name = out_datasource.get_available_dataset_name(dest_name)

    if isinstance(replace_value, (tuple, list)):
        replace_value = tuple_to_color(replace_value)
    listener = None
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'erase_and_replace_raster')
            _jvm.com.supermap.analyst.spatialanalyst.RasterClip.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
            listener = None
    try:
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.RasterClip.eraseAndReplace(oj(source_input),
                                                                                             oj(replace_region),
                                                                                             oj(out_datasource),
                                                                                             dest_name,
                                                                                             float(replace_value))
...
class InterpolationDensityParameter(InterpolationParameter):
    ...
    def __init__(self, resolution, search_radius=0.0, expected_count=12,
                 bounds=None):
        """
        构造点密度差值插值参数类对象
...
class InterpolationIDWParameter(InterpolationParameter):
    ...
    def __init__(self, resolution, search_mode=SearchMode.KDTREE_FIXED_COUNT, search_radius=0.0, expected_count=12,
                 power=1, bounds=None):
        """
        构造 IDW 插值参数类。
...
class InterpolationKrigingParameter(InterpolationParameter):
    ...
    def __init__(self, resolution, krighing_type=InterpolationAlgorithmType.KRIGING,
                 search_mode=SearchMode.KDTREE_FIXED_COUNT, search_radius=0.0, expected_count=12,
                 max_point_count_in_node=50, max_point_count_for_interpolation=200,
                 variogram=VariogramMode.SPHERICAL, angle=0.0, mean=0.0, exponent=Exponent.EXP1, nugget=0.0,
                 range_value=0.0, sill=0.0, bounds=None):
        """
        构造 克吕金插值参数对象。
    ...
    def _jobject(self):
        """Py4J 映射的 Java 对象"""
        java_obj = get_jvm().com.supermap.analyst.spatialanalyst.InterpolationKrigingParameter(
            self.type._jobject)
        if self.bounds is not None:
            java_obj.setBounds(self.bounds._jobject)
        if self.expected_count:
        ...
        if self.angle and self.search_mode != SearchMode.QUADTREE:
...
class InterpolationRBFParameter(InterpolationParameter):
    ...
    def __init__(self, resolution, search_mode=SearchMode.KDTREE_FIXED_COUNT, search_radius=0.0, expected_count=12,
                 max_point_count_in_node=50, max_point_count_for_interpolation=200, smooth=0.100000001490116,
                 tension=40, bounds=None):
...
def interpolate(input_data, parameter, z_value_field, pixel_format, z_value_scale=1.0, out_data=None,
                out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.Interpolator.interpolate(parameter._jobject,
                                                                                           _source_input._jobject,
                                                                                           str(z_value_field),
                                                                                           float(z_value_scale),
                                                                                           _ds._jobject,
...
def interpolate_points(points, values, parameter, pixel_format, prj, out_data, z_value_scale=1.0,
                       out_dataset_name=None, progress=None):
    """
    对点数组进行插值分析，并返回分析结果
...
def idw_interpolate(input_data, z_value_field, pixel_format, resolution, search_mode=SearchMode.KDTREE_FIXED_COUNT,
                    search_radius=0.0, expected_count=12, power=1, bounds=None,
                    z_value_scale=1.0, out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    return interpolate(input_data, parameter, z_value_field, pixel_format, z_value_scale, out_data, out_dataset_name,
                       progress)


def density_interpolate(input_data, z_value_field, pixel_format, resolution, search_radius=0.0, expected_count=12,
                        bounds=None, z_value_scale=1.0, out_data=None, out_dataset_name=None, progress=None):
    """
...
def kriging_interpolate(input_data, z_value_field, pixel_format, resolution, krighing_type='KRIGING',
                        search_mode=SearchMode.KDTREE_FIXED_COUNT,
                        search_radius=0.0, expected_count=12, max_point_count_in_node=50,
                        max_point_count_for_interpolation=200, variogram_mode=VariogramMode.SPHERICAL, angle=0.0,
                        mean=0.0, exponent=Exponent.EXP1, nugget=0.0, range_value=0.0, sill=0.0, bounds=None,
                        z_value_scale=1.0, out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    :rtype: DatasetGrid or str
    """
    parameter = InterpolationKrigingParameter(resolution, krighing_type, search_mode, search_radius, expected_count,
                                              max_point_count_in_node, max_point_count_for_interpolation,
                                              variogram_mode, angle, mean, exponent, nugget, range_value, sill, bounds)
    return interpolate(input_data, parameter, z_value_field, pixel_format, z_value_scale, out_data, out_dataset_name,
                       progress)


def rbf_interpolate(input_data, z_value_field, pixel_format, resolution, search_mode=SearchMode.KDTREE_FIXED_COUNT,
                    search_radius=0.0, expected_count=12, max_point_count_in_node=50,
                    max_point_count_for_interpolation=200, smooth=0.100000001490116, tension=40, bounds=None,
                    z_value_scale=1.0, out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    """
    parameter = InterpolationRBFParameter(resolution, search_mode, search_radius, expected_count,
                                          max_point_count_in_node,
                                          max_point_count_for_interpolation, smooth, tension, bounds)
    return interpolate(input_data, parameter, z_value_field, pixel_format, z_value_scale, out_data,
                       out_dataset_name, progress)


def vector_to_raster(input_data, value_field, clip_region=None, cell_size=None, pixel_format=PixelFormat.SINGLE,
                     out_data=None, out_dataset_name=None, progress=None, no_value=-9999, is_all_touched=True):
    """
    ...
    try:
        ...
        parameter.setNoValue(float(no_value))
        parameter.setAllTouched(bool(is_all_touched))
        listener = None
        if progress is not None and safe_start_callback_server():
            try:
...
def raster_to_vector(input_data, value_field, out_dataset_type=DatasetType.POINT, back_or_no_value=-9999,
                     back_or_no_value_tolerance=0.0, specifiedvalue=None, specifiedvalue_tolerance=0.0,
                     valid_region=None, is_thin_raster=True, smooth_method=None, smooth_degree=0.0,
                     out_data=None, out_dataset_name=None, progress=None):
    """
...
def cost_distance(input_data, cost_grid, max_distance=-1.0, cell_size=None, out_data=None, out_distance_grid_name=None,
                  out_direction_grid_name=None, out_allocation_grid_name=None, progress=None):
    """
    ...
    try:
        ...
        if listener is not None:
            try:
                ...
                log_error(e1)
            close_callback_server()
        if distance_analyst_result is not None:
            results = []
...
def cost_path(input_data, distance_dataset, direction_dataset, compute_type, out_data=None, out_dataset_name=None,
              progress=None):
    """
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.DistanceAnalyst.costPath(_source_input._jobject,
                                                                                           _source_distance._jobject,
                                                                                           _source_direction._jobject,
                                                                                           ComputeType._make(
                                                                                               compute_type)._jobject,
                                                                                           _ds._jobject,
                                                                                           _outDatasetName)
    except Exception as e:
        log_error(e)
...
def cost_path_line(source_point, target_point, cost_grid, smooth_method=None, smooth_degree=0, progress=None,
    ...
    try:
        ...
        memory_ds = None
        barrier_regions = get_input_dataset(barrier_regions)
        if barrier_regions:
            if isinstance(barrier_regions, (Rectangle, GeoRegion)):
                memory_ds = create_datasource(':memory:')
                barrier_regions = memory_ds.write_spatial_data([barrier_regions], 'barrier_regions')
            elif isinstance(barrier_regions, (list, tuple)):
                memory_ds = create_datasource(':memory:')
                barrier_regions = memory_ds.write_spatial_data(barrier_regions, 'barrier_regions')
            if not isinstance(barrier_regions, DatasetVector) or barrier_regions.type is not DatasetType.REGION:
                raise ValueError('barrier regions must be region dataset')
            parameter.setBarrierRegion(oj(barrier_regions))
        listener = None
        if progress is not None and safe_start_callback_server():
            try:
                ...
                log_error(e)
                listener = None
        java_result = _jvm.com.supermap.analyst.spatialanalyst.DistanceAnalyst.costPathLine(source_point._jobject,
                                                                                            target_point._jobject,
                                                                                            parameter)
    except Exception as e:
        log_error(e)
        java_result = None
        ...
        if memory_ds:
            memory_ds.close()
        if java_result is not None:
            return Geometry._from_java_object(java_result.getPathLine()), java_result.getCost()
        else:
            return None
...
def path_line(target_point, distance_dataset, direction_dataset, smooth_method=None, smooth_degree=0):
    ...
    _jvm = get_jvm()
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.DistanceAnalyst.pathLine(target_point._jobject,
                                                                                        _source_distance._jobject,
                                                                                        _source_direction._jobject,
                                                                                        SmoothMethod._make(
                                                                                            smooth_method)._jobject,
                                                                                        int(smooth_degree))
    except Exception as e:
        log_error(e)
        java_result = None
...
def straight_distance(input_data, max_distance=-1.0, cell_size=None, out_data=None, out_distance_grid_name=None,
                      out_direction_grid_name=None, out_allocation_grid_name=None, progress=None):
    """
...
def surface_distance(input_data, surface_grid_dataset, max_distance=-1.0, cell_size=None, max_upslope_degrees=90.0,
                     max_downslope_degree=90.0, out_data=None, out_distance_grid_name=None,
                     out_direction_grid_name=None, out_allocation_grid_name=None, progress=None):
    """
...
def surface_path_line(source_point, target_point, surface_grid_dataset, max_upslope_degrees=90.0,
                      max_downslope_degree=90.0, smooth_method=None, smooth_degree=0, progress=None,
                      barrier_regions=None):
    """
    ...
    try:
        ...
        if progress is not None and safe_start_callback_server():
            try:
                ...
                log_error(e)
                listener = None
        java_result = _jvm.com.supermap.analyst.spatialanalyst.DistanceAnalyst.surfacePathLine(source_point._jobject,
                                                                                               target_point._jobject,
                                                                                               parameter)
    except Exception as e:
        log_error(e)
...
def calculate_hill_shade(input_data, shadow_mode, azimuth, altitude_angle, z_factor, out_data=None,
                         out_dataset_name=None, progress=None):
    """
    ...
    >>> ds = open_datasource('/home/data/example_data.udbx')
    >>> calculate_hill_shade(ds['DEM'], ShadowMode.IllUMINATION, 315, 45, 0.001)
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.calculateHillShade(
            _source_input._jobject,
            ShadowMode._make(shadow_mode)._jobject,
            float(azimuth),
...
def calculate_slope(input_data, slope_type, z_factor, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.calculateSlope(
            _source_input._jobject,
            SlopeType._make(slope_type)._jobject,
            float(z_factor),
            _ds._jobject,
            _outDatasetName)
...
def calculate_aspect(input_data, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.calculateAspect(
            _source_input._jobject, _ds._jobject, _outDatasetName)
...
def compute_point_aspect(input_data, specified_point):
    ...
    _jvm = get_jvm()
    try:
        return _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.computePointAspect(
            _source_input._jobject, specified_point._jobject)
...
def compute_point_slope(input_data, specified_point, slope_type, z_factor):
    ...
    _jvm = get_jvm()
    try:
        return _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.computePointSlope(
            _source_input._jobject, specified_point._jobject, SlopeType._make(slope_type)._jobject, float(z_factor))
...
def calculate_ortho_image(input_data, colors, no_value_color, out_data=None, out_dataset_name=None, progress=None):
    ...
    >>> colors = Colors.make_gradient(32, ColorGradientType.YELLOWGREEN)
    >>> calculate_ortho_image(ds['DEM'], colors, 'white')
    ...
    _outDatasetName = _ds.get_available_dataset_name(_outDatasetName)
    _jvm = get_jvm()
    no_value_color = Color.make(no_value_color)
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.calculateOrthoImage(
            oj(_source_input), java_color, to_java_color(no_value_color), oj(_ds), _outDatasetName)
    except Exception as e:
        log_error(e)
        java_result_dt = None
    finally:
...
def compute_surface_area(input_data, region):
    ...
    _jvm = get_jvm()
    try:
        return _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.computeSurfaceArea(
            oj(_source_input), oj(region))
...
def compute_surface_distance(input_data, line):
    ...
    _jvm = get_jvm()
    try:
        return _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.computeSurfaceDistance(
            oj(_source_input), oj(line))
...
def compute_surface_volume(input_data, region, base_value):
    ...
    _jvm = get_jvm()
    try:
        return _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.computeSurfaceVolume(
            oj(_source_input), oj(region), float(base_value))
    except Exception as e:
        log_error(e)
    return None
...
def calculate_profile(input_data, line):
    ...
    * 剖面线和采样点坐标集合
    ...
    _jvm = get_jvm()
    try:
        profile_result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.calculateProfile(
            oj(_source_input), oj(line))
        if profile_result:
            return (Geometry._from_java_object(profile_result.getProfile()),
...
class CutFillResult:
    ...
    def _from_java_object(java_object, grid_result):
        return CutFillResult(java_object.getCutArea(),
                             java_object.getCutVolume(),
                             java_object.getFillArea(),
                             java_object.getFillVolume(),
                             java_object.getRemainderArea(),
                             grid_result)
...
def inverse_cut_fill(input_data, volume, is_fill, region=None, progress=None):
    ...
    try:
        ...
        if isinstance(region, GeoRegion):
            result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.cutFill(
                oj(_source_input), float(volume), bool(is_fill), oj(region))
        else:
            result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.cutFill(
                oj(_source_input), float(volume), bool(is_fill))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        result = None
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                log_error(e)
                listener = None
        cut_fill_result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.cutFill(
            oj(_before_source_input), oj(_after_source_input), oj(_ds), _outDatasetName)
    except Exception as e:
        log_error(e)
...
def cut_fill_oblique(input_data, line3d, buffer_radius, is_round_head, out_data=None, out_dataset_name=None,
                     progress=None):
    """
    斜面填挖方计算。
    ...
    try:
        ...
        cut_fill_result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.cutFill(
            oj(source_input), oj(line3d), float(buffer_radius), bool(is_round_head), oj(_ds), _outDatasetName)
    except Exception as e:
        log_error(e)
        cut_fill_result = None
    finally:
...
def cut_fill_region(input_data, region, base_altitude, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        cut_fill_result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.cutFill(
            oj(source_input), oj(region), float(base_altitude), oj(_ds), _outDatasetName)
    except Exception as e:
        log_error(e)
        cut_fill_result = None
    finally:
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                listener = None
        cut_fill_result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.cutFill(
            oj(source_input), oj(region), oj(_ds), _outDatasetName)
    except Exception as e:
        log_error(e)
        cut_fill_result = None
    finally:
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                log_error(e)
                listener = None
        java_result = _jvm.com.supermap.analyst.spatialanalyst.CalculationTerrain.flood(
            oj(_source_input), float(height), oj(region))
    except Exception as e:
        log_error(e)
        java_result = None
    finally:
...
def divide_math_analyst(first_operand, second_operand, user_region=None, out_data=None, out_dataset_name=None,
                        progress=None):
    """
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.MathAnalyst.divide(_first_input._jobject,
                                                                                     _second_input._jobject,
                                                                                     javaRegion,
                                                                                     _ds._jobject,
                                                                                     _outDatasetName)
    except Exception as e:
...
def plus_math_analyst(first_operand, second_operand, user_region=None, out_data=None, out_dataset_name=None,
                      progress=None):
    """
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.MathAnalyst.plus(_first_input._jobject,
                                                                                   _second_input._jobject,
                                                                                   javaRegion,
                                                                                   _ds._jobject,
                                                                                   _outDatasetName)
...
def minus_math_analyst(first_operand, second_operand, user_region=None, out_data=None, out_dataset_name=None,
                       progress=None):
    """
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.MathAnalyst.minus(_first_input._jobject,
                                                                                    _second_input._jobject,
                                                                                    javaRegion,
                                                                                    _ds._jobject,
                                                                                    _outDatasetName)
...
def multiply_math_analyst(first_operand, second_operand, user_region=None, out_data=None, out_dataset_name=None,
                          progress=None):
    """
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.MathAnalyst.multiply(_first_input._jobject,
                                                                                       _second_input._jobject,
                                                                                       javaRegion,
                                                                                       _ds._jobject,
                                                                                       _outDatasetName)
...
def to_float_math_analyst(input_data, user_region=None, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.MathAnalyst.toFloat(_first_input._jobject,
                                                                                      javaRegion,
                                                                                      _ds._jobject,
                                                                                      _outDatasetName)
    except Exception as e:
...
def to_int_math_analyst(input_data, user_region=None, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.MathAnalyst.toInt(_first_input._jobject,
                                                                                    javaRegion,
                                                                                    _ds._jobject,
                                                                                    _outDatasetName)
    except Exception as e:
...
def expression_math_analyst(expression, pixel_format, out_data, is_ingore_no_value=True,
                            user_region=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if user_region is not None:
            ...
            javaRegion = None
        java_result_dt = _jvm.com.supermap.analyst.spatialanalyst.MathAnalyst.execute(expression,
                                                                                      javaRegion,
                                                                                      oj(PixelFormat._make(
                                                                                          pixel_format)),
                                                                                      False,
                                                                                      is_ingore_no_value,
                                                                                      oj(out_datasource),
...
def compute_min_distance(source, reference, min_distance, max_distance, out_data=None, out_dataset_name=None,
                         progress=None):
    """
    ...
    * 注意事项：
    ...
    if isinstance(source, DatasetVector):
    ...
    elif isinstance(source, Recordset):
        source_rd = source
        ...
        raise ValueError('source required DatasetVector or Recordset, but now is ' + str(type(source)))
    ...
    if isinstance(reference, DatasetVector):
    ...
    elif isinstance(reference, Recordset):
        ...
        raise ValueError('reference required DatasetVector or Recordset, but now is ' + str(type(reference)))
    ...
    try:
        ...
        success = computeMinDistance(oj(source_rd), oj(reference_rd), float(min_distance), float(max_distance),
                                     oj(out_datasource), _outDatasetName)
    except:
        import traceback
        log_error(traceback.format_exc())
        success = False
...
def compute_range_distance(source, reference, min_distance, max_distance, out_data=None, out_dataset_name=None,
                           progress=None):
    """
    ...
    try:
        ...
        success = computeRangeDistance(oj(source_rd), oj(reference_rd), float(min_distance), float(max_distance),
                                       oj(out_datasource), _outDatasetName)
    except:
        import traceback
        log_error(traceback.format_exc())
        ...
        if listener is not None:
            try:
                ...
                log_error(e1)
            close_callback_server()
        if isinstance(source, DatasetVector) and source_rd is not None:
            source_rd.close()
            del source_rd
        if isinstance(reference, DatasetVector) and reference_rd is not None:
            reference_rd.close()
            del reference_rd
        if success:
            result_dt = out_datasource[_outDatasetName]
...
def integrate(source, tolerance, unit=None, progress=None):
    ...
    :rtype: bool
    """
    warnings.warn('integrate will be removed in version 12. Please use integrate in topo model.',
                  DeprecationWarning)
    try:
...
class _EliminateMode(JEnum):
    ...
    @classmethod
    def _externals(cls):
        return {'AREA': _EliminateMode.ELIMINATE_BY_AREA,
                'BORDER': _EliminateMode.ELIMINATE_BY_BORDER}


def eliminate(source, region_tolerance, vertex_tolerance, is_delete_single_region=False, progress=None,
              group_fields=None, priority_fields=None):
    """
    ...
    try:
        ...
        if group_fields:
            success = eliminate(oj(source_dt), to_java_string_array(group_fields),
                                to_java_string_array(priority_fields), float(region_tolerance),
                                float(vertex_tolerance), oj(eliminate_mode), bool(is_delete_single_region))
        else:
            success = eliminate(oj(source_dt), float(region_tolerance), float(vertex_tolerance), oj(eliminate_mode),
                                bool(is_delete_single_region))
    except:
        import traceback
        log_error(traceback.format_exc())
        success = False
...
def eliminate_specified_regions(source, small_region_ids, vertex_tolerance, exclude_region_ids=None, group_fields=None,
                                priority_fields=None, is_max_border=False, progress=None):
    """
    ...
    try:
        ...
        group_fields = split_input_list_from_str(group_fields)
        priority_fields = split_input_list_from_str(priority_fields)
        success = eliminateSpecifiedRegions(oj(source_dt), to_java_int_array(small_region_ids),
                                            to_java_int_array(exclude_region_ids), to_java_string_array(group_fields),
                                            to_java_string_array(priority_fields), parse_bool(is_max_border),
                                            float(vertex_tolerance))
    except:
        import traceback
        log_error(traceback.format_exc())
        success = False
...
def edge_match(source, target, edge_match_mode, tolerance=None, is_union=False, edge_match_line=None, out_data=None,
               out_dataset_name=None, progress=None):
    """
...
def region_to_center_line(region_data, out_data=None, out_dataset_name=None, progress=None):
    ...
    elif isinstance(region_data, DatasetVector):
...
def dual_line_to_center_line(source_line, max_width, min_width, out_data=None, out_dataset_name=None, progress=None):
    ...
    elif isinstance(source_line, DatasetVector):
        source_rd = source_line.get_recordset(False, 'STATIC')
    elif isinstance(source_line, str):
    ...
    try:
        ...
        java_result_dt = dualLineToCenterLine(oj(source_rd), float(max_width), float(min_width), oj(out_datasource),
                                              _outDatasetName)
    except:
        import traceback
        log_error(traceback.format_exc())
...
def _get_surface_extract_parameter(datum_value=0.0, interval=0.0, resample_tolerance=0.0,
                                   smooth_method='BSPLINE', smoothness=0, expected_z_values=None):
...
def grid_extract_isoline(extracted_grid, interval, datum_value=0.0, expected_z_values=None, resample_tolerance=0.0,
                         smooth_method='BSPLINE', smoothness=0, clip_region=None,
                         out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if progress is not None and safe_start_callback_server():
            try:
                ...
                log_error(e)
                listener = None
        java_param = _get_surface_extract_parameter(datum_value, interval, resample_tolerance, smooth_method,
                                                    smoothness, expected_z_values)
...
def point_extract_isoline(extracted_point, z_value_field, resolution, interval, terrain_interpolate_type=None,
                          datum_value=0.0, expected_z_values=None, resample_tolerance=0.0,
                          smooth_method='BSPLINE', smoothness=0, clip_region=None,
                          out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if out_datasource is not None:
            if terrain_interpolate_type is not None:
                ...
                java_result = extractIsoline(java_param, oj(source_dt), str(z_value_field),
                                             oj(terrain_interpolate_type),
                                             oj(out_datasource), _outDatasetName, float(resolution),
                                             java_clip_region)
            else:
                java_result = extractIsoline(java_param, oj(source_dt), str(z_value_field), oj(out_datasource),
                                             _outDatasetName, float(resolution), java_clip_region)
        else:
            if terrain_interpolate_type is not None:
                terrain_interpolate_type = TerrainInterpolateType._make(terrain_interpolate_type)
                java_result = extractIsoline(java_param, oj(source_dt), str(z_value_field),
                                             oj(terrain_interpolate_type),
                                             float(resolution), java_clip_region)
            else:
                java_result = extractIsoline(java_param, oj(source_dt), str(z_value_field),
                                             float(resolution), java_clip_region)
...
def point3ds_extract_isoline(extracted_points, resolution, interval, terrain_interpolate_type=None,
                             datum_value=0.0, expected_z_values=None, resample_tolerance=0.0,
                             smooth_method='BSPLINE',
                             smoothness=0, clip_region=None, out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if out_datasource is not None:
            if terrain_interpolate_type is not None:
                terrain_interpolate_type = TerrainInterpolateType._make(terrain_interpolate_type)
                java_result = extractIsoline(java_param, java_point_3ds,
                                             oj(terrain_interpolate_type),
                                             oj(out_datasource), _outDatasetName, float(resolution),
                                             java_clip_region)
            else:
                java_result = extractIsoline(java_param, java_point_3ds, oj(out_datasource),
                                             _outDatasetName, float(resolution), java_clip_region)
        else:
            if terrain_interpolate_type is not None:
                terrain_interpolate_type = TerrainInterpolateType._make(terrain_interpolate_type)
...
def grid_extract_isoregion(extracted_grid, interval, datum_value=0.0, expected_z_values=None,
                           resample_tolerance=0.0,
                           smooth_method='BSPLINE', smoothness=0, clip_region=None,
                           out_data=None, out_dataset_name=None, progress=None):
    """
    用于从栅格数据集中提取等值面。
    ...
    * 通过 expected_z_values 方法指定一个 Z 值的集合，则只提取高程为集合中值的等值面。例如，高程范围为0-1000的 DEM 栅格数据，
    ...
    try:
        ...
        if out_datasource is not None:
            java_result = extractIsoregion(java_param, oj(source_dt), oj(out_datasource), _outDatasetName,
                                           java_clip_region)
        else:
...
def points_extract_isoregion(extracted_point, z_value_field, interval, resolution=None, terrain_interpolate_type=None,
                             datum_value=0.0, expected_z_values=None, resample_tolerance=0.0,
                             smooth_method='BSPLINE', smoothness=0, clip_region=None, out_data=None,
                             out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if out_datasource is not None:
            if terrain_interpolate_type is not None:
                terrain_interpolate_type = TerrainInterpolateType._make(terrain_interpolate_type)
                java_result = extractIsoregion(java_param, oj(source_dt), str(z_value_field),
                                               oj(terrain_interpolate_type),
                                               oj(out_datasource), _outDatasetName, float(resolution),
                                               java_clip_region)
            else:
                java_result = extractIsoregion(java_param, oj(source_dt), str(z_value_field), oj(out_datasource),
                                               _outDatasetName, float(resolution), java_clip_region)
        else:
            if terrain_interpolate_type is not None:
                terrain_interpolate_type = TerrainInterpolateType._make(terrain_interpolate_type)
...
def point3ds_extract_isoregion(extracted_points, resolution, interval, terrain_interpolate_type=None,
                               datum_value=0.0, expected_z_values=None, resample_tolerance=0.0,
                               smooth_method='BSPLINE', smoothness=0, clip_region=None,
                               out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if out_datasource is not None:
            if terrain_interpolate_type is not None:
                terrain_interpolate_type = TerrainInterpolateType._make(terrain_interpolate_type)
                java_result = extractIsoRegion(java_param, java_point_3ds,
                                               oj(terrain_interpolate_type),
                                               oj(out_datasource), _outDatasetName, float(resolution),
                                               java_clip_region)
            else:
                java_result = extractIsoRegion(java_param, java_point_3ds, oj(out_datasource),
                                               _outDatasetName, float(resolution), java_clip_region)
        else:
            if terrain_interpolate_type is not None:
                terrain_interpolate_type = TerrainInterpolateType._make(terrain_interpolate_type)
...
def grid_common_statistics(grid_data, compare_datasets_or_value, compare_type, is_ignore_no_value, out_data=None,
                           out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        if isinstance(compare_datasets_or_value, (float, int)):
            ...
            if isinstance(compare_datasets_or_value, DatasetGrid):
                compare_value = [compare_datasets_or_value]
            elif isinstance(compare_datasets_or_value, str):
                compare_value = []
            ...
            elif isinstance(compare_datasets_or_value, list):
                compare_value = []
        ...
        if isinstance(compare_value, float):
            java_result = func_commonStatistics(oj(source_dt), compare_value,
                                                oj(StatisticsCompareType._make(compare_type)),
                                                bool(is_ignore_no_value), oj(out_datasource), _outDatasetName)
        else:
            java_dts = to_java_datasetgrid_array(compare_value)
            java_result = func_commonStatistics(oj(source_dt), java_dts, oj(StatisticsCompareType._make(compare_type)),
                                                bool(is_ignore_no_value), oj(out_datasource), _outDatasetName)
...
def grid_neighbour_statistics(grid_data, neighbour_shape, is_ignore_no_value=True, grid_stat_mode='SUM',
                              unit_type='CELL', out_data=None, out_dataset_name=None, progress=None):
    """
    栅格邻域统计分析。
    ...
    * 邻域统计还用于图像处理，如统计邻域内的平均值（称为均值滤波）或中位数（称为中值滤波）可以达到平滑的效果，从而去除噪声或过多的细节，等等。
    ...
    try:
        ...
        elif neighbour_shape.shape_type is NeighbourShapeType.ANNULUS:
        ...
        elif neighbour_shape.shape_type is NeighbourShapeType.CIRCLE:
        ...
        elif neighbour_shape.shape_type is NeighbourShapeType.WEDGE:
...
def altitude_statistics(point_data, grid_data, out_data=None, out_dataset_name=None):
    ...
    if not isinstance(point_dt, DatasetVector) or point_dt.type is not DatasetType.POINT:
    ...
    _outDatasetName = out_datasource.get_available_dataset_name(_outDatasetName)
    try:
        java_result = get_jvm().com.supermap.jsuperpy.Utils.AltitudeStatistics(oj(point_dt),
                                                                               oj(grid_data),
                                                                               oj(out_datasource),
                                                                               _outDatasetName)
    except:
...
def zonal_statistics_on_raster_value(value_data, zonal_data, zonal_field, is_ignore_no_value=True, grid_stat_mode='SUM',
                                     out_data=None, out_dataset_name=None, out_table_name=None, progress=None):
    """
...
class GridHistogram:
    ...
    def get_segments(self):
        ...
        result = []
        for seg in segments:
            result.append(GridHistogram.HistogramSegmentInfo(seg.getCount(),
                                                             seg.getMaxValue(),
                                                             seg.getMinValue(),
                                                             seg.getRangeMaxValue(),
                                                             seg.getRangeMinValue()))
        del segments
        return result
...
def thin_raster(source, back_or_no_value, back_or_no_value_tolerance, out_data=None, out_dataset_name=None,
                progress=None):
    """
    ...
    * 当对影像数据集进行细化时，栅格值为指定的值的单元格被视为背景色，不参与细化；同时，在 back_or_no_value_tolerance 参数指
    ...
    :param back_or_no_value: 指定栅格的背景色或表示无值的值。可以使用一个 int 或 Color 来表示一个 RGB 或 RGBA 值。
                             输入的数据集为24位或32位影像数据集时才支持使用 Color 类型的 RGB 或 RGB 值。
    :type back_or_no_value: int or Color
    :param back_or_no_value_tolerance: 栅格背景色的容限或无值的容限。可以使用一个 int 或 Color 来表示一个 RGB 或 RGBA 值。
                            输入的数据集为24位或32位影像数据集时才支持使用 Color 类型的 RGB 或 RGB 值。
    :type back_or_no_value_tolerance: float or Color
    :param out_data: 用于存储结果数据的数据源。
    :type out_data: Datasource or DatasourceConnectionInfo or str
    :param out_dataset_name: 结果数据集的名称
    :type out_dataset_name: str
    ...
    # c++ 层根据像素格式限制了 back_or_no_value 值的大小，所以，对于非 RGB 和 RGBA 的数据集都不能支持设置 RGB值。
    if back_or_no_value is not None:
        if isinstance(back_or_no_value, tuple):
            if source_dt.get_pixel_format(0) is PixelFormat.RGB or source_dt.get_pixel_format(0) is PixelFormat.RGBA:
                back_or_no_value = tuple_to_color(back_or_no_value)
            else:
                raise ValueError('back_or_no_value is tuple type and only support RGB or RGBA Image Dataset')
    ...
    if back_or_no_value_tolerance is not None:
        if isinstance(back_or_no_value_tolerance, tuple):
            if source_dt.get_pixel_format(0) is PixelFormat.RGB or source_dt.get_pixel_format(0) is PixelFormat.RGBA:
                ...
                raise ValueError('back_or_no_value_tolerance is tuple type and only support RGB or RGBA Image Dataset')
    ...
    try:
        ...
        java_result = thinRaster(oj(source_dt), int(back_or_no_value), float(back_or_no_value_tolerance),
                                 oj(out_datasource), _outDatasetName)
...
def thin_raster_bit(input_data, back_or_no_value, is_save_as_grid=True, out_data=None, out_dataset_name=None,
                    progress=None):
    """
    ...
    try:
        ...
        thinRaster = get_jvm().com.supermap.analyst.spatialanalyst.ConversionAnalyst.thinRaster
        java_result = thinRaster(oj(source_dt), oj(out_datasource), _outDatasetName,
                                 int(back_or_no_value), parse_bool(is_save_as_grid))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def build_terrain(source_datas, lake_dataset=None, lake_altitude_field=None, clip_data=None, erase_data=None,
                  interpolate_type='IDW', resample_len=0.0, z_factor=1.0, is_process_flat_area=False,
                  encode_type='NONE', pixel_format='SINGLE', cell_size=0.0, out_data=None, out_dataset_name=None,
                  progress=None):
    """
    根据指定的地形构建参数信息创建地形。
    ...
    if lake_dataset is not None and lake_altitude_field is not None:
    ...
    try:
        ...
        java_result = get_jvm().com.supermap.analyst.spatialanalyst.TerrainBuilder.buildTerrain(java_param,
                                                                                                oj(out_datasource),
                                                                                                _outDatasetName)
        del java_param
    except:
...
def area_solar_radiation_days(grid_data, latitude, start_day, end_day=160, hour_start=0, hour_end=24, day_interval=5,
                              hour_interval=0.5, transmittance=0.5, z_factor=1.0,
                              out_data=None, out_total_grid_name='TotalGrid', out_direct_grid_name=None,
                              out_diffuse_grid_name=None, out_duration_grid_name=None, progress=None):
    """
    ...
    try:
        ...
        java_result = areaSolarRadiation(oj(source_dt), java_param, oj(out_datasource), _out_total_grid_name,
                                         out_direct_grid_name, out_diffuse_grid_name, out_duration_grid_name)
        ...
        if java_result is None:
            ...
            if out_data is not None:
                return try_close_output_datasource((total_grid_dt, direct_grid_dt, diffuse_grid_dt, duration_grid_dt),
                                                   out_datasource)
            return total_grid_dt, direct_grid_dt, diffuse_grid_dt, duration_grid_dt


def area_solar_radiation_hours(grid_data, latitude, day, hour_start=0, hour_end=24, hour_interval=0.5,
                               transmittance=0.5, z_factor=1.0, out_data=None, out_total_grid_name='TotalGrid',
                               out_direct_grid_name=None, out_diffuse_grid_name=None, out_duration_grid_name=None,
                               progress=None):
    """
...
def raster_mosaic(inputs, back_or_no_value, back_tolerance, join_method, join_pixel_format, cell_size,
                  encode_type='NONE', valid_rect=None, out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                ...
                get_jvm().com.supermap.analyst.spatialanalyst.RasterMosaic.addSteppedListener(
                    listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
                listener = None
        java_result = get_jvm().com.supermap.analyst.spatialanalyst.RasterMosaic.mosaic(to_java_dataset_array(datasets),
                                                                                        float(back_or_no_value),
                                                                                        float(back_tolerance),
                                                                                        java_join_method,
                                                                                        java_join_pixel_format,
                                                                                        float(cell_size),
                                                                                        java_encode_type,
                                                                                        oj(valid_rect),
                                                                                        oj(out_datasource),
...
def compute_features_envelope(input_data, is_single_part=True, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.Generalization.featureEnvelope(
            oj(_source_input), _outDatasetName, oj(out_datasource), bool(is_single_part))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def calculate_view_shed(input_data, view_point, start_angle, view_angle, view_radius, out_data=None,
                        out_dataset_name=None, progress=None):
    """
    ...
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.VisibilityAnalyst.calculateViewShed(
            oj(_source_input), oj(view_point), float(start_angle), float(view_angle),
            float(view_radius), oj(out_datasource), _outDatasetName)
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def calculate_view_sheds(input_data, view_points, start_angles, view_angles, view_radiuses, view_shed_type,
                         out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    if (len(view_points) != len(start_angles)) or (len(view_points) != len(view_angles)) or (
            len(view_points) != len(view_radiuses)):
    ...
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.VisibilityAnalyst.calculateViewShed(
            oj(_source_input), to_java_point3ds(view_points), to_java_double_array(start_angles),
            to_java_double_array(view_angles),
            to_java_double_array(view_radiuses), oj(out_datasource), _outDatasetName, oj(view_shed_type))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def is_point_visible(input_data, from_point, to_point):
    ...
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.VisibilityAnalyst.isVisible(
            oj(_source_input), oj(from_point), oj(to_point))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def are_points_visible(input_data, from_points, to_points):
    ...
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.VisibilityAnalyst.isVisible(oj(_source_input),
...
def line_of_sight(input_data, from_point, to_point):
    ...
    to_point = Point3D.make(to_point)
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.VisibilityAnalyst.lineOfSight(
            oj(_source_input), oj(from_point), oj(to_point))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
...
def radar_shield_angle(input_data, view_point, start_angle, end_angle, view_radius, interval, out_data=None,
                       out_dataset_name=None, progress=None):
    """
    ...
    try:
        ...
        java_result = _jvm.com.supermap.analyst.spatialanalyst.VisibilityAnalyst.radarShieldAngle(
            java_dt, oj(view_point), float(start_angle), float(end_angle),
            float(view_radius), oj(out_datasource), _outDatasetName, float(interval))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def majority_filter(source_grid, neighbour_number_method, majority_definition, out_data=None, out_dataset_name=None,
                    progress=None):
    """
    众数滤波，返回结果栅格数据集。
    ...
    try:
        ...
        java_result = majorityFilter(oj(source_dt), oj(out_datasource), _outDatasetName, oj(neighbour_number_method),
                                     oj(majority_definition))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def expand(source_grid, neighbour_number_method, cell_number, zone_values, out_data=None, out_dataset_name=None,
           progress=None):
    """
    扩展，返回结果栅格数据集。
    ...
    try:
        ...
        java_result = expand(oj(source_dt), oj(out_datasource), _outDatasetName, oj(neighbour_number_method),
                             int(cell_number), to_java_int_array(split_input_int_list_from_str(zone_values)))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def shrink(source_grid, neighbour_number_method, cell_number, zone_values, out_data=None, out_dataset_name=None,
           progress=None):
    """
    收缩，返回结果栅格数据集。
    ...
    try:
        ...
        java_result = shrink(oj(source_dt), oj(out_datasource), _outDatasetName, oj(neighbour_number_method),
                             int(cell_number), to_java_int_array(split_input_list_from_str(zone_values)))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def region_group(source_grid, neighbour_number_method, is_save_link_value, is_link_by_neighbour=False,
                 exclude_value=None, out_data=None, out_dataset_name=None, progress=None):
    """
    区域分组。
    ...
    try:
        ...
        if exclude_value is None:
            java_result = regionGroup(oj(source_dt), oj(out_datasource), _outDatasetName, oj(neighbour_number_method),
                                      bool(is_save_link_value))
        else:
            java_result = regionGroup(oj(source_dt), oj(out_datasource), _outDatasetName, oj(neighbour_number_method),
                                      bool(is_save_link_value), bool(is_link_by_neighbour), int(exclude_value))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def nibble(source_grid, mask_grid, zone_grid, is_mask_no_value, is_nibble_no_value, out_data=None,
           out_dataset_name=None, progress=None):
    """
    蚕食，返回结果栅格数据集。
    ...
    if mask_dt is not None and not isinstance(mask_dt, DatasetGrid):
        raise ValueError('source required DatasetGrid, but is ' + str(type(mask_grid)))
    zone_dt = get_input_dataset(zone_grid)
    if zone_dt is not None and not isinstance(zone_dt, DatasetGrid):
    ...
    try:
        ...
        java_result = nibble(oj(source_dt), oj(mask_dt), oj(zone_dt), oj(out_datasource), _outDatasetName,
                             bool(is_mask_no_value), bool(is_nibble_no_value))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def boundary_clean(source_grid, sort_type, is_run_two_times, out_data=None, out_dataset_name=None,
                   progress=None):
    """
    边界清理，返回结果栅格数据集。
    ...
    try:
        ...
        java_result = boundaryClean(oj(source_dt), oj(out_datasource), _outDatasetName,
                                    oj(sort_type), bool(is_run_two_times))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
class CellularAutomataParameter:
    ...
    def set_spatial_variable_grids(self, spatial_variable_grids):
        ...
        elif isinstance(self.spatial_variable_grids, (DatasetGrid, str)):
            self._spatial_variable_grids = [get_input_dataset(spatial_variable_grids)]
        return self
...
class CellularAutomataFlushedListener(PythonListenerBase):
    ...
    class Java:
        implements = ["com.supermap.analyst.spatialanalyst.CAFlushedListener"]
...
class PCACellularAutomata(JVMBase):
    ...
    def pca(self, spatial_variable_grids, sample_count, component_radio, progress_func=None):
        ...
        if isinstance(spatial_variable_grids, (DatasetGrid, str)):
            spatial_variable_grids = [spatial_variable_grids]
        ...
        listener = None
        try:
            if progress_func is not None and safe_start_callback_server():
                try:
                    ...
                    log_error(e)
                    listener = None
            java_result = self._jobject.pca(to_java_datasetgrid_array(spatial_variable_grids),
                                            int(sample_count), float(component_radio))
        except:
            import traceback
            log_error(traceback.format_exc())
            java_result = None
    ...
    def pca_cellular_automata(self, parameter, out_data=None, out_dataset_name=None, progress_func=None,
                              flush_func=None):
        """
        基于主成分分析的元胞自动机。
        ...
        listener = None
        try:
            if progress_func is not None or flush_func is not None and safe_start_callback_server():
...
class MCECellularAutomataParameter:
    """
    基于多准则判断的元胞自动机参数类。
    """

    def __init__(self):
        self._is_check_result = False
        self._end_cell_grid = None
        self._ahp_comparison_matrix = None
        self._global_value = 0.5
        self._local_value = 0.5
        self._alpha = 2
        self._conversion_rules = None
        self._conversion_target = None
        self._ca_parameter = None
    ...
    def set_check_result(self, value):
        ...
        """
        if value:
            self._is_check_result = bool(value)
        else:
            self._is_check_result = False
        return self
    ...
    def set_end_cell_grid(self, value):
        ...
        :rtype: MCECellularAutomataParameter
        """
        value = get_input_dataset(value)
        if isinstance(value, DatasetGrid):
            self._end_cell_grid = value
        return self
    ...
    def set_global_value(self, value):
        ...
        """
        if value:
            self._global_value = float(value)
        return self
    ...
    def set_local_value(self, value):
        ...
        """
        if value:
            self._local_value = float(value)
        return self

    @staticmethod
    def check_ahp_consistent(ahp_comparison_matrix):
        """
        层次分析法的一致性检验。
        ...
        :return: 成功返回权重数组，失败返回 None
        :rtype: list[float]
        """
        matrix = MCECellularAutomataParameter._convert_ahp_matrix(ahp_comparison_matrix)
        if matrix is None:
            raise ValueError('ahp_comparison_matrix is invalid')
        mce = get_jvm().com.supermap.analyst.spatialanalyst.MCECellularAutomata()
        return java_array_to_list(mce.checkAHPConsistent(to_java_2d_double_array(matrix)))

    @staticmethod
    def _convert_ahp_matrix(value):
        if isinstance(value, (list, tuple)):
            count = -1
            for item in value:
                if isinstance(item, (list, tuple)):
                    if count < 0:
                        ...
                        raise ValueError("value require float matrix.")
                else:
                    raise ValueError("value require float matrix.")
            return value
        else:
    ...
    @property
    def ahp_comparison_matrix(self):
        """list[list[float]]: 层次分析法一致性检验的判断矩阵"""
        return self._ahp_comparison_matrix

    def set_ahp_comparison_matrix(self, value):
        """
        设置层次分析法一致性检验的判断矩阵。
        ...
        :rtype: MCECellularAutomataParameter
        """
        self._ahp_comparison_matrix = MCECellularAutomataParameter._convert_ahp_matrix(value)
        return self
    ...
    @property
    def _jobject(self):
        java_object = get_jvm().com.supermap.analyst.spatialanalyst.MCECellularAutomataParameter()
        ...
        if self._conversion_rules:
            ...
            for item in self._conversion_rules.keys():
                rules[int(item)] = bool(self._conversion_rules[item])
            java_object.setConversionRules(rules)
        java_object.setCheckResult(bool(self.is_check_result))
        if self.ahp_comparison_matrix:
            java_object.setAHPComparisonMatrix(to_java_2d_double_array(self.ahp_comparison_matrix))
        if self.end_cell_grid:
            java_object.setEndCellGrid(oj(self.end_cell_grid))
        java_object.setGlobalValue(self.global_value)
        java_object.setLocalValue(self.local_value)
        return java_object


class MCECellularAutomata(JVMBase):
    """
    基于多准则判断的元胞自动机。
    """
    ...
    def _make_java_object(self):
        self._java_object = get_jvm().com.supermap.analyst.spatialanalyst.MCECellularAutomata()
        return self._java_object
    ...
    def get_kappa(self):
        ...
        :rtype: float
        """
        return self._java_object.getKappa()

    def mce_cellular_automata(self, parameter, out_data=None, out_dataset_name=None, progress_func=None,
                              flush_func=None):
        """
        基于多准则判断的元胞自动机。
        ...
        :rtype: DatasetGrid
        """
        if not isinstance(parameter, MCECellularAutomataParameter):
            raise ValueError('parameter must be MCECellularAutomataParameter')
        ...
        check_output_datasource(out_datasource)
        if out_dataset_name is None:
            _dest_name = source_cell_grid.name + '_mce_ca'
        else:
            _dest_name = out_dataset_name
        _dest_name = out_datasource.get_available_dataset_name(_dest_name)
        ...
        try:
            if progress_func is not None or flush_func is not None and safe_start_callback_server():
                if progress_func is not None:
                    try:
                        listener = ProgressListener(progress_func, 'MCECA')
                        self._jobject.addSteppedListener(listener)
                    except Exception as e:
                        close_callback_server()
                        log_error(e)
                ...
                if flush_func is not None:
                    try:
                        flushed_listener = CellularAutomataFlushedListener(flush_func, 'MCECA')
                        self._jobject.addCAFlushedListener(flushed_listener)
                    except Exception as e:
                        close_callback_server()
                        log_error(e)
            ...
            java_result = self._jobject.mceCellularAutomata(oj(out_datasource), _dest_name, oj(parameter))
        except:
            import traceback
            log_error(traceback.format_exc())
            java_result = None
...
class ANNParameter:
    ...
    def __init__(self, is_custom_neighborhood=False, neighborhood_number=7, custom_neighborhoods=None,
                 learning_rate=0.2, sample_count=1000):
        """
        初始化对象
        ...
        self.set_custom_neighborhood(is_custom_neighborhood).set_neighborhood_number(
            neighborhood_number).set_custom_neighborhoods(custom_neighborhoods).set_learning_rate(
            learning_rate).set_sample_count(sample_count)
...
class ANNCellularAutomata(JVMBase):
    ...
    def initialize_ann(self, train_start_cell_grid, train_end_cell_grid, ann_train_values,
                       spatial_variable_grids, ann_parameter):
        """
        初始化基于人工神经网络的元胞自动机
        ...
        return self._jobject.initializeAnn(oj(train_start_cell_grid), oj(train_end_cell_grid),
                                           to_java_int_array(ann_train_values),
                                           to_java_datasetgrid_array(spatial_variable_grids),
                                           oj(ann_parameter))
    ...
    def ann_cellular_automata(self, parameter, out_data=None, out_dataset_name=None, progress_func=None,
                              flush_func=None):
        """
        基于人工神经网络的元胞自动机。
        ...
        try:
            ...
            if java_result:
                return ANNCellularAutomataResult(java_array_to_list(java_result.getConvertValues()),
                                                 java_array_to_list(java_result.getAccuracys()),
                                                 result_dt)
            return None


def _create_geometry_random_points(geo, random_num, min_distance=None, clip_bounds=None):
    if not isinstance(geo, (GeoLine, GeoRegion)):
        raise ValueError('geometry required (GeoRegion, GeoLine), but now is ' + str(type(geo)))
    if min_distance:
        min_distance = float(min_distance)
    else:
        min_distance = 0.0
    if clip_bounds is not None:
        clip_bounds = Rectangle.make(clip_bounds)
        if not clip_bounds.is_empty():
            from iobjectspy import intersect
            geo = intersect(geo, clip_bounds)
    points = get_jvm().com.supermap.data.Geometrist.createRandomPoints(oj(geo), int(random_num),
                                                                       float(min_distance))
    if points:
        return [Point2D._from_java_object(java_obj) for java_obj in java_array_to_list(points)]
    return None


def _create_dataset_random_points(dataset, random_number, min_distance=None, clip_bounds=None,
                                  out_data=None, out_dataset_name=None, progress=None):
    if not isinstance(dataset, DatasetVector):
        raise ValueError('dataset required DatasetVector, but now is ' + str(type(dataset)))
    parameter = get_jvm().com.supermap.analyst.spatialanalyst.RandomPointsParameter()
    if random_number:
        parameter.setRandomNumber(random_number)
    if min_distance:
        parameter.setMinDistance(min_distance)
    if clip_bounds:
        clip_bounds = Rectangle.make(clip_bounds)
        parameter.setRectBounds(oj(clip_bounds))

    if out_data is not None:
        out_datasource = get_output_datasource(out_data)
    else:
        out_datasource = dataset.datasource
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        _dest_name = dataset.name + '_random_points'
    else:
        _dest_name = out_dataset_name
    _dest_name = out_datasource.get_available_dataset_name(_dest_name)
    ...
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'create_dataset_random_points')
            _jvm.com.supermap.analyst.spatialanalyst.Generalization.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
            listener = None
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.Generalization.createRandomPoints(
            oj(dataset), parameter, oj(out_datasource), str(_dest_name))
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def create_random_points(dataset_or_geo, random_number, min_distance=None, clip_bounds=None,
                         out_data=None, out_dataset_name=None, progress=None):
    """
    在几何对象内随机生成点。生成随机点可以指定结果随机点数目和随机点距离，当同时指定随机点数目和最小距离时，优先满足最小距离，即生成的随机点之间的距
    离一定大于最小距离，当数目可能少于随机点数目。
    ...
    :param dataset_or_geo: 用于创建随机点的几何对象或数据集。当指定为单个几何对象时，支持线和面类型几何对象。当指定为数据集时，支持点、线和面类
                           型的数据集。
    :type dataset_or_geo: GeoRegion or GeoLine or Rectangle or DatasetVector or str
    :param random_number: 随机点数目或随机点数目所在的字段名称。只有在数据集中生成随机点才能指定为字段名称。
    :type random_number: str or float
    :param min_distance: 随机点最小距离或最小距离所在的字段名称。当随机点距离值为 None 或 0 时，则生成的随机点不考虑两点之间距离限制。当大于
                         0时，任意两个随机点之间的距离必须大于指定的距离，此时，生成的随机点数目可能不一定等于指定的随机点数目。只有在数据集中
                         生成随机点才能指定为字段名称。
    ...
    if isinstance(dataset_or_geo, Rectangle):
        dataset_or_geo = dataset_or_geo.to_region()

    if isinstance(dataset_or_geo, Geometry):
        return _create_geometry_random_points(dataset_or_geo, random_number, min_distance, clip_bounds)

    dataset = get_input_dataset(dataset_or_geo)
    return _create_dataset_random_points(dataset, random_number, min_distance, clip_bounds,
                                         out_data, out_dataset_name, progress)


def _create_regularize_building_footprint_parameter(offset_distance, offset_distance_unit=None,
                                                    regularize_method='RIGHTANGLE', min_area=0.0, min_hole_area=0.0):
    param = get_jvm().com.supermap.data.RegularizeParameter()
    if offset_distance:
        param.setOffsetDistance(float(offset_distance))
        offset_distance_unit = make_enum(Unit, offset_distance_unit)
        param.setOffsetDistanceUnit(oj(offset_distance_unit))
    param.setRegularizeMethod(oj(make_enum(RegularizeMethod, regularize_method, 'RIGHTANGLE')))
    if min_area:
        param.setMinArea(float(min_area))
    if min_hole_area:
        param.setMinHoleArea(float(min_hole_area))
    return param


def _regularize_geometry_building_footprint(geometry, offset_distance, offset_distance_unit=None,
                                            regularize_method='RIGHTANGLE', min_area=0.0, min_hole_area=0.0,
                                            prj=None):
    if not isinstance(geometry, GeoRegion):
        raise ValueError('geometry required GeoRegion, but now is ' + str(type(geometry)))

    prj = PrjCoordSys.make(prj)
    offset_distance_unit = make_enum(Unit, offset_distance_unit)
    if prj and offset_distance_unit is None:
        offset_distance_unit = prj.coord_unit
    parameter = _create_regularize_building_footprint_parameter(offset_distance, offset_distance_unit,
                                                                regularize_method, min_area, min_hole_area)
    result_geo = get_jvm().com.supermap.data.Geometrist.regularizeBuildingFootprint(oj(geometry), parameter, oj(prj))
    if result_geo:
        return Geometry._from_java_object(result_geo)
    return None


def _regularize_dataset_building_footprint(dataset, offset_distance, offset_distance_unit=None,
                                           regularize_method='RIGHTANGLE', min_area=0.0, min_hole_area=0.0,
                                           is_attribute_retained=False,
                                           out_data=None, out_dataset_name=None, progress=None):
    if not isinstance(dataset, DatasetVector):
        raise ValueError('dataset required DatasetVector, but now is ' + str(type(dataset)))

    prj = dataset.prj_coordsys
    offset_distance_unit = make_enum(Unit, offset_distance_unit)
    if prj and offset_distance_unit is None:
        offset_distance_unit = prj.coord_unit
    parameter = _create_regularize_building_footprint_parameter(offset_distance, offset_distance_unit,
    ...
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        _dest_name = dataset.name + '_regularize'
    else:
        _dest_name = out_dataset_name
    _dest_name = out_datasource.get_available_dataset_name(_dest_name)
    ...
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'regularize_dataset_building_footprint')
            _jvm.com.supermap.analyst.spatialanalyst.Generalization.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
            listener = None
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.Generalization.regularizeBuildingFootprint(
            oj(dataset), oj(out_datasource), str(_dest_name), parameter, bool(is_attribute_retained), None)
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def regularize_building_footprint(dataset_or_geo, offset_distance, offset_distance_unit=None,
                                  regularize_method=RegularizeMethod.ANYANGLE, min_area=0.0, min_hole_area=0.0,
                                  prj=None, is_attribute_retained=False,
                                  out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    :param float offset_distance: 规则化对象可从其原始对象的边界偏移的最大距离。通过 .py:attr:`.offset_distance_unit` 可以设置基于
                                  数据坐标系的线性单位值，也可以设置距离单位值。
    ...
    :param float min_area: 规则化对象的最小面积，小于该面积的对象将被删除。当值大于0时才有效。当空间坐标系或数据集的坐标系为投影或经纬度时，面
                           积单位为平方米，当坐标系为 None 或为平面坐标系时，面积单位与数据单位相对应。
    :param float min_hole_area: 规则化对象中洞的最小面积，小于该面积的洞将被删除。当值大于0时才有效。当空间坐标系或数据集的坐标系为投影或经纬
                                度时，面积单位为平方米，当坐标系为 None 或为平面坐标系时，面积单位与数据单位相对应
    ...
    :param str out_dataset_name: 结果数据集名称
    :param function progress: 进度信息处理函数，具体参考 :py:class:`.StepEvent`
    :return: 结果规则化面对象或面数据集。输入为面对象的，生成的结果也为面对象。输入为面数据集时，生成的结果也为面数据集，生成的面数据集中将会生成一
            个 status 字段，status 字段值为 0 时表示规则化失败，保存的对象为原始对象。status 字段值为 1 时表示规则化成功，保存的对象为规则化
            对象。
    ...
    if isinstance(dataset_or_geo, Geometry):
        return _regularize_geometry_building_footprint(dataset_or_geo, offset_distance, offset_distance_unit,
                                                       regularize_method, min_area, min_hole_area, prj)
    dataset = get_input_dataset(dataset_or_geo)
    return _regularize_dataset_building_footprint(dataset, offset_distance, offset_distance_unit,
                                                  regularize_method, min_area, min_hole_area, is_attribute_retained,
                                                  out_data, out_dataset_name, progress)


def auto_compute_project_points(point_input, line_input, max_distance,
                                out_data=None, out_dataset_name=None, progress=None):
    """
    自动计算点到线的垂足。
    ...
    :rtype: DatasetVector or str
    """
    point_input = get_input_dataset(point_input)
    line_input = get_input_dataset(line_input)
    if point_input is None:
        raise ValueError('point input_data is None')
    if line_input is None:
        raise ValueError('line input_data is None')
    point_is_dt = False
    if isinstance(point_input, DatasetVector):
        point_input = point_input.get_recordset(False, 'STATIC')
        point_is_dt = True
    line_is_dt = False
    if isinstance(line_input, DatasetVector):
        line_input = line_input.get_recordset(False, 'STATIC')
        line_is_dt = True

    if out_data is not None:
        out_datasource = get_output_datasource(out_data)
    else:
        out_datasource = line_input.datasource
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        _dest_name = line_input.dataset.name + '_projects'
    else:
        _dest_name = out_dataset_name
    _dest_name = out_datasource.get_available_dataset_name(_dest_name)

    memory_ds = create_datasource(':memory:')
    temp_dest_name = 'PrjPoint'

    if not max_distance:
        max_distance = 1.0e-10
    else:
    ...
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'auto_compute_project_point')
            _jvm.com.supermap.analyst.spatialanalyst.ProximityAnalyst.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
            listener = None
    try:
        java_result = _jvm.com.supermap.analyst.spatialanalyst.ProximityAnalyst.autoComputeProjectPoint(
            oj(point_input), oj(line_input), float(max_distance), oj(memory_ds), str(temp_dest_name))
        if java_result:
            temp_projects_table = memory_ds[temp_dest_name]
            if temp_projects_table:
                result_dt = out_datasource.create_vector_dataset(_dest_name, DatasetType.POINT)
                fields = []
                for field in temp_projects_table.field_infos:
                    if not field.is_system_field():
                        new_field = field.clone()
                        if new_field.name == 'ProjectPoint_X':
                            new_field.set_name('SourcePoint_X').set_caption('SourcePoint_X')
                        elif new_field.name == 'ProjectPoint_Y':
                            new_field.set_name('SourcePoint_Y').set_caption('SourcePoint_Y')
                        fields.append(new_field)

                result_dt.create_fields(fields)
                result_dt_recordset = result_dt.get_recordset()
                result_dt_recordset.batch_edit()
                features = temp_projects_table.get_features()
                feature_dict = {}
                point_id_field_name = point_input.dataset.name + '_ID'
                line_id_field_name = line_input.dataset.name + '_ID'
                for feature in features:
                    feature_dict[feature[point_id_field_name]] = feature
                point_input.move_first()
                for point in point_input:
                    point_id = point.geometry.id
                    if point_id in feature_dict:
                        project_info = feature_dict[point_id]
                        project_point = GeoPoint(
                            Point2D(project_info['ProjectPoint_X'], project_info['ProjectPoint_Y']))
                        result_dt_recordset.add(project_point)
                        result_dt_recordset.set_value(point_id_field_name, point_id)
                        result_dt_recordset.set_value(line_id_field_name, project_info[line_id_field_name])
                        result_dt_recordset.set_value('Distance', project_info['Distance'])
                        result_dt_recordset.set_value('SegmentIndex', project_info['SegmentIndex'])
                        result_dt_recordset.set_value('SourcePoint_X', point.geometry.get_x())
                        result_dt_recordset.set_value('SourcePoint_Y', point.geometry.get_y())
                        result_dt_recordset.set_value('Angle', project_info['Angle'])
                        result_dt_recordset.set_value(point_id_field_name, point_id)
                result_dt_recordset.batch_update()
                result_dt_recordset.close()
            else:
                result_dt = None
        else:
            result_dt = None
        ...
        if result_dt:
            result_dt.set_prj_coordsys(line_input.dataset.prj_coordsys)
        memory_ds.close()
    except Exception:
        import traceback
        log_error(traceback.format_exc())
...
def compute_natural_breaks(input_dataset_or_values, number_zones, value_field=None):
    """
    ...
    input_dataset_or_values = get_input_dataset(input_dataset_or_values)
    instance = get_jvm().com.supermap.analyst.spatialanalyst.Classification()
    if isinstance(input_dataset_or_values, DatasetGrid):
        java_results = instance.computeNaturalBreaks(oj(input_dataset_or_values), int(number_zones))
    elif isinstance(input_dataset_or_values, DatasetVector):
        if not value_field or input_dataset_or_values.index_of_field(value_field) == -1:
            raise ValueError('input data is DatasetVector, must have valid value field')
        java_results = instance.computeNaturalBreaks(oj(input_dataset_or_values), str(value_field), int(number_zones))
    else:
        values = split_input_list_from_str(input_dataset_or_values)
        java_results = instance.computeNaturalBreaks(to_java_double_array(values), int(number_zones))
    return java_array_to_list(java_results)


def tabulate_area(class_dataset, zone_dataset, class_field=None, zone_field=None, resolution=None,
                  out_data=None, out_dataset_name=None, progress=None):
    """
    ...
    :param class_dataset: 待进行面积统计的类别数据集，支持栅格和点、线、面数据集。建议优先使用栅格数据集，若使用点或线数据集，则将输出与要素相
                          交的区域
    ...
    :param float resolution: 分辨率。区域制表结果汇总了各区域各类别的面积情况，该面积是用各类别栅格数和分辨率计算得到，因此，分辨率直接影响面积
                             统计结果。用户根据需求及期望设置分辨率，如果设置的分辨率与区域数据或类型数据的分辨率不同，内部会重采样为设置的分辨
                             率，使得耗时增加。若没有分辨率要求，建议可以不设置该值，即使用默认分辨率。
                             未设置该值时，默认分辨率由区域数据集确定：
                             - 如果区域数据集为栅格，默认分辨率与区域栅格分辨率相同
    ...
    :rtype: DatasetVector
    """
    class_dataset = get_input_dataset(class_dataset)
    if not isinstance(class_dataset, (DatasetVector, DatasetGrid)):
        raise ValueError('class_dataset required DatasetVector or DatasetGrid')
    zone_dataset = get_input_dataset(zone_dataset)
    if not isinstance(zone_dataset, (DatasetVector, DatasetGrid)):
        raise ValueError('zone_dataset required DatasetVector or DatasetGrid')

    if isinstance(class_dataset, DatasetVector):
        if not class_field:
            raise ValueError('class_field is invalid')

    if isinstance(zone_dataset, DatasetVector):
        if not zone_field:
            raise ValueError('zone_field is invalid')

    if out_data is not None:
        out_datasource = get_output_datasource(out_data)
    else:
        out_datasource = class_dataset.datasource
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        dest_name = class_dataset.name + '_stats'
    else:
        dest_name = out_dataset_name
    dest_name = out_datasource.get_available_dataset_name(dest_name)

    param = get_jvm().com.supermap.analyst.spatialanalyst.TabulateParameter()
    param.setClassDataset(oj(class_dataset))
    param.setZoneDataset(oj(zone_dataset))
    if class_field:
        param.setClassFieldName(str(class_field))
    if zone_field:
        param.setZoneFieldName(str(zone_field))
    if resolution:
        param.setResolution(float(resolution))
    param.setTargetDatasource(oj(out_datasource))
    param.setTargetDatasetName(dest_name)
    instance = get_jvm().com.supermap.analyst.spatialanalyst.ZonalAnalyst()
    listener = None
    if progress is not None and safe_start_callback_server():
        try:
            listener = ProgressListener(progress, 'tabulate_area')
            instance.addSteppedListener(listener)
        except Exception as e:
            close_callback_server()
            log_error(e)
            listener = None
    try:
        java_result = instance.tabulateArea(param)
    except Exception:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
        ...
        if listener is not None:
            try:
                instance.removeSteppedListener(listener)
            except Exception as e1:
                log_error(e1)
            close_callback_server()
        if java_result is not None:
