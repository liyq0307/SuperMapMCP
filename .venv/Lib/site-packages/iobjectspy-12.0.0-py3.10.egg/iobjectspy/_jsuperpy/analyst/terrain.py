...
__all__ = ['basin', 'build_quad_mesh', 'fill_sink', 'flow_accumulation', 'flow_direction',
...
def build_quad_mesh(quad_mesh_region, left_bottom, left_top, right_bottom, right_top, cols=0, rows=0,
                    out_col_field=None, out_row_field=None, out_data=None, out_dataset_name=None, progress=None):
    """
    对单个简单面对象进行网格剖分。
...
def flow_direction(surface_grid, force_flow_at_edge, out_data=None, out_dataset_name=None, out_drop_grid_name=None,
                   progress=None):
    """
...
def flow_length(direction_grid, up_stream, weight_grid=None, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result = flowLength(oj(direction_dt), oj(weight_dt), parse_bool(up_stream), oj(out_datasource),
                                 _outDatasetName)
    except:
        import traceback
        log_error(traceback.format_exc())
...
def pour_points(direction_grid, accumulation_grid, area_limit, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result = pourPoints(oj(direction_dt), oj(accumulation_dt), int(area_limit), oj(out_datasource),
                                 _outDatasetName)
    except:
        import traceback
        log_error(traceback.format_exc())
...
def stream_order(stream_grid, direction_grid, order_type, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result = streamOrder(oj(stream_dt), oj(direction_dt), oj(stream_order_type), oj(out_datasource),
                                  _outDatasetName)
    except:
        import traceback
        log_error(traceback.format_exc())
...
def stream_to_line(stream_grid, direction_grid, order_type, out_data=None, out_dataset_name=None, progress=None):
    ...
    try:
        ...
        java_result = streamToLine(oj(stream_dt), oj(direction_dt), oj(out_datasource),
                                   _outDatasetName, oj(stream_order_type))
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
...
def snap_pour_point(pour_point_or_grid, accumulation_grid, snap_distance, pour_point_field=None, out_data=None,
    ...
    :param str pour_point_field: 用于为汇水点位置赋值的字段。当汇水点数据集为点数据集时，需指定汇水点栅格值字段。字段类型仅支持整型，如果非整型会
                                 强制转为整型。
    :param out_data: 用于存储结果数据集的数据源
    :type out_data: DatasourceConnectionInfo or Datasource or str
    :param str out_dataset_name: 结果数据集名称。
    ...
    :rtype: DatasetGrid or str
    """
    pour_point_or_grid = get_input_dataset(pour_point_or_grid)
    if not isinstance(pour_point_or_grid, (DatasetVector, DatasetGrid)):
        raise ValueError('pour_point_or_grid required DatasetVector point or DatasetGrid.')
    if isinstance(pour_point_or_grid, DatasetVector):
        if pour_point_or_grid.type is not DatasetType.POINT:
            raise ValueError('pour_point_or_grid required dataset point or grid.')
        if pour_point_field is None or pour_point_or_grid.index_of_field(pour_point_field) == -1:
            raise ValueError('pour_point_or_grid is DatasetVector, required valid pour_point_field')

    accumulation_grid = get_input_dataset(accumulation_grid)
    if not isinstance(accumulation_grid, DatasetGrid):
        raise ValueError('accumulation_grid require DatasetGrid')

    if out_data is not None:
        out_datasource = get_output_datasource(out_data)
    else:
        out_datasource = accumulation_grid.datasource
    check_output_datasource(out_datasource)
    if out_dataset_name is None:
        dest_name = accumulation_grid.name + '_snap'
    else:
    ...
    try:
        if progress is not None and safe_start_callback_server():
            try:
                listener = ProgressListener(progress, 'snap_pour_point')
                get_jvm().com.supermap.analyst.terrainanalyst.HydrologyAnalyst.addSteppedListener(listener)
            except Exception as e:
                close_callback_server()
                log_error(e)
        ...
        snapPourPoint = get_jvm().com.supermap.analyst.terrainanalyst.HydrologyAnalyst.snapPourPoint
        java_result = snapPourPoint(oj(pour_point_or_grid), oj(accumulation_grid), float(snap_distance),
                                    pour_point_field, oj(out_datasource), dest_name)
    except:
        import traceback
        log_error(traceback.format_exc())
        java_result = None
