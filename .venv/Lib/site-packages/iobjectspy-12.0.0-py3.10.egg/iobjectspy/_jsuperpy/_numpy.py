...
from .data import (DatasetVector,
                   DatasetGridInfo,
                   Rectangle,
                   DatasetImageInfo,
                   DatasetGrid,
...
__all__ = ['recordset_to_numpy_array', 'datasetvector_to_numpy_array', 'numpy_array_to_datasetvector',
...
def _get_default_null_value(fieldInfo):
    _default_null = {FieldType.BOOLEAN: None,
                     ...
                     FieldType.INT16: -32767 - 1,
                     FieldType.INT32: -2147483647 - 1,
                     FieldType.INT64: -9223372036854775807 - 1,
...
def recordset_to_numpy_array(recordset, fields=None, export_spatial=False, skip_null_value=True, null_values=None):
    ...
    if recordset.get_record_count() is 0:
        return None
    ...
    if datasetType not in [DatasetType.POINT, DatasetType.LINE, DatasetType.REGION, DatasetType.TABULAR]:
        raise ValueError(
            'invalid dataset type, support point, line, region and tabular dataset, but now is ' + str(datasetType))
    ...
    if fields is not None:
        ...
        for i in range(fieldCount):
            ...
            if fieldInfo is not None and not fieldInfo.is_system_field():
    ...
    if export_spatial and datasetType is not DatasetType.TABULAR:
        ...
        elif datasetType is DatasetType.REGION:
    ...
    while i < record_count:
        ...
        if export_spatial:
            ...
            if geo is not None:
                ...
                elif datasetType is DatasetType.LINE:
        ...
        if valuesMaxSize is None:
            ...
            while k < len(dataTypes) - 1:
                ...
                if dataTypes[k][1] in ('U', 'S'):
                    try:
                        if values[k] is not None and len(values[k]) > valuesMaxSize[k]:
...
def datasetvector_to_numpy_array(source, attr_filter=None, fields=None, export_spatial=False, skip_null_value=True,
    ...
    if isinstance(dt, DatasetVector):
        ...
        narray = recordset_to_numpy_array(rd, fields, export_spatial, skip_null_value=skip_null_value,
...
def numpy_array_to_datasetvector(narray, output, out_dataset_name=None, x_col=None, y_col=None):
    ...
    is_point = False
    if x_col in names and narray.dtype[x_col].type in \
            (numpy.short, numpy.int32, numpy.int64, numpy.single, numpy.double,
             numpy.ushort, numpy.uint32, numpy.uint64, numpy.float_,
             numpy.float16, numpy.float32, numpy.float64) and \
            y_col in names and narray.dtype[y_col].type in (
            numpy.short, numpy.int32, numpy.int64, numpy.single, numpy.double,
            numpy.ushort, numpy.uint32, numpy.uint64, numpy.float_,
    ...
    for i in range(len(narray)):
        ...
        for fieldIndex, valueIndex in indexes.items():
            ...
            rd.set_value(fieldInfos[fieldIndex].name,
                         _to_sm_value(value, narray.dtype[array_field_name].type))
...
def datasetraster_to_numpy_array(source, no_value=None, origin_is_left_up=True):
    ...
    if height * width > 5000 * 5000 and is_use_rasterio:
        ...
        if not origin_is_left_up:
            ...
            if result_array.ndim == 3:
                for k in range(result_array.shape[0]):
                    result_array[k][rows, :] = array_copy[k][rows_r, :]
            else:
                result_array[rows, :] = result_array.copy()[rows_r, :]
        ...
        if isinstance(dt, DatasetGrid):
            ...
            if no_value is not None and dt_no_value != no_value:
            ...
            if bands == 1 and pixel is PixelFormat.RGB:
                if no_value is not None:
                ...
                return result_array
            elif bands == 1 and pixel is PixelFormat.RGBA:
                if no_value is not None:
                    rgb = color_to_tuple(dt_no_value)
                    _rgb = color_to_tuple(no_value)
                    if rgb[0] != _rgb[0]:
                ...
                if no_value is not None and dt_no_value != no_value:
                    result_array[result_array == dt_no_value] = no_value
                if bands == 1 and result_array.ndim == 3:
                    result_array = result_array[0]
                return result_array
    else:
        res = []
        ...
        read_thread.start()
        get_jvm().com.supermap.jsuperpy.DatasetUtils.ReadDatasetToPb(oj(dt), origin_is_left_up, _null_value,
...
def numpy_array_to_datasetraster(narray, x_resolution, y_resolution, output, x_start=0, y_start=0,
    ...
    if as_grid and bands > 1:
        log_warning(
            'The NumPy array cannot be written to the grid dataset because the grid dataset does not support Multi-Band.'
            'Instead of writing to the image dataset')
        as_grid = False
    ...
    bounds = Rectangle(left=x_start, top=y_start + rows * y_resolution, right=x_start + cols * x_resolution,
                       bottom=y_start)
    ...
    if narray.ndim == 2:
        for row in range(rows):
            row_data = _write_floats_to_stream(narray[row][0:], no_value)
        ...
        if as_grid:
            for row in range(rows):
                if origin_is_left_up:
                    ...
                    row_index = rows - row - 1
                row_data = _write_floats_to_stream(narray[0][row][0:], no_value)
                SetRows(dt._jobject, row_index, row_data)
        else:
            for band in range(bands):
                for row in range(rows):
                    if origin_is_left_up:
                        ...
                        row_index = rows - row - 1
                    row_data = _write_floats_to_stream(narray[band][row][0:], no_value)
