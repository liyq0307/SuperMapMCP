...
__all__ = ['JEnum', 'PixelFormat', 'BlockSizeOption', 'AreaUnit', 'Unit', 'EngineType', 'DatasetType', 'FieldType',
           ...
           'NeighbourNumber', 'MajorityDefinition', 'BoundaryCleanSortType', 'OverlayAnalystOutputType', 'FieldSign',
           'PyramidResampleType', 'DividePolygonOrientation', 'DividePolygonType', 'RegularizeMethod',
           'OverlayOutputAttributeType', 'TimeDistanceUnit', 'GJBLayerType',
           'CSGNodeType', 'CSGEntityType', 'CSGBooleanType', 'PromptSampleType']


class JEnum(IntEnum):
    """枚举值类型，提供根据名称和枚举值构造枚举项的接口。"""
    ...
    def _to_java_enum_type(self):
        ...
        if java_class_full_name is not None:
            try:
                ...
                elif super_class_name == 'java.lang.Enum':
...
class GeometryType(JEnum):
    ...
    :var GeometryType.GEOMODEL3D: 三维模型几何对象。
...
class BufferRadiusUnit(JEnum):
    ...
    @classmethod
    def _externals(cls):
        return {'m': BufferRadiusUnit.METER,
                'cm': BufferRadiusUnit.CENTIMETER,
                'mm': BufferRadiusUnit.MILIMETER,
                'dm': BufferRadiusUnit.DECIMETER,
                'km': BufferRadiusUnit.KILOMETER}
...
class StatisticsFieldType(JEnum):
    ...
    @classmethod
    def _externals(cls):
        return {'mean': StatisticsFieldType.AVERAGE,
                'max': StatisticsFieldType.MAXVALUE,
...
class RasterJoinPixelFormat(JEnum):
    ...
    @classmethod
    def _externals(cls):
        return {'UBIT1': RasterJoinPixelFormat.RJPMONO,
                'UBIT4': RasterJoinPixelFormat.RJPFBIT,
...
@unique
class DividePolygonOrientation(JEnum):
    """
    ...
    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.data.DividePolygonOrientation'


@unique
class DividePolygonType(JEnum):
    """
    ...
    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.data.DividePolygonType'


@unique
class RegularizeMethod(JEnum):
    """
    :var RegularizeMethod.RIGHTANGLES: 直角
    :var RegularizeMethod.RIGHTANGLESANDDIAGONALS: 直角和对角
    :var RegularizeMethod.ANYANGLE: 任意角
    :var RegularizeMethod.CIRCLE: 圆
    """
    ...
    @classmethod
    def _externals(cls):
        return {'RightAngle': cls.RIGHTANGLES,
                'RightAngleAndDiagonal': cls.RIGHTANGLESANDDIAGONALS}

    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.data.RegularizeMethod'


@unique
class OverlayOutputAttributeType(JEnum):
    """
    多图层叠加分析字段属性返回类型
    ...
    @classmethod
    def _externals(cls):
        return {'id': cls.ONLYID,
                'attributes': cls.ONLYATTRIBUTES}

    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.analyst.spatialanalyst.OverlayOutputAttributeType'


@unique
class TimeDistanceUnit(JEnum):
    """
    时间距离单位
    ...
    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.analyst.spatialstatistics.TimeSpanUnits'


@unique
class GJBLayerType(JEnum):
    """
    ...
    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.data.conversion.GJBLayerType'

@unique
class CSGNodeType(JEnum):
    """
    CSGNode类型
    :var CSGNodeType.CSG_SIMPLENODE: 简单节点，直接挂实体
    :var CSGNodeType.CSG_BOOLEANNODE: 布尔节点，二叉树，左右节点挂CSGNode,可嵌套
    """
    CSG_SIMPLENODE = 1
    ...
    @classmethod
    def _externals(cls):
        return {'SIMPLENODE': cls.CSG_SIMPLENODE,
                'BOOLEANNODE': cls.CSG_BOOLEANNODE}

    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.data.CSGNodeType'

@unique
class CSGEntityType(JEnum):
    """
    CSG实体类型
    """
    CSG_None = 0
    ...
    CSG_Ellipsoid = 5  # 椭圆体
    CSG_Torus = 6  # 圆环体
    CSG_TruncatedCone = 7  # 圆台体
    CSG_Table3D = 8  # 棱台体
    CSG_Wedge = 9  # 楔形体
    CSG_Pyramid = 10  # 棱锥体
    CSG_EllipticRing = 11  # 椭圆环体
    CSG_RectangularRing = 12  # 矩形环体
    CSG_SlopedCylinder = 13  # 斜口圆柱
    CSG_BendingCylinder = 14  # 弯折圆柱体
    #CSG_LoftedBody = 15  # 放样体
    #CSG_Revol = 16  # 旋转体
    #CSG_ExtrudedBody = 17  # 拉伸体

    @classmethod
    def _externals(cls):
        return {'SPHERE': cls.CSG_Sphere,
                'BOX': cls.CSG_Box,
                'CYLINDER': cls.CSG_Cylinder,
                'CONE': cls.CSG_Cone,
                'ELLIPSOID': cls.CSG_Ellipsoid,
                'TORUS': cls.CSG_Torus,
                'TRUNCATEDCONE': cls.CSG_TruncatedCone,
                'TABLE3D': cls.CSG_Table3D,
                'WEDGE': cls.CSG_Wedge,
                'PYRAMID': cls.CSG_Pyramid,
                'ELLIPTICRING': cls.CSG_EllipticRing,
                'RECTANGULARRING': cls.CSG_RectangularRing,
                'SLOPEDCYLINDER': cls.CSG_SlopedCylinder,
                'BENDINGCYLINER': cls.CSG_BendingCylinder}

    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.data.CSGEntityType'

@unique
class CSGBooleanType(JEnum):
    """
    CSG布尔类型
    """
    BOOL_INTERSECTION = 1   #交
    BOOL_DIFFERENCE = 2     #差
    BOOL_UNION = 3          #并

    @classmethod
    def _get_java_class_type(cls):
        return 'com.supermap.data.CSGBooleanType'


@unique
class PromptSampleType(IntEnum):
    LOCALSIMILARITY = 1
    EQUALDISTANCE = 2
