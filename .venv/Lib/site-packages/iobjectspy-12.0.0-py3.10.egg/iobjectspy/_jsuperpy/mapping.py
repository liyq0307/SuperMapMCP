...
__all__ = ['Map', 'LayerSetting', 'LayerSettingImage', 'LayerSettingVector', 'LayerSettingGrid', 'TrackingLayer',
           'Layer', 'LayerGridAggregation', 'LayerHeatmap',
           'ThemeType', 'Theme', 'ThemeUniqueItem', 'ThemeUnique', 'ThemeRangeItem', 'RangeMode', 'ThemeRange',
           'MixedTextStyle', 'LabelMatrix', 'LabelMatrixImageCell', 'LabelMatrixSymbolCell', 'LabelBackShape',
           'AvoidMode', 'AlongLineCulture', 'AlongLineDirection', 'AlongLineDrawingMode',
           'OverLengthLabelMode', 'ThemeLabelRangeItem', 'ThemeLabelRangeItems', 'ThemeLabelUniqueItem',
           'ThemeLabelUniqueItems', 'ThemeLabel', 'ThemeGraphItem', 'ThemeGraphType', 'ThemeGraphTextFormat',
           'GraphAxesTextDisplayMode', 'ThemeGraph', 'GraduatedMode', 'ThemeGraduatedSymbol', 'ThemeDotDensity',
           'ThemeGridUniqueItem', 'ThemeGridUnique', 'ThemeGridRangeItem', 'ThemeGridRange', 'ThemeCustom']
...
class LayerSetting(JVMBase):
    ...
    def _from_java_object(java_object):
        ...
        elif layer_setting_type == 'GRID':
            layer_setting = LayerSettingGrid()
        elif layer_setting_type == 'IMAGE':
...
class TrackingLayer(JVMBase):
    ...
    def add(self, geo, tag):
        ...
        if isinstance(geo, Rectangle):
            geo = geo.to_region()
        elif isinstance(geo, Point2D):
            geo = GeoPoint(geo)
        elif isinstance(geo, Feature):
            geo = geo.geometry
    ...
    # def cancel_bulk_operated(self):
    #     """
    #     取消本次批量更新并恢复到上次提交点编辑的内容，但可以继续进行批量编辑。
    #
    #     :return: 取消批量更新返回 true，否则返回 false。
    #     :rtype: bool
    #     """
    #     return self._jobject.cancelBulkOperated()
...
class Map(JVMBase):
    ...
    def _output_to_file(that_map, file_name, image_type, is_back_transparent):
        ...
        elif image_type is ImageType.BMP:
            is_ok = oj(that_map).outputMapToBMP(file_name)
        elif image_type is ImageType.JPG:
            is_ok = oj(that_map).outputMapToJPG(file_name)
        elif image_type is ImageType.PNG:
            is_ok = oj(that_map).outputMapToPNG(file_name, is_back_transparent)
        elif image_type is ImageType.TIFF:
            is_ok = oj(that_map).outputMapToFile(file_name, oj(image_type),
                                                 int(that_map.get_dpi()),
                                                 oj(that_map.get_view_bounds()),
                                                 is_back_transparent)
        elif image_type is ImageType.PDF:
            is_ok = oj(that_map).outputMapToPDF(file_name)
        else:
            raise ValueError('Unsupported image type ' + image_type.name)
        return is_ok

    def output_to_file(self, file_name, output_bounds=None, dpi=0, image_size=None, is_back_transparent=False,
                       is_show_to_ipython=False):
        """
        ...
        t_map = None
        if (output_bounds and not output_bounds.is_empty()) or dpi > 0 or isinstance(image_size, tuple):
            t_map = Map()
            t_map.from_xml(self.to_xml())
            if dpi > 0:
                t_map.set_use_system_dpi(False)
                t_map.set_dpi(int(dpi))
            if isinstance(image_size, tuple) and len(image_size) > 1:
        ...
        if is_ok and image_type in (ImageType.JPG, ImageType.PNG, ImageType.GIF):
    ...
    def output_tracking_layer_to_png(self, file_name, output_bounds=None, dpi=0,
                                     is_back_transparent=False,
                                     is_show_to_ipython=False):
        """
        ...
        if is_ok and is_show_to_ipython:
    ...
    def add_dataset(self, dataset, is_add_to_head=True, layer_setting=None):
        ...
        if not isinstance(dt, (DatasetVector, DatasetImage, DatasetGrid)):
            raise ValueError('Required DatasetVector, DatasetImage, DatasetGrid, but is ' + str(dt.type))
        if isinstance(layer_setting, LayerSetting):
            java_layer = self._jobject.getLayers().add(oj(dt), oj(layer_setting), parse_bool(is_add_to_head))
        elif isinstance(layer_setting, Theme):
            java_layer = self._jobject.getLayers().add(oj(dt), oj(layer_setting), parse_bool(is_add_to_head))
        else:
    ...
    def add_to_tracking_layer(self, geos, style=None, is_antialias=False, is_symbol_scalable=False,
                              symbol_scale=None):
        """
        添加几何对象到跟踪图层
    ...
    def remove_layer(self, index_or_name):
        ...
        if isinstance(index_or_name, int):
            ...
            return False
        elif isinstance(index_or_name, Layer):
    ...
    def add_heatmap(self, dataset, kernel_radius, max_color=None, min_color=None):
        ...
        if min_color and max_color:
            layer = self._jobject.getLayers().AddHeatmap(oj(dt), int(kernel_radius), to_java_color(min_color),
                                                         to_java_color(max_color))
        else:
    ...
    def add_aggregation(self, dataset, min_color=None, max_color=None):
        ...
        if min_color and max_color:
            layer = self._jobject.getLayers().AddGridAggregation(oj(dt), to_java_color(min_color),
                                                                 to_java_color(max_color))
        else:
...
class Theme(JVMBase):
    ...
    def _create_instance(theme_type):
        if theme_type is ThemeType.UNIQUE:
            theme = ThemeUnique()
        elif theme_type is ThemeType.GRAPH:
            theme = ThemeGraph()
        elif theme_type is ThemeType.LABEL:
            theme = ThemeLabel()
        elif theme_type is ThemeType.DOTDENSITY:
            theme = ThemeDotDensity()
        elif theme_type is ThemeType.GRADUATEDSYMBOL:
            theme = ThemeGraduatedSymbol()
        elif theme_type is ThemeType.GRIDUNIQUE:
            theme = ThemeGridUnique()
        elif theme_type is ThemeType.GRIDRANGE:
            theme = ThemeGridRange()
        elif theme_type is ThemeType.RANGE:
            theme = ThemeRange()
        elif theme_type is ThemeType.CUSTOM:
            theme = ThemeCustom()
        else:
            theme = None
        return theme
    ...
    def make_from_xml(xml):
        ...
        import os
        if not xml.startswith("<sml:Theme>") and os.path.isfile(xml):
            all_lines = []
...
class ThemeUniqueItem:
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return ThemeUniqueItem(java_object.getUnique(), GeoStyle._from_java_object(java_object.getStyle()),
                               java_object.getCaption(), java_object.isVisible())
...
class ThemeRangeItem:
    ...
    def __str__(self):
        return 'ThemeRangeItem(start={}, end={}, caption={}, visible={})'.format(self.start, self.end, self.caption,
                                                                                 self.visible)
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return ThemeRangeItem(java_object.getStart(), java_object.getEnd(),
                              GeoStyle._from_java_object(java_object.getStyle()),
                              java_object.getCaption(), java_object.isVisible())
...
class RangeMode(JEnum):
    ...
    @classmethod
    def _externals(cls):
        return {'square': RangeMode.SQUAREROOT,
                'log': RangeMode.LOGARITHM,
...
class ThemeRange(Theme):
    ...
    @staticmethod
    def make_default(dataset, expression, range_mode, range_parameter, color_gradient_type=None, range_precision=0.1,
                     join_items=None):
        """
        ...
        java_theme = get_jvm().com.supermap.mapping.ThemeRange.makeDefault(oj(dataset),
                                                                           str(expression),
                                                                           oj(range_mode),
                                                                           float(range_parameter),
                                                                           java_color_gradient_type,
                                                                           java_join_items,
...
class MixedTextStyle:
    ...
    def __init__(self, default_style=None, styles=None, separator=None, split_indexes=None):
        ...
        self.set_styles(styles).set_default_style(default_style).set_separator(separator).set_split_indexes(
            split_indexes)
    ...
    def _from_java_object(java_object):
        ...
        return MixedTextStyle(default_style, styles, java_object.getSeparator(),
                              java_array_to_list(java_object.getSplitIndexes())).set_separator_enabled(
            java_object.isSeparatorEnabled())
...
class LabelMatrix:
    ...
    def set(self, col, row, value):
        ...
        if not isinstance(value, (LabelMatrixImageCell, LabelMatrixSymbolCell, ThemeLabel)):
            raise ValueError(
                'required LabelMatrixImageCell, LabelMatrixSymbolCell, ThemeLabel, but ' + str(type(value)))
    ...
    def _jobject(self):
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                java_object.set(int(col), int(row), oj(self.get(col, row)))
        return java_object
...
class LabelMatrixImageCell:
    ...
    def __init__(self, path_field, width=1.0, height=1.0, rotation=0.0, is_size_fixed=False):
        ...
        self.set_path_field(path_field).set_width(width).set_height(height).set_rotation(rotation).set_size_fixed(
            is_size_fixed)
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return LabelMatrixImageCell(java_object.getPathField(),
                                    java_object.getWidth(),
                                    java_object.getHeight(),
                                    java_object.getRotation(),
                                    java_object.isSizeFixed())
...
class ThemeLabelRangeItem:
    ...
    def __init__(self, start, end, caption, style, visible=True, offset_x=0.0, offset_y=0.0):
        ...
        self._offset_x = 0.0
        self._offset_y = 0.0
        self.set_start(start).set_end(end).set_style(style).set_caption(caption).set_visible(visible).set_offset_x(
            offset_x).set_offset_y(offset_y)
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return ThemeLabelRangeItem(java_object.getStart(), java_object.getEnd(),
                                   TextStyle._from_java_object(java_object.getStyle()),
                                   java_object.getCaption(), java_object.isVisible(),
                                   java_object.getOffsetX(), java_object.getOffsetY())
...
class ThemeLabelUniqueItem:
    ...
    def __init__(self, unique_value, caption, style, visible=True, offset_x=0.0, offset_y=0.0):
        ...
        self._offset_x = 0.0
        self._offset_y = 0.0
        self.set_unique_value(unique_value).set_style(style).set_caption(caption).set_visible(visible).set_offset_x(
            offset_x).set_offset_y(offset_y)
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return ThemeLabelUniqueItem(java_object.getUnique(), TextStyle._from_java_object(java_object.getStyle()),
                                    java_object.getCaption(), java_object.isVisible(), java_object.getOffsetX(),
                                    java_object.getOffsetY())
...
class ThemeLabel(Theme):
    ...
    def set_along_line(self, is_along_line=True, is_angle_fixed=False, culture=None, direction='ALONG_LINE_NORMAL',
                       drawing_mode='COMPATIBLE', space_ratio=None, word_angle_range=None,
                       repeat_interval=0, is_repeat_interval_fixed=False, is_repeated_label_avoided=False):
        """
    ...
    def get_text_extent_inflation(self):
        ...
        if java_size:
            return java_size.getWidth(), java_size.getHeight()
        return None
    ...
    def set_unique_label(self, unique_expression):
        ...
        if unique_expression:
            self._jobject.setUniqueExpression(unique_expression)
        return self
    ...
    def make_default_unique(dataset, label_expression, unique_expression, color_gradient_type=None, join_items=None):
        ...
        if join_items:
            ...
            java_join_items = None
        java_theme = get_jvm().com.supermap.mapping.ThemeLabel.makeDefault(oj(dataset), str(unique_expression),
                                                                           oj(color_gradient_type),
                                                                           java_join_items)
    ...
    @staticmethod
    def make_default_range(dataset, label_expression, range_expression, range_mode, range_parameter,
                           color_gradient_type=None, join_items=None):
        """
        生成默认的分段标签专题图。
        ...
        if join_items and len(join_items) > 0:
            ...
            java_join_items = None
        java_color_gradient_type = oj(color_gradient_type)
        java_theme = get_jvm().com.supermap.mapping.ThemeLabel.makeDefault(oj(dataset),
                                                                           str(range_expression),
                                                                           oj(range_mode),
                                                                           float(range_parameter),
                                                                           java_color_gradient_type,
                                                                           java_join_items)
...
class ThemeGraphItem:
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return ThemeGraphItem(java_object.getGraphExpression(),
                              java_object.getCaption(),
                              GeoStyle._from_java_object(java_object.getUniformStyle()),
                              Theme._from_java_object(java_object.getRangeSetting()))
...
class ThemeGraph(Theme):
    ...
    def set_axes(self, is_displayed=True, color=(128, 128, 128), is_text_displayed=False,
                 text_display_mode='', text_style=None, is_grid_displayed=False):
        """
...
class GraduatedMode(JEnum):
    ...
    @classmethod
    def _externals(cls):
        return {'square': GraduatedMode.SQUAREROOT,
                'log': GraduatedMode.LOGARITHM,
...
class ThemeGraduatedSymbol(Theme):
    ...
    def __init__(self, expression=None, base_value=0, positive_style=None, graduated_mode='CONSTANT'):
        ...
        self.set_expression(expression).set_base_value(base_value). \
            set_graduated_mode(graduated_mode).set_positive_style(positive_style)
    ...
    def make_default(dataset, expression, graduated_mode='CONSTANT'):
        ...
        if not expression:
            expression = ''
        java_theme = get_jvm().com.supermap.mapping.ThemeGraduatedSymbol.makeDefault(oj(dataset), str(expression),
                                                                                     oj(graduated_mode))
        return Theme._from_java_object(java_theme)
...
class ThemeGridUniqueItem:
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return ThemeGridUniqueItem(java_object.getUnique(), Color._from_java_object(java_object.getColor()),
                                   java_object.getCaption(), java_object.isVisible())
...
class ThemeGridRangeItem:
    ...
    def _from_java_object(java_object):
        if not java_object:
            return None
        return ThemeGridRangeItem(java_object.getStart(), java_object.getEnd(),
                                  Color._from_java_object(java_object.getColor()),
                                  java_object.getCaption(), java_object.isVisible())
...
class ThemeGridRange(Theme):
    ...
    def make_default(dataset, range_mode, range_parameter, color_gradient_type=None):
        ...
        if range_mode is RangeMode.STDDEVIATION:
            range_parameter = 0.0
        java_color_gradient_type = oj(color_gradient_type)
        java_theme = get_jvm().com.supermap.mapping.ThemeGridRange.makeDefault(oj(dataset),
                                                                               oj(range_mode),
                                                                               float(range_parameter),
                                                                               java_color_gradient_type)
        return Theme._from_java_object(java_theme)
