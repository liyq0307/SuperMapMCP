    ...
    def forward(self, inputs, initial_hidden_state):
        ...
        for i_layer in range(self._num_rnn_layers):
            ...
            if torch.cuda.device_count()>0:
                current_inputs = current_inputs.cuda()  # seq_len, B, ...
...
class DCRNNModel(BaseModel):
    ...
    def forward(self, source, target,teacher_forcing_ratio):
        ...
        # self.GO_Symbol = torch.zeros(1, target.shape[1], target.shape[2], target.shape[3]).cuda()
        # self._batch_size = target.shape[1]
        # print("target.shape=",target.shape)
        # print("GO_Symbol.shape=",GO_Symbol.shape)
        # print("target.device=",target.device)
        # print("GO_Symbol.device=",GO_Symbol.device)
