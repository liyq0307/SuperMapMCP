...
class DCRNNTrainer(BaseTrainer):
    ...
    def __init__(
            self, model, loss, metrics, optimizer, config, data_loader,
            epochs = None,
        ...
        # self.log_step = int(np.sqrt(data_loader.batch_size))  # sqrt(128)
        # sqrt(64)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(
                self.data_loader.get_iterator()):
            data = torch.FloatTensor(data)
            target = torch.FloatTensor(target)
            # (..., 1)  supposed to be numpy array
            label = target[..., :self._output_dim]
            # label = target[..., :self.model.output_dim]  # (..., 1)  supposed
            # to be numpy array
            data, target = data.to(self.device), target.to(self.device)

            # GO_Symbol = torch.zeros(1, self.model.batch_size,
            # GO_Symbol = torch.zeros(1, self._batch_size,
            #                        self._num_nodes * self._output_dim,1)
            # GO_Symbol = GO_Symbol.to(self.device)

            self.optimizer.zero_grad()

            # compute sampling ratio, which gradually decay to 0 during
            # training
            ...
            # output = torch.transpose(output.view(12, self.model.batch_size,
