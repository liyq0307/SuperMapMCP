...
def binary_classification_all(inference_data, ground_truth_data, inference_class_value_field,
                              ground_truth_class_value_field,
                              out_data, out_data_name, nproc=4):
    """
        基于输入的真实标签数据和预测标签数据计算结果的iou, pixel_acc, f1, dice, kappa, confusion_matrix，支持影像和影像数据计算，矢量和矢量数据计算

    """

    CPA,IoU, F1, Kappa, confusion_matrix = _seg_all(inference_data, ground_truth_data,
                                                                 true_field_name=ground_truth_class_value_field,
                                                                 predict_field_name=inference_class_value_field,
                                                                 compute_type='binary', average='macro')
    metric_dict = {'CPA': CPA, 'IoU': IoU, 'F1': F1, 'Kappa': Kappa}
...
def binary_classification_iou(inference_data, ground_truth_data, inference_class_value_field,
                              ground_truth_class_value_field,
    ...
    IoU = _seg_iou(ground_truth_data, inference_data, true_field_name=ground_truth_class_value_field,
                   predict_field_name=inference_class_value_field, compute_type='binary',
                   average='macro')
    metric_dict = {'IoU': IoU}
    if out_data is not None:
        save_metric_dict_to_attribute_table(metric_dict, out_data, out_data_name)
...
def binary_classification_pixel_acc(inference_data, ground_truth_data, inference_class_value_field,
                                    ground_truth_class_value_field,
                                    out_data, out_data_name):
    """
        基于输入的真实标签数据和预测标签数据计算结果的IOU得分，支持影像和影像数据计算，矢量和矢量数据计算
...
def binary_classification_f1(inference_data, ground_truth_data, inference_class_value_field,
                             ground_truth_class_value_field,
                             out_data, out_data_name):
    """
        基于输入的真实标签数据和预测标签数据计算结果的IOU得分，支持影像和影像数据计算，矢量和矢量数据计算
...
def binary_classification_kappa(inference_data, ground_truth_data, inference_class_value_field,
                                ground_truth_class_value_field,
                                out_data, out_data_name):
    """
        基于输入的真实标签数据和预测标签数据计算结果的IOU得分，支持影像和影像数据计算，矢量和矢量数据计算
...
def binary_classification_confusion_matrix(inference_data, ground_truth_data, inference_class_value_field,
                                           ground_truth_class_value_field,
                                           out_data, out_data_name):
    """
        基于输入的真实标签数据和预测标签数据计算结果的IOU得分，支持影像和影像数据计算，矢量和矢量数据计算
