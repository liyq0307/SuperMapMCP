...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2],
                 num_heads=[3, 6, 12, 24], window_size=6, mlp_ratio=4., qkv_bias=True, qk_scale=None,
...
class SwinEncoder(SwinTransformer, EncoderMixin):
    ...
    def load_state_dict(self, model_dict, in_channels=3):
        ...
        model_dict = remap_pretrained_keys_swin(model_dict, self.window_size)
...
def remap_pretrained_keys_swin(checkpoint_model, window_size):
    ...
    for table_key in relative_position_bias_table_keys:
        ...
        L2 = (2 * window_size - 1) * (2 * window_size - 1)
        if L1 != L2:
            S1 = int(L1 ** 0.5)
            S2 = int(L2 ** 0.5)
            ...
            checkpoint_model[table_key] = table_pretrained_resized.view(nH1, L2).permute(1, 0)
...
swin_encoders = {
    ...
    "swin_large": {
        ...
        "pretrained_settings": {
            "imagenet": {
                "url": "https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_large_patch4_window14_384_22k.pth",
                "input_space": "RGB",
                "input_size": [3, 224, 224],
                "input_range": [0, 1],
                "mean": [0.485, 0.456, 0.406],
