...
class ASPPPooling(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        super(ASPPPooling, self).__init__(
            ...
            # nn.BatchNorm2d(out_channels), # batchsize 1 error
            nn.ReLU())
...
class DeepLabV3PlusDecoder(nn.Sequential):
    def __init__(self, in_channels, out_channels=256, atrous_rates=(12, 24, 36)):
        ...
        self.shortcut_conv = nn.Sequential(
            nn.Conv2d(indim, 48, 1, 1,
                      padding=1 // 2, bias=True),
            BatchNorm2d(48, momentum=0.1),
            nn.ReLU(inplace=True),
        )
        self.cat_conv = nn.Sequential(
            nn.Conv2d(out_channels+48, out_channels, 3, 1, padding=1,
                      bias=True),
            BatchNorm2d(out_channels, momentum=0.1),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Conv2d(out_channels, out_channels, 3, 1, padding=1, bias=True),
            BatchNorm2d(out_channels, momentum=0.1),
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),
        )
