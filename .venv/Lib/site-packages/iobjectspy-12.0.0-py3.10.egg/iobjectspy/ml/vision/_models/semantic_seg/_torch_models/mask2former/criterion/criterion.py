...
from ...util._toolkit import is_dist_avail_and_initialized, nested_tensor_from_tensor_list, get_world_size
...
# function of torch.jit.script is for accelerating
# torch.jit.script(dice_loss)  # type: torch.jit.ScriptModule
...
# function of torch.jit.script is for accelerating
# torch.jit.script(sigmoid_ce_loss)  # type: torch.jit.ScriptModule
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
        ...
        self.losses = losses
        empty_weight = Device_type.to_device(torch.ones(self.num_classes + 1))
    ...
    def _get_binary_mask(self, target):
        ...
        target_onehot = torch.zeros(256, y, x).to(target.device)
        target_onehot = target_onehot.scatter(dim=0, index=target.unsqueeze(0), value=1)
        target_onehot = target_onehot[0:self.num_classes, :, :]
        return target_onehot

    def prepare_targets(self, masks, labels):
        new_targets = []
        for targets_per_image, labels_per_image in zip(masks, labels):
            new_targets.append(
                {
                    "masks": labels_per_image,
                    "labels": targets_per_image,
                }
            )
        return new_targets
    ...
    def forward(self, outputs, gt_masks):
        ...
        targets = self._get_targets(gt_masks)   ## prepare the label and mask
        # targets = self.prepare_targets(gt_masks, gt_labels)
