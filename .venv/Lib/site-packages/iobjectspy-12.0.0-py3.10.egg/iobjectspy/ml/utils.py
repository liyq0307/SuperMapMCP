...
def recordset_to_numpy_array(recordset, fields=None, export_spatial=False, skip_null_value=True, null_values=None):
    ...
    写出矢量数据时，可以选择是否写出空间信息，对于点对象，写出点的 X 和 Y 坐标到 SmX 列和 SmY 列中；对线对象，将写出线的内点 :py:meth:`GeoLine.get_inner_point` 到 SmX 列和 SmY列，并将线
    ...
    :param bool export_spatial: 是否写出空间几何信息，对点对象，将直接写出点的 X 和 Y 坐标到 ‘SmX’ 和 ‘SmY’ 列中。对于线面对象，将写出线和面对象的内点坐标，并输出到 ‘SmX’ 和 ‘SmY’ 列中，同时，线将写出 ‘SmLength’字段，面将写出 ‘SmPerimeter’ 和 ‘SmArea 字段。
    ...
    :param null_values: 指定的字段对应的空值，key 值为字段名称或字段序号，value 为指定的空值，value 的值的类型需要与字段的类型匹配。例如，需要指定整型字段 ID 的空值为 -9999，则 null_values = {'ID': -9999}
...
def datasetvector_to_numpy_array(source, attr_filter=None, fields=None, export_spatial=False, skip_null_value=True,
                                 null_values=None):
    """
    ...
    :type fields: list[str]
    :param bool export_spatial: 是否导出空间几何对象。对点对象，将直接写出点的 X 和 Y 坐标到 ‘SmX’ 和 ‘SmY’ 列中。对于线面对象，将写出线和面对象的内点，并输出到 ‘SmX’ 和 ‘SmY’ 列中，同时，线将写出 ‘SmLength’字段，面将写出 ‘SmPerimeter’ 和 ‘SmArea 字段。
    :param is skip_null_value: 是否跳过含有空值的记录。numpy数组中，不支持整型类型的值为空值，如果列的类型为整型类型但又是空值，可能会写出失败。所以，如果数据集中字段含有空值，需要填入一个空值。对于浮点型的字段，空值为 numpy.nan，对于文本型（TEXT，WTEXT，CHAR，JSONB）为空字符串，布尔类型、二进制类型和时间类型字段的空值为 None。
    :param null_values: 指定的字段对应的空值，key 值为字段名称或字段序号，value 为指定的空值，value 的值的类型需要与字段的类型匹配。例如，需要指定整型字段 ID 的空值为 -9999，则 null_values = {'ID': -9999}
    :type null_values: dict
    :return: numpy 数组，将返回一个维度为1的数组。
...
def datasetraster_to_numpy_array(source, no_value=None, origin_is_left_up=True):
    ...
    数据集，返回一个三维数组，数组的第一维度为波段，大小为 3（RGB）或 4（RGBA），数组的第二维度为行，第三维度为列。
    ...
    :param source: 需要读取数据的栅格数据集或影像数据集对象。如果输入为 str, 则可以使用数据源信息加数据集名称方式表示数据集，
                   例如，可使用数据源别名'alias/imagedataset',也可以使用数据源的连接信息（udb文件路径， dcf文件路径，或者数据
    ...
    :param float no_value: SuerpMap 的栅格数据集和影像数据集均有无值，用户可以设定一个新值来表示数据集的无值，如果 noValue
                           不为 None，在返回的 numpy 数组中会使用用户设定的 noValue值来替换无值的栅格或像素。默认为 None，即返回
                           的数组的无值不改变。
    :param bool origin_is_left_up: 在返回的 numpy 数组中，(0,0) 位置的值是数据集的左上角还是左下角，如果为 True，则表示数组中
                                  （0,0）的值对应数据集的左上角的值， ndarray[i][j] 对应数据集中 第i行j列的数值。如果为 False，
                                  则数组的 (0,0) 的值为数据集的左下角的值，即 ndarray[i][j] 对应数据集中第(height-i)行j列的
...
def numpy_array_to_datasetraster(narray, x_resolution, y_resolution, output, x_start=0, y_start=0,
                                 out_dataset_name=None, no_value=None,
                                 origin_is_left_up=True, as_grid=False):
    """
    ...
    return numpy_array_to_datasetraster(narray, x_resolution, y_resolution, output, x_start, y_start,
                                        out_dataset_name, no_value, origin_is_left_up, as_grid)
...
def recordset_to_df(recordset, fields=None, export_spatial=False, skip_null_value=True, null_values=None):
    ...
    :param fields: 需要写出的字段名称，如果为 None，则将全部非系统字段都写出
    :type fields: list[str]
    :param bool export_spatial: 是否写出空间几何信息，对点对象，将直接写出点的 X 和 Y 坐标到 ‘SmX’ 和 ‘SmY’ 列中。对于线面对象，将写出线和面对象的内点，并输出到 ‘SmX’ 和 ‘SmY’ 列中，同时，线将写出 ‘SmLength’字段，面将写出 ‘SmPerimeter’ 和 ‘SmArea 字段。
...
def datasetvector_to_df(source, attr_filter=None, fields=None, export_spatial=False, skip_null_value=True,
                        null_values=None):
    """
...
def datasetraster_to_df_or_xarray(source, no_value=None, origin_is_left_up=True):
    ...
    :param bool origin_is_left_up: 在返回的 DataFrame 或 DataArray 中，(0,0) 位置的值是数据集的左上角还是左下角，如果为 True，
                                   则表示数组（0,0）的值对应数据集的左上角的值， DataFrame[i][j] 对应数据集中 第i行j列的数值；
                                   如果为 False，则数组的 (0,0) 的值为数据集的左下角的值，即 DataFrame[i][j] 对应数据集中第 (height-i)行j列的
                                   数值。在 SuperMap 的栅格数据集或影像数据集中，数据集的 (0,0) 位置为左上角。默认为 True。
...
def df_or_xarray_to_datasetraster(data, x_resolution, y_resolution, output, x_start=0, y_start=0,
                                  out_dataset_name=None, no_value=None,
                                  origin_is_left_up=True, as_grid=False):
    """
    ...
    return df_or_xarray_to_datasetraster(data, x_resolution, y_resolution, output, x_start, y_start,
                                         out_dataset_name, no_value, origin_is_left_up, as_grid)
